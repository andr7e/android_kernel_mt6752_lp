...
#ifdef CONFIG_HTC_RECEIVER
#include <linux/htc_receiver.h>
#endif
...
static bool mHeadsetSpeaker = false;
#ifdef CONFIG_MTK_SPEAKER
...
static bool MuteLeftFlag = false;
#endif
...
static void Voice_Amp_Change(bool enable)
{
    if (enable)
    {
        ...
        if (GetDLStatus() == false)
        {
            ...
            Ana_Set_Reg(AUDDEC_ANA_CON6, 0x2BC6, 0xffff); 
...
#define AW8736_BACK_MODE1  \
    mt_set_gpio_out(pin_extspkamp_2,GPIO_OUT_ONE);
#define AW8736_FRONT_MODE1  \
    mt_set_gpio_out(pin_extspkamp,GPIO_OUT_ONE);
...
#define SPK_WARM_UP_TIME        (25) 
	...
	AW8736_BACK_MODE1;
        if (MuteLeftFlag == false) {
	    mt_set_gpio_dir(GPIO_SPEAKER_RECEIVER_SWITCH_PIN,GPIO_DIR_OUT); 
	    mt_set_gpio_out(GPIO_SPEAKER_RECEIVER_SWITCH_PIN,GPIO_OUT_ONE); 
            printk("%s() Trun on Left SPK due to not mute left SPK before\n", __func__);
	    AW8736_FRONT_MODE1;
        }
#else
        mt_set_gpio_out(pin_extspkamp, GPIO_OUT_ONE); 
...
#ifndef CONFIG_MTK_SPEAKER
static int Mute_Left_Speaker_Get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
    printk("%s()\n", __func__);
    ucontrol->value.integer.value[0] = MuteLeftFlag ? 1 : 0;
    return 0;
}

static int Mute_Left_Speaker_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
    if (ucontrol->value.integer.value[0])
    {
        printk("%s() Value is %ld to Mute left SPK", __func__, ucontrol->value.integer.value[0]);
        MuteLeftFlag = true;
        mt_set_gpio_dir(GPIO_SPEAKER_RECEIVER_SWITCH_PIN,GPIO_DIR_OUT); 
        mt_set_gpio_out(GPIO_SPEAKER_RECEIVER_SWITCH_PIN,GPIO_OUT_ZERO); 
        mt_set_gpio_dir(pin_extspkamp, GPIO_DIR_OUT);
    ...
    {
        int rSPKGpio = mt_get_gpio_out(pin_extspkamp_2);
        MuteLeftFlag = false;
        if (1 == rSPKGpio) {
            printk("%s() Value is %ld to Unmute left SPK", __func__, ucontrol->value.integer.value[0]);
            mt_set_gpio_dir(GPIO_SPEAKER_RECEIVER_SWITCH_PIN,GPIO_DIR_OUT); 
            mt_set_gpio_out(GPIO_SPEAKER_RECEIVER_SWITCH_PIN,GPIO_OUT_ONE); 
            mt_set_gpio_dir(pin_extspkamp, GPIO_DIR_OUT);
            mt_set_gpio_out(pin_extspkamp, GPIO_OUT_ONE);
...
static int Mixer_To_Right_Get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
    return 0;
}

static int Mixer_To_Right_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
    return 0;
}
#endif

static int Headset_Speaker_Switch_Get(struct snd_kcontrol *kcontrol,
                               struct snd_ctl_elem_value *ucontrol)
{
    ucontrol->value.integer.value[0] = mHeadsetSpeaker;
    printk("eleven: %s() Headset_Speaker_Switch = %d \n ", __func__, mHeadsetSpeaker);

    return 0;
}

static int Headset_Speaker_Switch_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{

    printk("eleven: %s() Headset_Speaker_Switch = %ld \n ", __func__, ucontrol->value.integer.value[0]);

    mHeadsetSpeaker = ucontrol->value.integer.value[0];
    if (mHeadsetSpeaker)
    {
#ifdef CONFIG_HTC_RECEIVER
        setHeadsetSpeaker(true);
#endif
    }
    else
    {
#ifdef CONFIG_HTC_RECEIVER
        setHeadsetSpeaker(false);
#endif
    }
    return 0;
}
...
static const char *const Mixer_To_Right_Text[] = {"disable", "enable"};
static const struct soc_enum Mute_Left_SPK_Enum[] = {
    SOC_ENUM_SINGLE_EXT(2, Mute_Left_SPK_Text),
    SOC_ENUM_SINGLE_EXT(2, Mixer_To_Right_Text),
};
#endif
...
static const struct snd_kcontrol_new mt6331_snd_controls[] =
{
    ...
    SOC_ENUM_EXT("Headset_Speaker_Switch", Audio_DL_Enum[12], Headset_Speaker_Switch_Get, Headset_Speaker_Switch_Set),
    SOC_SINGLE_EXT("Audio HP Impedance", SND_SOC_NOPM, 0, 512, 0, Audio_Hp_Impedance_Get, Audio_Hp_Impedance_Set),
#ifndef CONFIG_MTK_SPEAKER
    SOC_ENUM_EXT("Mute_Left_SPK", Mute_Left_SPK_Enum[0], Mute_Left_Speaker_Get, Mute_Left_Speaker_Set),
    SOC_ENUM_EXT("Mixer_To_Right", Mute_Left_SPK_Enum[1], Mixer_To_Right_Get, Mixer_To_Right_Set),
#endif
};
    ...
    else
    {
        ...
        else if (ADCType == AUDIO_ANALOG_DEVICE_IN_ADC2) 
        {
            if (refmic_using_ADC_L == false)
            {
                ...
                Ana_Set_Reg(AUDENC_ANA_CON3, 0x0000, 0xffff);   
                Ana_Set_Reg(AUDENC_ANA_CON15, 0x0000, 0x00f0); 
...
static bool TurnOnADcPowerDCC(int ADCType, bool enable)
{
    ...
    if (enable)
    {
        ...
        else if (ADCType == AUDIO_ANALOG_DEVICE_IN_ADC2) 
        {
            ...
            Ana_Set_Reg(AUDENC_ANA_CON15, 0x0020, 0x00f0); 
    ...
    if (enable)
    {
        ...
        switch (MicType)
        {
            case AUDIO_VOW_MIC_TYPE_Headset_MIC:
                ...
                mt_set_gpio_mode(pin_vowclk, GPIO_MODE_01); 
                
                mt_set_gpio_mode(pin_audmiso, GPIO_MODE_02); 
#else
            ...
            case AUDIO_VOW_MIC_TYPE_Handset_DMIC_VENDOR01:
                ...
                mt_set_gpio_mode(GPIO_ALS_EINT_PIN, GPIO_MODE_04); 
#ifdef MTK_VOW_SUPPORT
#ifdef CONFIG_OF
    ...
    else
    {
        ...
        switch (MicType)
        {
            case AUDIO_VOW_MIC_TYPE_Headset_MIC:
                ...
                mt_set_gpio_mode(pin_vowclk, GPIO_MODE_00); 
                ...
                Ana_Set_Reg(AUDENC_ANA_CON9, (MicbiasRef|0x0000), 0x0f01);  
                Ana_Set_Reg(AUDDEC_ANA_CON7, 0x0000, 0x0006);  
...
#endif
#endif
                Ana_Set_Reg(AFE_VOW_TOP, 0x0, 0x80); 
            ...
            case AUDIO_VOW_MIC_TYPE_Handset_DMIC_800K:
                ...
                Ana_Set_Reg(AUDENC_ANA_CON9, (MicbiasRef|0x0000), 0x0701); 
                Ana_Set_Reg(AUDENC_ANA_CON2, 0x0000, 0x0002); 
            ...
            case AUDIO_VOW_MIC_TYPE_Headset_MIC_DCCECM:
                ...
                Ana_Set_Reg(AUDENC_ANA_CON9, (MicbiasRef|0x0000), 0x0f10);   
                Ana_Set_Reg(AUDDEC_ANA_CON7, 0x0000, 0x0006);  
                Ana_Set_Reg(AUDDEC_ANA_CON6, 0x0000, 0x0100);  
                Ana_Set_Reg(AUDENC_ANA_CON3, 0x0000, 0x000D);  
                Ana_Set_Reg(AFE_DCCLK_CFG0, 0x2064, 0xffff);
