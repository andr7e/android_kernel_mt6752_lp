...
static int Audio_i2s0_SideGen_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
    ...
    {
        printk("%s() Soc_Aud_InterCon_Connection  I00  O14\n",  __func__);
        
        SetConnection(Soc_Aud_InterCon_Connection, Soc_Aud_InterConnectionInput_I00, Soc_Aud_InterConnectionOutput_O14);
    ...
    if (mi2s0_sidegen_control != 0)
    {
        ...
        if (mi2s0_hdoutput_control == true)
        {
            ...
            MclkDiv0 = SetCLkMclk(Soc_Aud_I2S0, samplerate); 
            SetCLkBclk(MclkDiv0, samplerate, 2, Soc_Aud_I2S_WLEN_WLEN_32BITS);

            MclkDiv3 = SetCLkMclk(Soc_Aud_I2S3, samplerate); 
