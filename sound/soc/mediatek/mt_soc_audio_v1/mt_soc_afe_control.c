...
static AudioDigtalI2S *m2ndI2Sout = NULL; 
...
bool Register_Aud_Irq(void *dev, uint32 afe_irq_number)
{
    ...
    ret = request_irq(MT6595_AFE_MCU_IRQ_LINE, AudDrv_IRQ_handler, IRQF_TRIGGER_LOW, "Afe_ISR_Handle", dev);
#endif
    return ret;
}
...
bool Set2ndI2SAdcIn(AudioDigtalI2S *DigtalI2S)
{
    ...
    if (false == Audio2ndAdcI2SStatus)
    {
        ...
        Afe_Set_Reg(AFE_ADDA2_NEWIF_CFG0, 0x03F87201, 0xFFFFFFFF); 
    }
    else
    {
        Afe_Set_Reg(AFE_ADDA_TOP_CON0, 1, 0x1); 
    ...
    else if (Enable)
    {
        switch (connection)
        {
            ...
            case Soc_Aud_InterConnectionOutput_O12:
                if (Soc_Aud_I2S_SAMPLERATE_I2S_8K == mAudioMEMIF[Soc_Aud_Digital_Block_MEM_MOD_DAI]->mSampleRate) 
                {
...
bool EnableSideToneFilter(bool stf_on)
{
    ...
    else
    {
        ...
        const bool read_write_sel = true;    
