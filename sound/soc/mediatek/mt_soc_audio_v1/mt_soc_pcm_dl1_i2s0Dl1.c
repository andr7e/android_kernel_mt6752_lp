...
static int Audio_I2S0dl1_hdoutput_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
    ...
    if ((ucontrol->value.integer.value[0] == true) &&(mI2S0dl1_hdoutput_control == false))
    {
    ...
    else  if ((ucontrol->value.integer.value[0] == false) &&(mI2S0dl1_hdoutput_control == true))
    {
...
Audio_Speaker_Switch_get(struct snd_kcontrol *kcontrol,
        struct snd_ctl_elem_value *ucontrol)
{
...
Audio_Receiver_Switch_get(struct snd_kcontrol *kcontrol,
        struct snd_ctl_elem_value *ucontrol)
{
...
Audio_Speaker_Switch_set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	int useSram;
...
Audio_Receiver_Switch_set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	int useSram;
	printk("%s () %d\n", __func__, (int) ucontrol->value.integer.value[0]);
...
static int Audio_Dl1_MD_Echo_Ref_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
    ...
    if (mDl1_md_echo_ref_control)
    {
        ...
        SetConnection(Soc_Aud_InterCon_Connection, Soc_Aud_InterConnectionInput_I06, Soc_Aud_InterConnectionOutput_O14);
        
        SetConnection(Soc_Aud_InterCon_Connection, Soc_Aud_InterConnectionInput_I11, Soc_Aud_InterConnectionOutput_O24);
    ...
    {
        SetConnection(Soc_Aud_InterCon_DisConnect, Soc_Aud_InterConnectionInput_I11, Soc_Aud_InterConnectionOutput_O24);
        SetConnection(Soc_Aud_InterCon_DisConnect, Soc_Aud_InterConnectionInput_I06, Soc_Aud_InterConnectionOutput_O14);
...
static const struct snd_kcontrol_new Audio_snd_I2S0dl1_controls[] =
{
    ...
    SOC_ENUM_EXT("Audio_Speaker_Switch", Audio_I2S0dl1_Enum[3], Audio_Speaker_Switch_get, Audio_Speaker_Switch_set),
    SOC_ENUM_EXT("Audio_Receiver_Switch", Audio_I2S0dl1_Enum[4], Audio_Receiver_Switch_get, Audio_Receiver_Switch_set),
};
                                     ...
                                     struct snd_pcm_hw_params *hw_params)
{
    ...
    printk("1 dma_bytes = %zu dma_area = %p dma_addr = 0x%x\n",
...
static int mtk_pcm_I2S0dl1_prepare(struct snd_pcm_substream *substream)
{
    ...
    if (mPrepareDone == false)
    {
        ...
        if (GetMemoryPathEnable(Soc_Aud_Digital_Block_I2S_OUT_DAC) == false)
        {
            ...
            if(mI2S0dl1_hdoutput_control == true)
            {
                SetI2SDacOutlowJitterMode(substream->runtime->rate);
            }
            else
            {
