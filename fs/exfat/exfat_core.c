...
s32 exfat_alloc_cluster(struct super_block *sb, s32 num_alloc, CHAIN_T *p_chain)
{
	...
	if (hint_clu == CLUSTER_32(~0)) {
		...
		hint_clu = 2;
		p_chain->flags = 0x01;
	}else if (!(hint_clu >= 2 && hint_clu < p_fs->num_clusters)) {
		printk(KERN_WARNING "[EXFAT] Invalid cluster chain:%u (num_clusters:%u)\n", hint_clu, p_fs->num_clusters);
		return -1;
	}
