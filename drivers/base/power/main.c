...
enum {
	MT_SUSPEND_LOG_DEBUG	= BIT(0),
	MT_RESUME_LOG_DEBUG	= BIT(1), 
};

static int mt_suspend_resume_debug_mask = 0;
module_param_named(
	sr_debug_mask,mt_suspend_resume_debug_mask,int,S_IRUGO | S_IWUSR | S_IWGRP
);
...
#ifdef LOG
	if(mt_suspend_resume_debug_mask & MT_RESUME_LOG_DEBUG) {
...
#ifdef LOG
	if(mt_suspend_resume_debug_mask & MT_SUSPEND_LOG_DEBUG) {
