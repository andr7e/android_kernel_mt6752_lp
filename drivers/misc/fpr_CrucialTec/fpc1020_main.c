...
#ifdef GPIO_POWER
static int fpc_power_control(int power_state);
#endif
...
int fp_sensor = 0;
fpc1020_data_t *fpc1020_firmware;
char firmware_fpc[30] = {0};
int pos;
bool match_firmware;
		...
		const char *buf, size_t count)
{
	...
	{
		case '0':
			printk("[fp][CT]Fingerprint disable 1V8 & 3V3 power\n");
			fpc_power_control(FPC1020_POWER_STATE_OFF);
			break;
		case '1':
			printk("[fp][CT]Fingerprint enable 1V8 & 3V3 power\n");
			fpc_power_control(FPC1020_POWER_STATE_ON);
			break;
		default:
			printk("[fp][CT] not support item\n");
			break;
	}
	return 0;
}
...
{
	int error;
	printk("[fp][CT]Show finegrprint firmware version\n");
	if(fpc1020_firmware != NULL)
	{
		match_firmware = false;
		pos = 0;
		pos += snprintf(firmware_fpc+pos, sizeof(firmware_fpc), "[FP]FW Ver:[ ");
		error = fpc1020_check_hw_id(fpc1020_firmware);
		pos += snprintf(firmware_fpc+pos, sizeof(firmware_fpc), "]");
		if(match_firmware)
			pos += snprintf(firmware_fpc+pos, sizeof(firmware_fpc), "[FPC][PASS]");
		else
			pos += snprintf(firmware_fpc+pos, sizeof(firmware_fpc), "[FPC][FAIL]");
	}
	else
	{
		pos += snprintf(firmware_fpc+pos, sizeof(firmware_fpc), "[FP]Driver probe fail[FPC][FAIL]");
		printk("[fp][CT]fpc1020_firmware == NULL\n");
	}

	pos = snprintf(buf, pos + 2, "%s", firmware_fpc);
...
{
	int error;
	printk("[fp][CT]Show finegrprint vendor\n");
	pos = 0;
	switch(fp_sensor)
	{
		case 0: 
			printk("[fp][CT] Fingerprint sensor non exist\n");
			pos += snprintf(firmware_fpc+pos, sizeof(firmware_fpc), "0");
			break;
		case 1: 
			printk("[fp][CT] Fingerprint sensor is Validity\n");
			pos += snprintf(firmware_fpc+pos, sizeof(firmware_fpc), "1");
			break;
		case 2: 
			printk("[fp][CT] Fingerprint sensor is Synaptics\n");
			pos += snprintf(firmware_fpc+pos, sizeof(firmware_fpc), "2");
			break;
		case 3: 
			printk("[fp][CT] Fingerprint sensor is Fingerprint Card\n");
			pos += snprintf(firmware_fpc+pos, sizeof(firmware_fpc), "3");
			break;
	}
...
static DEVICE_ATTR(fp_mount, S_IRUGO | S_IWUSR , vendor_show, NULL);
...
static int fpc1020_probe(struct spi_device *spi)
{
...
#ifdef GPIO_POWER
    error = fpc_power_control(FPC1020_POWER_STATE_ON);
#else
	...
	error = fpc1020_input_enable(fpc1020, true);
	if (error)
		goto err_cdev; 
	...
	fpc1020->early_suspend.level = EARLY_SUSPEND_LEVEL_STOP_DRAWING - 1;
	fpc1020->early_suspend.suspend = early_suspend;
	fpc1020->early_suspend.resume = late_resume;
	register_early_suspend(&fpc1020->early_suspend);
	fpc1020->early_suspend_registered = 1;
#endif

	up(&fpc1020->mutex);
	fpc1020_firmware = fpc1020;
...
err:
#ifdef CONFIG_HAS_EARLYSUSPEND
	if(fpc1020->early_suspend_registered == 1)
...
static void early_suspend(struct early_suspend *h)
{
	...
	fpc1020 = container_of(h, fpc1020_data_t, early_suspend);

	dev_dbg(&fpc1020->spi->dev, "%s\n", __func__);

	if (fpc1020->input.enabled && fpc1020->worker.req_mode != FPC1020_WORKER_INPUT_MODE)
		fpc1020_start_input(fpc1020);
...
static void late_resume(struct early_suspend *h)
{
	...
	if (fpc1020->worker.req_mode == FPC1020_WORKER_INPUT_MODE) {
		fpc1020_worker_goto_idle(fpc1020);
		fpc1020_sleep(fpc1020, true);
	}
	return;
					...
					struct fpc1020_platform_data *pdata)
{
	...
	mt_set_gpio_dir(fpc1020->irq_gpio, GPIO_DIR_IN);
	mt_set_gpio_mode(fpc1020->irq_gpio, 0);
	mt_set_gpio_pull_enable(fpc1020->irq_gpio, TRUE);
	mt_set_gpio_pull_select(fpc1020->irq_gpio, GPIO_PULL_DOWN);
...
#endif
LOG_FUNCTION_RETURN_WITH_RET(error);
...
#ifdef GPIO_POWER
	struct device_node *node = of_find_compatible_node(NULL, NULL, "mediatek, FP-eint");;
