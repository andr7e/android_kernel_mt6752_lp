...
static int ov13850_chip_ver_sjc = OV13850_R1A; 

#define USE_OTP_FUNCTION
#ifdef USE_OTP_FUNCTION	
#include "ov13850_otp.h"
extern void otp_cali(unsigned char writeid);
extern int read_module_info(struct otp_struct *otp_ptr);
#endif 
...
{{ 4208, 3120, 0, 0, 4208, 3120, 2104, 1560, 0, 0, 2104, 1560, 0, 0, 2096, 1552}, 
 { 4208, 3120, 0, 0, 4208, 3120, 4208, 3120, 0, 0, 4208, 3120, 0, 0, 4192, 3104}, 
  { 4208, 3120, 0, 0, 4208, 3120, 4208, 3120, 0, 0, 4208, 3120, 0, 0, 4192, 3104}, 
   { 4256, 3152, 840, 850, 2576, 1448, 1288, 724, 4, 2, 1280, 720, 0, 0, 1280, 720}, 
   { 4256, 3152, 576, 704, 3104, 1744, 776, 436, 4, 2, 768, 432, 0, 0, 768, 432}, 
...
extern CAMERA_DUAL_CAMERA_SENSOR_ENUM g_invokeSocketIdx[] ;
static kal_uint32 return_sensor_id()
{

    kal_uint32 cmos_id = ((read_cmos_sensor(0x300A) << 8) | read_cmos_sensor(0x300B));
    LOG_INF("xxxxmain driver  g_invokeSocketIdx[0] %d \n",g_invokeSocketIdx[0]);
    if(g_invokeSocketIdx[0] == DUAL_CAMERA_MAIN_SENSOR || g_invokeSocketIdx[1] == DUAL_CAMERA_MAIN_SENSOR){
    LOG_INF("kkkkkkkkkmain driver  g_invokeSocketIdx[0] %d \n",g_invokeSocketIdx[0]);
        return cmos_id ;
    }else{
        return 0x0000;
    }
...
static char ov13850main_otp_data[OV13850_LITEON_OTP_SIZE];
...
static kal_uint16 gain2reg(const kal_uint16 gain)
{
	...
	if (ov13850_chip_ver_sjc == OV13850_R1A)
	{
	...
	else if(ov13850_chip_ver_sjc == OV13850_R2A)
	{
...
static int capture_first_flag_sjc = 0;
static int pre_currefps_sjc = 0;
static void capture_setting(kal_uint16 currefps)
{
    LOG_INF("E! currefps:%d\n",currefps);
	
if (pre_currefps_sjc != currefps)
{
	capture_first_flag_sjc = 0;
	pre_currefps_sjc = currefps;
}
else
{
	capture_first_flag_sjc = 1;
}
if (capture_first_flag_sjc == 0)
{
    ...
    if (currefps == 240) { 
			...
			write_cmos_sensor(0x380c, ((imgsensor_info.cap1.linelength >> 8) & 0xFF)); 
      		write_cmos_sensor(0x380d, (imgsensor_info.cap1.linelength & 0xFF));        
      		write_cmos_sensor(0x380e, ((imgsensor_info.cap1.framelength >> 8) & 0xFF));  
      		write_cmos_sensor(0x380f, (imgsensor_info.cap1.framelength & 0xFF));         
			write_cmos_sensor(0x3811, 0x04);
...
static void hs_video_setting()
{
    ...
    write_cmos_sensor(0x4021,0xe4); 
    write_cmos_sensor(0x4022,0x04); 
    write_cmos_sensor(0x4023,0x1f); 
    write_cmos_sensor(0x4024,0x05); 
    write_cmos_sensor(0x4025,0x04); 
    write_cmos_sensor(0x4026,0x05); 
    write_cmos_sensor(0x4027,0x07); 
...
static void slim_video_setting()
{
	...
	write_cmos_sensor(0x4020, 0x00);
	write_cmos_sensor(0x4021, 0xe3);
	write_cmos_sensor(0x4022, 0x02);
	write_cmos_sensor(0x4023, 0x1f);
	write_cmos_sensor(0x4024, 0x03);
	write_cmos_sensor(0x4025, 0x04);
	write_cmos_sensor(0x4026, 0x03);
	write_cmos_sensor(0x4027, 0x07);
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	kal_uint8 module_id = 0x0;
	LOG_INF("get the module_id from OTP\n");
	sensor_init();
	struct otp_struct *otp_ptr1 = (struct otp_struct *)kzalloc(sizeof(struct otp_struct), GFP_KERNEL);
	module_id=read_module_info(otp_ptr1);
	if(module_id == 0x10)
	{
		LOG_INF("is sjc module\n");
	}
	else
	{
		LOG_INF("is not sjc module\n");
...
static kal_uint32 open(void)
{
    ...
    if (imgsensor_info.sensor_id != sensor_id)
		...
		{
				LOG_INF("----R1A---- \n");
				ov13850_chip_ver_sjc = OV13850_R1A;
		}else if((read_cmos_sensor(0x302a))==0xb2)
		{
				LOG_INF("----R2A---- \n");
				ov13850_chip_ver_sjc = OV13850_R2A;
		}
...
static const char *ov13850NAME = "ov13850_main";
		...
		struct device_attribute *attr, char *buf)
{
    ...
    LOG_INF("OTP AF Macro position code (MSByte) = 0x%x\n",    ov13850main_otp_data[0xD]);
    LOG_INF("OTP AF Macro position code (LSByte) = 0x%x\n",    ov13850main_otp_data[0xC]);
    unsigned short inf_val = (ov13850main_otp_data[0xB]<<8 | ov13850main_otp_data[0xA]);
    unsigned short mac_val = (ov13850main_otp_data[0xD]<<8 | ov13850main_otp_data[0xC]);
...
static int ov13850_sysfs_init(void)
{
    ...
    if(android_ov13850 == NULL){
        ...
        if (ret) {
            ...
            android_ov13850 = NULL;
        }

        ret = sysfs_create_file(android_ov13850, &dev_attr_otp.attr);
        if (ret) {
            LOG_INF("sysfs_create_file " \
                    "failed\n");
            kobject_del(android_ov13850);
