...
kal_uint32 imx135_r_ratio;
kal_uint32 imx135_b_ratio;
...
static bool wb_gain_set()
{
    ...
    if(!imx135_r_ratio || !imx135_b_ratio)
    {
        LOG_INF("OTP WB ratio Data Err!\n");
        return 0;
    }

    if(imx135_r_ratio >= 512 )
    {
        if(imx135_b_ratio>=512)
        {
            R_GAIN = (USHORT)(GAIN_DEFAULT * imx135_r_ratio / 512);
            G_GAIN = GAIN_DEFAULT;
            B_GAIN = (USHORT)(GAIN_DEFAULT * imx135_b_ratio / 512);
        }
        else
        {
            R_GAIN =  (USHORT)(GAIN_DEFAULT*imx135_r_ratio / imx135_b_ratio );
            G_GAIN = (USHORT)(GAIN_DEFAULT*512 / imx135_b_ratio );
            B_GAIN = GAIN_DEFAULT;
        }
    }
    else
    {
        ...
        {
            R_GAIN = GAIN_DEFAULT;
            G_GAIN = (USHORT)(GAIN_DEFAULT*512 /imx135_r_ratio);
            B_GAIN =  (USHORT)(GAIN_DEFAULT*imx135_b_ratio / imx135_r_ratio );
        }
        else
        {
            Gr_GAIN = (USHORT)(GAIN_DEFAULT*512/ imx135_r_ratio );
            Gb_GAIN = (USHORT)(GAIN_DEFAULT*512/imx135_b_ratio );
            if(Gr_GAIN >= Gb_GAIN)
            {
                R_GAIN = GAIN_DEFAULT;
                G_GAIN = (USHORT)(GAIN_DEFAULT *512/ imx135_r_ratio );
    ...
    LOG_INF("OTP_imx135_r_ratio=%d,imx135_b_ratio=%d \n",imx135_r_ratio,imx135_b_ratio);
...
static bool otp_wb_update(BYTE zone)
{
    ...
    imx135_r_ratio = 512 * golden_r * current_g /( golden_g * current_r );
    imx135_b_ratio = 512 * golden_b * current_g /( golden_g * current_b );
...
bool imx135_otp_update(BYTE update_sensor_otp_awb, BYTE update_sensor_otp_lsc)
{
    BYTE zone = 0x01;
    BYTE FLG  = 0x00;
    BYTE MID = 0x00;
