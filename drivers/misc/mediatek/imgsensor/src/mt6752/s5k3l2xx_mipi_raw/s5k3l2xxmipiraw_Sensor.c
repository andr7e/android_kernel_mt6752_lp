...
#define PFX "S5K3L2XX_camera_sensor"
#define LOG_1 LOG_INF("S5K3L2XX,MIPI 4LANE\n")
...
static imgsensor_info_struct imgsensor_info = {
    ...
    .slim_video = {
        ...
        .startx = 0,
        .starty = 0,
        .grabwindow_width = 832,
				.grabwindow_height = 468,
        .mipi_data_lp2hs_settle_dc = 85,
        .max_framerate = 960, 
    },
    .margin = 16,            
...
{{ 4192, 3104, 0000, 0000, 4192, 3104, 2096, 1552, 0000, 0000, 2096, 1552, 000, 0000, 2096, 1552}, 
 { 4192, 3104, 0000, 0000, 4192, 3104, 4192, 3104, 0000, 0000, 4192, 3104, 000, 0000, 4192, 3104}, 
 { 4192, 3104, 0000, 0000, 4192, 3104, 2096, 1552, 0000, 0000, 2096, 1552, 000, 0000, 2096, 1552}, 
 { 4208, 3120,  186,  480, 3840, 2159, 1280,  720, 0000, 0000, 1280,  720, 000, 0000, 1280,  720}, 
 { 4208, 3120,  440,  624, 832*4, 468*4, 832, 468, 0000, 0000, 832, 468, 000, 0000, 832, 468}
};
...
static int new_module_flag = 0;
static char temp_str[26];
void S5K3L2XX_MIPI_get_calc_wb_data_from_otp(void);
...
static struct S5K3L2XX_MIPI_otp_struct current_otp = {0};

#define GAIN_DEFAULT       0x0100
#define S5K3L2XXOTP_WRITE_ID         0xB0

inline kal_uint16 read_cmos_sensor1(kal_uint32 addr)
{
	kal_uint16 get_byte = 0;
...
void S5K3L2XX_MIPI_read_otp_wb(struct S5K3L2XX_MIPI_otp_struct *otp)
{
   	...
   	FLG = read_cmos_sensor1(0x0000); 
   LOG_INF("[S5K3L2XX] [S5K3L2XX_MIPI_read_otp_wb]normal 0x00=0x%x\n", FLG);
  	...
  	if(FLG==0)
   	...
   	else
   	{
		...
		golden_Gr = 0xA1;
 		golden_Gb = 0xA1;
		golden_B = 0x6D;
  		LOG_INF("[S5K3L2XX] [S5K3L2XX_MIPI_read_otp_wb] golden_R=0x%x, golden_Gr=0x%x, golden_Gb=0x%x, golden_B=0x%x\n", golden_R, golden_Gr, golden_Gb, golden_B);

		if(1 == new_module_flag)
		{
			current_R = read_cmos_sensor1(0x0009);
	  		current_Gr = read_cmos_sensor1(0x000a);
	  		current_Gb = read_cmos_sensor1(0x000b);
	  		current_B = read_cmos_sensor1(0x000c); 
		}else{
			current_R = read_cmos_sensor1(0x0012);
	  		current_Gr = read_cmos_sensor1(0x0013);
	  		current_Gb = read_cmos_sensor1(0x0014);
	  		current_B = read_cmos_sensor1(0x0015); 
		}
  		LOG_INF("[S5K3L2XX] [S5K3L2XX_MIPI_read_otp_wb] current_R=0x%x, current_Gr=0x%x, current_Gb=0x%x, current_B=0x%x\n", current_R, current_Gr, current_Gb, current_B);
   		...
   		printk("[S5K3L2XX] [S5K3L2XX_MIPI_read_otp_wb] r_ratio=0x%x, b_ratio=0x%x\n", otp->r_ratio, otp->b_ratio);      
   	}
}
...
void S5K3L2XX_MIPI_algorithm_otp_wb1(struct S5K3L2XX_MIPI_otp_struct *otp)
{
   ...
   printk("[S5K3L2XX] [S5K3L2XX_MIPI_algorithm_otp_wb1] R_gain=0x%x, B_gain=0x%x, G_gain=0x%x\n", otp->R_Gain, otp->B_Gain, otp->G_Gain);    
}
...
void S5K3L2XX_MIPI_write_otp_wb(struct S5K3L2XX_MIPI_otp_struct *otp)
{
   ...
   printk("[S5K3L2XX][S5K3L2XX_MIPI_write_otp_wb] R_gain=0x%x, B_gain=0x%x, G_gain=0x%x\n", otp->R_Gain, otp->B_Gain, otp->G_Gain);
   if(!otp->R_Gain || !otp->B_Gain || !otp->G_Gain){
   	printk("[S5K3L2XX][S5K3L2XX_MIPI_write_otp_wb] warning: found gain zero, re-read cal data...\n");
	S5K3L2XX_MIPI_get_calc_wb_data_from_otp();
   	printk("[S5K3L2XX][S5K3L2XX_MIPI_write_otp_wb] new read R_gain=0x%x, B_gain=0x%x, G_gain=0x%x\n", otp->R_Gain, otp->B_Gain, otp->G_Gain);
   }
   R_GAIN = otp->R_Gain;
   B_GAIN = otp->B_Gain;
   G_GAIN = otp->G_Gain;
...
void S5K3L2XX_MIPI_get_calc_wb_data_from_otp(void)
{
	int otp_module_flag,otp_module_year,otp_module_month,otp_module_day;
	otp_module_flag = read_cmos_sensor1(0x0007);

	memset(temp_str,0,sizeof(temp_str));
	if(0x1 == otp_module_flag)
	{
		new_module_flag = 1;
		otp_module_year = read_cmos_sensor1(0x0015);
		otp_module_month = read_cmos_sensor1(0x0016);
		otp_module_day = read_cmos_sensor1(0x0017);
		LOG_INF("%s,otp_module_flag = %x,date:%d-%d-%d\n",__func__,otp_module_flag,otp_module_year+2000,otp_module_month,otp_module_day);
		sprintf(temp_str,"NewModule,%d-%d-%d",otp_module_year+2000,otp_module_month,otp_module_day);
	}else{
		new_module_flag = 0;
		otp_module_year = read_cmos_sensor1(0x0008);
		otp_module_month = read_cmos_sensor1(0x0009);
		otp_module_day = read_cmos_sensor1(0x000a);
		LOG_INF("%s,otp_module_flag = %x,date:%d-%d-%d\n",__func__,otp_module_flag,otp_module_year+2000,otp_module_month,otp_module_day);
		sprintf(temp_str,"OldModule,%d-%d-%d ",otp_module_year+2000,otp_module_month,otp_module_day);
	}
...
static void capture_setting_PIP(kal_uint16 currefps)
{	
	write_cmos_sensor(0x0100, 0x00  );
	mDELAY(10);                        
  check_output_stream_off();                        
	...
	write_cmos_sensor(0x380E, 0x01  );
	write_cmos_sensor(0x3027, 0x10  );
	write_cmos_sensor1(0x0342, 0x16c0);
	...
	write_cmos_sensor1(0x303E, 0x00A0);
	write_cmos_sensor(0x30DE, 0x01  );
	write_cmos_sensor1(0x0340, 0x0C60);
	write_cmos_sensor(0x0100, 0x01  );
...
{	
	...
	write_cmos_sensor(0x303A,0x02BC);
	write_cmos_sensor(0x0B05,0x01);   
	write_cmos_sensor(0x0B08,0x01);   
	write_cmos_sensor(0x380E,0x01);   
	write_cmos_sensor(0x3027,0x10);   
	write_cmos_sensor1(0x0342,0x1190);   
	write_cmos_sensor1(0x0340,0x0640);   
	write_cmos_sensor1(0x034C,0x0500);   
	write_cmos_sensor1(0x034E,0x02D0);   
	write_cmos_sensor(0x0114,0x03);   
...
static void slim_video_setting()
{
	...
	write_cmos_sensor1(0x6F12,0x0006);                                      
	write_cmos_sensor1(0x6F12,0x00A2);                                      
	...
	write_cmos_sensor1(0x6F12,0x11A0);                           
	write_cmos_sensor1(0x602A,0x0340);                           
	write_cmos_sensor1(0x6F12,0x03E5);                           
	write_cmos_sensor1(0x602A,0x034C);                           
	write_cmos_sensor1(0x6F12,0x0340);                           
	write_cmos_sensor1(0x6F12,0x01D4);                           
	write_cmos_sensor1(0x602A,0x0114);                           
	write_cmos_sensor(0x6F12,0x03);                             
	write_cmos_sensor1(0x602A,0x0344);                            
	write_cmos_sensor1(0x6F12,0x01B8);                            
	write_cmos_sensor1(0x6F12,0x0270);                            
	write_cmos_sensor1(0x6F12,0x0EB7);                            
	write_cmos_sensor1(0x6F12,0x09BF);                            
...
static const char *s5k3l2Vendor = "Samsung";
static const char *s5k3l2NAME = "s5k3l2";
static const char *s5k3l2Size = "13.0M";
		...
		struct device_attribute *attr, char *buf)
{
	...
	sprintf(buf, "%s %s %s\n", s5k3l2Vendor, s5k3l2NAME, s5k3l2Size);
	ret = strlen(buf) + 1;
...
static struct kobject *android_s5k3l2;

static int s5k3l2_sysfs_init(void)
{
	int ret ;
	LOG_INF("kobject creat and add\n");
    if(android_s5k3l2 == NULL){
        android_s5k3l2 = kobject_create_and_add("android_camera", NULL);
        if (android_s5k3l2 == NULL) {
        ...
        ret = sysfs_create_file(android_s5k3l2, &dev_attr_sensor.attr);
        if (ret) {
            LOG_INF("sysfs_create_file " \
                    "failed\n");
            kobject_del(android_s5k3l2);
            android_s5k3l2 = NULL;
        }

        ret = sysfs_create_file(android_s5k3l2, &dev_attr_otp.attr);
        if (ret) {
            LOG_INF("sysfs_create_file " \
                    "failed\n");
            kobject_del(android_s5k3l2);
                          ...
                          MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    ...
    if(imgsensor.current_fps == 240)
    	capture_setting_PIP(imgsensor.current_fps);
    else
                             ...
                             UINT8 *feature_para,UINT32 *feature_para_len)
{
    ...
    unsigned long long *feature_data_64 = (unsigned long long *) feature_para;
    ...
    switch (feature_id) {
        ...
        case SENSOR_FEATURE_GET_DEFAULT_FRAME_RATE_BY_SCENARIO:
            get_default_framerate_by_scenario((MSDK_SCENARIO_ID_ENUM)*feature_data_32, (MUINT32 *)(*(feature_data_64+1)));
            break;
        ...
        case SENSOR_FEATURE_GET_CROP_INFO:
            ...
            wininfo = (SENSOR_WINSIZE_INFO_STRUCT *)(*(feature_data_64+1));
