...
u32 i = 0;
    ...
    if (g_bEnableDriver[i]) {
        ...
        if (ERROR_NONE != ret) {
        PK_ERR("[%s] power on failed", __func__);
        return ret;
        }
    }
    }
...
int kdSetDriver(unsigned int *pDrvIndex)
{
    ...
    PK_ERR("%s xxxxxxxxxxxxxxxxxxxxxxxx",__func__);
...
#endif
    PK_ERR("[kdSetDriver]g_invokeSocketIdx[%d] = %d\n", i, g_invokeSocketIdx[i]);
    PK_ERR("[kdSetDriver]drvIdx[%d] = %d\n", i, drvIdx[i]);
...
static int __init CAMERA_HW_i2C_init(void)
{
    ...
    if(SUPPORT_I2C_BUS_NUM1 != SUPPORT_I2C_BUS_NUM2)  
    {
