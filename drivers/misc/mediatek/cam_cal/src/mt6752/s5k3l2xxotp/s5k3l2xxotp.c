...
volatile u64 I2CDMABuf_pa = NULL;
...
static long s5k3l2xx_Ioctl_Compat(struct file *filp, unsigned int cmd, unsigned long arg)
{
    long ret;
    COMPAT_stCAM_CAL_INFO_STRUCT __user *data32;
    stCAM_CAL_INFO_STRUCT __user *data;
    int err;
	  CAM_CALDB("[CAMERA SENSOR] s5k3l2xx_Ioctl_Compat,%p %p %x ioc size %d\n",filp->f_op ,filp->f_op->unlocked_ioctl,cmd,_IOC_SIZE(cmd) );
...
#endif
{
    ...
    switch(a_u4Command)
    {
        ...
        case CAM_CALIOC_G_READ:
            ...
            CAM_CALDB("[S5K3L2XX_CAM_CALL] Before read Working buffer address 0x%p \n", pWorkingBuff);
...
static int CAM_CAL_Open(struct inode * a_pstInode, struct file * a_pstFile)
{
    ...
    if(TRUE != hwPowerOn(MT6325_POWER_LDO_VCAMA, VOL_2800, CAM_CAL_DRVNAME))
    {
...
static int CAM_CAL_Release(struct inode * a_pstInode, struct file * a_pstFile)
{
    ...
    spin_unlock(&g_CAM_CALLock);
    if(TRUE != hwPowerDown(MT6325_POWER_LDO_VCAMA, CAM_CAL_DRVNAME))
    {
        CAM_CALDB("[S24CAM_CAL] Fail to disable analog gain\n");
        return -EIO;
    }
    return 0;
}
...
#ifdef CONFIG_COMPAT
    .compat_ioctl = s5k3l2xx_Ioctl_Compat,
#endif	
