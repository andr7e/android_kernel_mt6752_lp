    ...
    for(layid = 0; layid < (HW_OVERLAY_COUNT + 1); layid++)
    {
        ...
        if(fence_idx >= 0) 
		    ...
		    if(data_config && (layid >= HW_OVERLAY_COUNT))
		    {
                ...
                DISPMSG("mem_seq %x-seq+ %d\n", pgc->session, mtkfb_query_frm_seq_by_addr(pgc->session, layid, wdma_layer.dstAddress ));
                dprec_logger_frame_seq_end(pgc->session, mtkfb_query_frm_seq_by_addr(pgc->session, layid, wdma_layer.dstAddress ));
