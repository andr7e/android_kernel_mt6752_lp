...
disp_lcm_handle* disp_lcm_probe(char* plcm_name, LCM_INTERFACE_ID lcm_id)
{
	...
	snprintf(disp_name,64,"%s",lcm_drv->name);
	display_sysfs_init();
...
int disp_lcm_set_cmd(disp_lcm_handle *plcm,void * cmdq_handle, unsigned int *lcm_cmd,unsigned char *lcm_count,unsigned char *lcm_value)
{
	...
	if(_is_lcm_inited(plcm))
	{
		...
		{
		    printk("disp_lcm_set_cmd lcm_cmd =%x,lcm_count =%d,lcm_value =%x\n",*lcm_cmd,*lcm_count,*lcm_value);
