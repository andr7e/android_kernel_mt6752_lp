        ...
        if(primary_display_is_video_mode()==0)
        {
            ...
            if(1==enter)  
            {
                if(isIdlePowerOff==0)
                {
                    ...
                    spin_lock_irqsave(&gLockTopClockOff, flags);
                    _disp_primary_path_idle_clock_off(0);
                    isIdlePowerOff = 1;
                    spin_unlock_irqrestore(&gLockTopClockOff, flags);
            ...
            else
            {
                if(isIdlePowerOff==1)
                {
                    ...
                    spin_lock_irqsave(&gLockTopClockOff, flags);
                    isIdlePowerOff = 0;
                    _disp_primary_path_idle_clock_on(0);
                    spin_unlock_irqrestore(&gLockTopClockOff, flags);
...
int _trigger_display_interface(int blocking, void *callback, unsigned int userdata)
{
	...
	if(gPrefetchControl==1 && cnt>=20 && primary_display_is_video_mode())
	{
	    cmdqRecDisablePrefetch(pgc->cmdq_handle_config);
	}
#endif   
...
static int _config_ovl_input(disp_session_input_config *session_input,
				    ...
				    cmdqRecHandle cmdq_handle )
{
	int ret=0, i=0, layer=0, dst_id=0;
	disp_ddp_path_config *data_config = NULL;	
...
int primary_display_setbacklight(unsigned int level)
{
	...
	{
		disp_update_trigger_time();
#ifdef CONFIG_CUSTOM_KERNEL_LCM_BLS_PWM
		DISPCHECK("set backlight by DISP_PWM0 level=%d\n",level*4);
		disp_pwm_set_backlight(DISP_PWM0,level*4);
#else
		if(primary_display_cmdq_enabled())	
		{	
			if(primary_display_is_video_mode())
...
int _set_lcm_cmd_by_cmdq(unsigned int *lcm_cmd,unsigned char *lcm_count,unsigned char *lcm_value)
{
	...
	if(primary_display_is_video_mode())
	{
	    printk("_set_lcm_cmd_by_cmdq lcm_cmd=%x,lcm_count =%d,lcm_value=%x\n",*lcm_cmd,*lcm_count,*lcm_value);
		MMProfileLogEx(ddp_mmp_get_events()->primary_set_cmd, MMProfileFlagPulse, 1, 2);
		cmdqRecReset(cmdq_handle_lcm_cmd);
		disp_lcm_set_cmd(pgc->plcm,cmdq_handle_lcm_cmd, lcm_cmd,lcm_count,lcm_value);
...
int primary_display_setlcm_cmd(unsigned int *lcm_cmd,unsigned char *lcm_count,unsigned char *lcm_value)
{
	DISPFUNC();
	int ret = 0;
