...
void _debug_fps_meter(unsigned long mva, unsigned long va, unsigned int w, unsigned int h, unsigned int linepitch, unsigned int color, unsigned int layerid, unsigned int bufidx)
{
	...
	current_idx = ((mod/1000/16666) > (DEBUG_FPS_METER_SHOW_COUNT-1))?(DEBUG_FPS_METER_SHOW_COUNT-1):(mod/1000/16666);
...
static void process_dbg_opt(const char *opt)
{
	...
	{
		char *p = (char *)opt + 4;
		unsigned int lcm_cmd = 0;
		unsigned char cmd_num = 0;
		unsigned char lcm_value[30];
		int i=0;
		lcm_cmd = (unsigned int) simple_strtoul(p, &p, 16);
		cmd_num = (unsigned char) simple_strtoul(p+1, &p, 16);
		for(i=0;i<cmd_num;i++)
		{
			lcm_value[i]=(unsigned int) simple_strtoul(p+1, &p, 16);
		}
		primary_display_setlcm_cmd(&lcm_cmd,&cmd_num,&lcm_value);
	}
