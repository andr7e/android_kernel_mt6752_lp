...
static disp_session_sync_info *_get_session_sync_info(unsigned int session_id)
{
	...
	for(i=0;i<sizeof(_disp_fence_context)/sizeof(_disp_fence_context[0]);i++)
	{
		if(_disp_fence_context[i].session_id == 0xffffffff)
		{
			...
			snprintf(name,31, "%s%d_prepare", disp_session_mode_spy(session_id), DISP_SESSION_DEV(session_id));
			...
			snprintf(name,31, "%s%d_setinput", disp_session_mode_spy(session_id), DISP_SESSION_DEV(session_id));
			...
			snprintf(name,31, "%s%d_setoutput", disp_session_mode_spy(session_id), DISP_SESSION_DEV(session_id));
			...
			snprintf(name,31, "%s%d_trigger", disp_session_mode_spy(session_id), DISP_SESSION_DEV(session_id));
			...
			snprintf(name,31, "%s%d_findidx", disp_session_mode_spy(session_id), DISP_SESSION_DEV(session_id));
			dprec_logger_event_init(&session_info->event_findidx, name, DPREC_LOGGER_LEVEL_DEFAULT, ddp_mmp_get_events()->session_Parent);
			
			snprintf(name,31, "%s%d_release", disp_session_mode_spy(session_id), DISP_SESSION_DEV(session_id));
			...
			snprintf(name,31, "%s%d_waitvsync", disp_session_mode_spy(session_id), DISP_SESSION_DEV(session_id));
			...
			snprintf(name,31, "%s%d_err", disp_session_mode_spy(session_id), DISP_SESSION_DEV(session_id));
			...
			for(j=0;j<(sizeof(session_info->session_layer_info)/sizeof(session_info->session_layer_info[0]));j++)
			{
				if(DISP_SESSION_TYPE(session_id) == DISP_SESSION_PRIMARY)
				{
					snprintf(name,31, "%s-primary-%d-%d", prefix, DISP_SESSION_DEV(session_id), j);
				}
				else if(DISP_SESSION_TYPE(session_id) == DISP_SESSION_EXTERNAL)
				{
					snprintf(name,31, "%s-external-%d-%d", prefix, DISP_SESSION_DEV(session_id), j);
				}
				else if(DISP_SESSION_TYPE(session_id) == DISP_SESSION_MEMORY)
				{
					snprintf(name,31, "%s-memory-%d-%d", prefix, DISP_SESSION_DEV(session_id), j);
				}
				else
				{
					snprintf(name,31, "%s-unknown-%d-%d", prefix, DISP_SESSION_DEV(session_id), j);
				}
...
unsigned int mtkfb_query_buf_va(unsigned int session_id, unsigned int layer_id, unsigned int idx) 
{
	...
	if(session_info==NULL)
	{
		MTKFB_FENCE_ERR("session_info NULL!\n");
		return 0;
	}
