...
#define DW_KEY_HIGH_THR	 (500) 
#define DW_KEY_THR		 (220) 
#define UP_KEY_THR       (80) 
#define MD_KEY_THR		 (0)
...
{ 
   ...
   mt6325_upmu_set_rg_audmicbiaslowpen(1);
	...
	pmic_pwrap_write(ACCDET_RSV, 0x1290);    
   #ifdef ACCDET_EINT_IRQ
	...
	#if  (ACCDET_MIC_MODE==1) 
	mt6325_upmu_set_rg_audmicbias1dcswpen(0);
	#elif (ACCDET_MIC_MODE==2) 
	pmic_pwrap_write(ACCDET_RSV, pmic_pwrap_read(ACCDET_RSV)|ACCDET_INPUT_MICP);
	#elif  (ACCDET_MIC_MODE==6)  
	pmic_pwrap_write(ACCDET_RSV, pmic_pwrap_read(ACCDET_RSV)|ACCDET_INPUT_MICP);
