...
{
	struct bma250_i2c_data *obj = dev_get_drvdata(dev);
	char *token;
	char *str_buf;
	char *running;
	long input_val[3] = {0};
	...
	if (str_buf == NULL) {
		GSE_ERR("%s: cannot allocate buffer\n", __func__);
		return -1;
	}
	running = str_buf;
	...
	for (i = 0; i < 3; i++) {
		...
		if (token == NULL) {
			GSE_ERR("%s: token = NULL, i = %d\n", __func__, i);
			break;
		}

		rc = kstrtol(token, 10, &input_val[i]);
		if (rc) {
			GSE_ERR("%s: kstrtol fails, rc = %d, i = %d\n",
			  __func__, rc, i);
			kfree(str_buf);
			return rc;
		}
	...
	GSE_LOG("%s: User Calibration(x, y, z) = (%ld, %ld, %ld)\n", __func__,
	  input_val[0], input_val[1], input_val[2]);

	temp_kvalue[0] = (s8)input_val[0];
	temp_kvalue[1] = (s8)input_val[1];
	temp_kvalue[2] = (s8)input_val[2];

	GSE_LOG("%s: temp_kvalue(x, y, z) = (0x%x, 0x%x, 0x%x)\n", __func__,
	  temp_kvalue[0] & 0xFF, temp_kvalue[1] & 0xFF, temp_kvalue[2] & 0xFF);

	for (i = 0; i < 3; i++) {
		user_cali_offset_buf[i]=temp_kvalue[i]*10;
		GSE_LOG("%s: user_cali_offset_buf[%d] = %d\n", __func__, i, user_cali_offset_buf[i]);
	}

	obj->cali_sw[BMA250_AXIS_X] = -obj->cvt.sign[BMA250_AXIS_X]*(user_cali_offset_buf[obj->cvt.map[BMA250_AXIS_X]]);
	obj->cali_sw[BMA250_AXIS_Y] = -obj->cvt.sign[BMA250_AXIS_Y]*(user_cali_offset_buf[obj->cvt.map[BMA250_AXIS_Y]]);
	obj->cali_sw[BMA250_AXIS_Z] = -obj->cvt.sign[BMA250_AXIS_Z]*(user_cali_offset_buf[obj->cvt.map[BMA250_AXIS_Z]]);
...
static int bma250_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	obj->g_sensor_class = class_create(THIS_MODULE, "htc_g_sensor");
		if (IS_ERR(obj->g_sensor_class)) {
			err = PTR_ERR(obj->g_sensor_class);
			obj->g_sensor_class = NULL;
			GSE_ERR("%s: could not allocate data->g_sensor_class\n", __func__);
			goto exit_create_class_failed;
		}

	obj->g_sensor_user_cali_dev = device_create(obj->g_sensor_class,
				NULL, 0, "%s", "g_sensor");
	if (unlikely(IS_ERR(obj->g_sensor_user_cali_dev))) {
		err = PTR_ERR(obj->g_sensor_user_cali_dev);
		obj->g_sensor_user_cali_dev = NULL;
		GSE_ERR("%s: could not allocate data->g_sensor_user_cali_dev\n", __func__);
		goto exit_create_g_sensor_user_cali_device_failed;
	}

	dev_set_drvdata(obj->g_sensor_user_cali_dev, obj);
	err = device_create_file(obj->g_sensor_user_cali_dev, &dev_attr_g_sensor_user_offset);
	if (err) {
		GSE_ERR("%s: create g_sensor_user_offset fail!\n", __func__);
		goto exit_dev_attr_g_sensor_user_offset_failed;
	}
...
exit_dev_attr_g_sensor_user_offset_failed:
	device_unregister(obj->g_sensor_user_cali_dev);
exit_create_g_sensor_user_cali_device_failed:
	class_destroy(obj->g_sensor_class);
exit_create_class_failed:
...
static int bma250_i2c_remove(struct i2c_client *client)
{
	...
	device_remove_file(obj->g_sensor_user_cali_dev, &dev_attr_g_sensor_user_offset);
	device_unregister(obj->g_sensor_user_cali_dev);
	class_destroy(obj->g_sensor_class);
