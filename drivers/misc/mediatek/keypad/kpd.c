					 ...
					 const char *buf, size_t count)
{
	...
	tempmask = simple_strtoull(buf, NULL, 10);
	if (tempmask) {
		if (tempmask == 127)
			bitmask &= tempmask;
		else if (tempmask > 128)
			bitmask &= tempmask;
		else
			bitmask |= tempmask;
	}
	kpd_print("vol_wakeup_store : %x\n", bitmask);	
	return count;
}
...
static int kpd_pdrv_probe(struct platform_device *pdev)
{
...
#ifdef CONFIG_SENSE_VOL_WAKEUP
	err = sysfs_create_group(&pdev->dev.kobj, &gpio_keys_attr_group);
	if (err) {
		printk("KEY_ERR: %s: Unable to export keys/switches, error: %d\n", __func__, err);
		
	}

	
	err = sysfs_create_link(NULL, &pdev->dev.kobj, "keyboard");
	if (err) {
		printk("KEY_ERR: %s: subsystem_register failed\n", __func__);
		err = -ENOMEM;
	...
	if(!(mt6325_upmu_get_pwrkey_deb())) {
		kpd_pwrkey_pmic_handler(!(mt6325_upmu_get_pwrkey_deb()));
	}
