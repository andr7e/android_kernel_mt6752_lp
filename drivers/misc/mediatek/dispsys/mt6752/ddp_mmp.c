...
void init_ddp_mmp_events(void)
{
		...
		DDP_MMP_Events.primary_seq_insert = MMProfileRegisterEvent(DDP_MMP_Events.primary_Parent, "seq_insert");
		DDP_MMP_Events.primary_seq_config=  MMProfileRegisterEvent(DDP_MMP_Events.primary_Parent, "seq_config");
		DDP_MMP_Events.primary_seq_trigger=  MMProfileRegisterEvent(DDP_MMP_Events.primary_Parent, "seq_trigger");
		DDP_MMP_Events.primary_seq_rdma_irq=  MMProfileRegisterEvent(DDP_MMP_Events.primary_Parent, "seq_rdma_irq");
		DDP_MMP_Events.primary_seq_release=  MMProfileRegisterEvent(DDP_MMP_Events.primary_Parent, "seq_release");
...
void ddp_mmp_ovl_layer(OVL_CONFIG_STRUCT* pLayer,unsigned int down_sample_x,unsigned int down_sample_y,unsigned int session)
{
...
void ddp_mmp_rdma_layer(RDMA_CONFIG_STRUCT *rdma_layer,unsigned int rdma_num, unsigned int down_sample_x,unsigned int down_sample_y)
{
    if(rdma_num > 1)
    {
    ...
    switch (rdma_layer->inputFormat)
    {
        ...
        case eRGBA8888:
            ...
            break;
        default:
          DDPERR("dump_rdma_layer(), unknow fmt=%d, dump raw\n", rdma_layer->inputFormat);
          raw = 1;
    }
    if(!raw)
    {
        ...
        Bitmap.down_sample_x = down_sample_x;
        Bitmap.down_sample_y = down_sample_y;
        if (m4u_mva_map_kernel(rdma_layer->address, Bitmap.data_size,(unsigned int*)&Bitmap.pData, &Bitmap.data_size)==0)
        {
    ...
    else
    {
        ...
        if(m4u_mva_map_kernel(rdma_layer->address, meta.size, (unsigned int*)&meta.pData, &meta.size)==0)
        {
