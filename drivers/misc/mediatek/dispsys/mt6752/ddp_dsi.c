...
static void DSI_WaitForNotBusy(DISP_MODULE_ENUM module, cmdqRecHandle cmdq)
{
	...
	while(1)
	{
		...
		if((end_time - start_time)/1000 > 2000000)
		{
	...
	for(i = DSI_MODULE_BEGIN(module);i <= DSI_MODULE_END(module);i++)
	{
		...
		if(0!=data_Rate)
		{
			...
			if(1 != dsi_params->ssc_disable)
			{
				...
				MIPITX_OUTREGBIT(MIPITX_DSI_PLL_CON1_REG,DSI_PHY_REG[i]->MIPITX_DSI_PLL_CON1,RG_DSI0_MPPLL_SDM_SSC_PRD,0x1B1);
...
int ddp_dsi_switch_lcm_mode(DISP_MODULE_ENUM module, void* params)
{
	...
	if(mode == 0){
		DSI_OUTREG32(NULL, (unsigned long)(DSI_REG[i])+0x130, 0x00001521|(lcm_cmd.addr << 16)|(lcm_cmd.val[0]<< 24)); 
...
#if 0
		DSI_OUTREG32(NULL,  (unsigned long)(DSI_REG[i])+0x130, 0x00001539|(lcm_cmd.addr << 16)|(lcm_cmd.val[1]<< 24)); 
		DSI_OUTREGBIT(NULL, DSI_START_REG,DSI_REG[i]->DSI_START,VM_CMD_START,0);
		DSI_OUTREGBIT(NULL, DSI_START_REG,DSI_REG[i]->DSI_START,VM_CMD_START,1);
		wait_vm_cmd_done = false;
		wait_event_interruptible(_dsi_wait_vm_cmd_done_queue[i],wait_vm_cmd_done);
...
int ddp_dsi_switch_mode(DISP_MODULE_ENUM module, void *cmdq_handle, void* params)
{
	...
	if(mode == 0){
		...
		DSI_OUTREG32(cmdq_handle,  (unsigned long)(DSI_REG[i])+0x130, 0x00001539|(lcm_cmd.addr << 16)|(lcm_cmd.val[1]<< 24)); 
		...
		DSI_MASKREG32(cmdq_handle, 0xF4020028, 0x1, 0x1); 
		DSI_MASKREG32(cmdq_handle, 0xF4020028, 0x1, 0x0); 
		DSI_MASKREG32(cmdq_handle, 0xF4020030, 0x1, 0x0); 
		...
		DSI_MASKREG32(cmdq_handle, 0xF4020030, 0x1, 0x1); 
		...
		DSI_MASKREG32(NULL, 0xF4020020, 0x1, 0x1); 
		cmdqRecFlush(cmdq_handle);
		cmdqRecReset(cmdq_handle);
		cmdqRecWaitNoClear(cmdq_handle, CMDQ_SYNC_TOKEN_STREAM_EOF);
#else
		...
		DSI_MASKREG32(NULL, 0xF4020030, 0x1, 0x1); 
