...
void rdma_set_ultra(unsigned int idx, unsigned int width, unsigned int height, unsigned int bpp, unsigned int frame_rate,void * handle)
{
  ...
  static const unsigned int blank_overhead = 115; 
  static const unsigned int rdma_fifo_width = 16; 
  static const unsigned int ultra_low_time      = 6; 
  static const unsigned int pre_ultra_low_time  = 8; 
  static const unsigned int pre_ultra_high_time = 9; 
  static const unsigned int fifo_size = 512;
  static const unsigned int fifo_valid_line_ratio = 125; 
                    ...
                    void * handle)
{
    ...
    else
    {
        ...
        DISP_REG_SET_FIELD(handle,FIFO_CON_FLD_FIFO_UNDERFLOW_EN, idx*DISP_RDMA_INDEX_OFFSET+ DISP_REG_RDMA_FIFO_CON, 1);	 
        if(0x200<fifolen)
            DISP_REG_SET_FIELD(handle,FIFO_CON_FLD_OUTPUT_VALID_FIFO_THRESHOLD, idx*DISP_RDMA_INDEX_OFFSET+ DISP_REG_RDMA_FIFO_CON, 0x200); 
        else
            DISP_REG_SET_FIELD(handle,FIFO_CON_FLD_OUTPUT_VALID_FIFO_THRESHOLD, idx*DISP_RDMA_INDEX_OFFSET+ DISP_REG_RDMA_FIFO_CON, fifolen); 
    }  
#endif
    return 0;
}
...
void rdma_dump_analysis(DISP_MODULE_ENUM module)
{
    ...
    if(idx>1 || idx<0)
    {
        DDPDUMP("can not dump DISP_MODULE_RDMA2!\n");
        return;
    }
...
static int do_rdma_config_l(DISP_MODULE_ENUM module, disp_ddp_path_config* pConfig, void* handle)
{
	...
	if(mode == RDMA_MODE_DIRECT_LINK &&
		...
		mode,
			...
			(mode == RDMA_MODE_DIRECT_LINK) ? eRGB888 : r_config->inputFormat,  
