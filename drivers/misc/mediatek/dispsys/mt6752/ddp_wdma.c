...
enum WDMA_OUTPUT_FORMAT {
    WDMA_OUTPUT_FORMAT_BGR565   = 0x00,   
...
void wdma_calc_ultra(unsigned int idx, unsigned int width, unsigned int height, unsigned int bpp, unsigned int frame_rate,void * handle)
{
  ...
  unsigned int blank_overhead = 115; 
  unsigned int rdma_fifo_width = 16; 
  
  unsigned int pre_ultra_low_time ; 
  unsigned int ultra_low_time     ; 
  unsigned int ultra_high_time    ; 
  ...
  if(idx==0 && primary_display_is_decouple_mode()==0)
  {
      ...
      DISP_REG_SET(handle,idx*DISP_WDMA_INDEX_OFFSET+DISP_REG_WDMA_BUF_CON1, 0xD0100100);
  }
  else
  {
      DISP_REG_SET(handle,idx*DISP_WDMA_INDEX_OFFSET+DISP_REG_WDMA_BUF_CON2, 0x35011b44);
      DISP_REG_SET(handle,idx*DISP_WDMA_INDEX_OFFSET+DISP_REG_WDMA_BUF_CON1, 0x50100100);
  }
...
static int wdma_config_l(DISP_MODULE_ENUM module, disp_ddp_path_config* pConfig, void *handle)
{
	...
	if(handle) {
			...
			primary_display_is_decouple_mode()==1)
		{
			...
			DISP_REG_SET(handle, idx_offst+DISP_REG_WDMA_RST, 0x01); 
			cmdqRecPoll(handle, disp_addr_convert(idx_offst+DISP_REG_WDMA_FLOW_CTRL_DBG), 1, 0x1);
			DISP_REG_SET(handle, idx_offst+DISP_REG_WDMA_RST , 0x0); 
		}
	}
