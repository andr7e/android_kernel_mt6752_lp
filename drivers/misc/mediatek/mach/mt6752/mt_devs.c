...
u64 pmem_start = 0x12345678;  
u64 kernel_mem_sz = 0x0;       
...
static struct resource resource_xhci[] = {
	{
		.start		= IO_VIRT_TO_PHYS(USB3_BASE), 
...
#define NFI_base    NFI_BASE
#define NFIECC_base NFIECC_BASE
...
static void parse_dfo_tag(struct tag *t)
{
    ...
    for (i = 0; i < nr; i++) {
        ...
        if (dfo_db.cnt <= MAX_DFO_ENTRIES) {
            ...
            dfo_db.dfo_tag[dfo_db.cnt].info[0].name[31] = '\0'; 
