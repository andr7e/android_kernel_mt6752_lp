...
int dcm_armcore(ENUM_ARMCORE_DCM mode)
{
        ...
        if (mode == ARMCORE_DCM_OFF) {
                ...
                reg_write(INFRACFG_TOP_CKDIV1, 
                          aor(reg_read(INFRACFG_TOP_CKDIV1), ~(0x1f<<0), 0)); 
#endif
                return 0;
        }
...
#if 0
                reg_write(INFRACFG_TOP_CKDIV1, 
                          aor(reg_read(INFRACFG_TOP_CKDIV1), ~(0x1f<<0), 0x01d)); 
#endif
        }
...
#define	CLK_MISC_CFG_0  (0xf0000000 + 0x0104) 
...
void debug_mon(int major, int minor, int fqmtr_div)
{
        ...
        if (major & INFRA_PERI_AXI) {
                reg_write(INFRA_AO_DBG_CON0, 1); 
                reg_write(INFRA_AO_DBG_CON1, 1); 
                reg_write(INFRA_AO_DBG_CON2, 1); 
                reg_write(INFRA_AO_DBG_CON3, 1); 
        ...
        if (major & CKGEN_CK) {
                ...
                reg_write(CLK_DBG_CFG, aor(reg_read(CLK_DBG_CFG), ~0x03, 1)); 
        ...
        if (major & ABIST_CLK) {
                ...
                reg_write(CLK_DBG_CFG, aor(reg_read(CLK_DBG_CFG), ~0x03, 0)); 
