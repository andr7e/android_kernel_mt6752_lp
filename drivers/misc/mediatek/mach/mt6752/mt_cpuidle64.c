...
#define _MCUCFG_BASE (mcucfg_base)      
...
#define _MCUCFG_BASE (0xf0200000)      
...
#define _MCUCFG_BASE (0x10200000)      
 ...
 **/
static unsigned int *copy_words(unsigned int *dst, unsigned int *src, unsigned int num)
{
#if 0
...
int mt_cpu_dormant_reset(unsigned long flags)
{
	...
	disable_dcache_safe(!!IS_DORMANT_INNER_OFF(flags), "\n\t");
        ...
        __flush_dcache_area(core->timestamp, sizeof(core->timestamp));
#else
