...
static struct clkmux muxs[NR_MUXS] = {
    {
        ...
        .name = __stringify(MUX_MSDC50_0_hclk),
        .sel_mask = 0x00000300,
        .pdn_mask = 0x00008000,
        .upd_mask = 1<<12,
        .offset = 8,
...
#else
    clk_writel(mux->base_addr+8, mux->sel_mask); 
    clk_writel(mux->base_addr+4, (clksrc << mux->offset)); 
    clk_writel(CLK_CFG_UPDATE, mux->upd_mask);
...
static void clkmux_enable_op(struct clkmux *mux)
{
    ...
    clk_writel(CLK_CFG_UPDATE, mux->upd_mask);
#else
    clk_writel(mux->base_addr+8, mux->pdn_mask);
    clk_writel(CLK_CFG_UPDATE, mux->upd_mask);
...
static void clkmux_disable_op(struct clkmux *mux)
{
    ...
    clk_writel(mux->base_addr+4, mux->pdn_mask); 
    clk_writel(CLK_CFG_UPDATE, mux->upd_mask);
...
return;
    ...
    clks[id].mux = &muxs[MT_MUX_MM];
    clk_info("clks[MT_CG_DISP0_SMI_COMMON].cnt=%d, clks[MT_CG_DISP0_SMI_LARB0].cnt=%d, muxs[MT_MUX_MM].cnt=%d\n", clks[MT_CG_DISP0_SMI_COMMON].cnt, clks[MT_CG_DISP0_SMI_LARB0].cnt, muxs[MT_MUX_MM].cnt);
    muxs[MT_MUX_MM].cnt++;
    if (id == MT_CG_DISP0_SMI_COMMON)
        muxs[MT_MUX_MM].cnt--;
...
static void cg_bootup_pdn(void)
{
    ...
    clk_writel(INFRA_PDN_SET1, 0x6f0876); 
     
    
    clk_writel(MFG_CG_SET, MFG_CG);

    
    clk_writel(DISP_CG_SET0, 0xff9ffffc); 
...
int mt_clkmgr_init(void)
{
    ...
    mt_enable_clock(MT_CG_MFG_BG3D, "clkmgr");
    mt_disable_clock(MT_CG_MFG_BG3D, "clkmgr");
...
int clk_pm_restore_noirq(struct device *device)
{
    ...
    clks[MT_CG_DISP0_SMI_COMMON].mux = NULL;
    clks[MT_CG_DISP0_SMI_LARB0].mux = NULL;
    es_flag = 0;
