...
enum {
	SLP_GPIO_DUMP_DEBUG = BIT(0),
	SLP_VREG_DUMP_DEBUG = BIT(1),
};

static int slp_debug_mask;

module_param_named(
	debug_mask, slp_debug_mask, int, S_IRUGO | S_IWUSR | S_IWGRP
);
...
static int slp_suspend_ops_enter(suspend_state_t state)
{
    ...
    if (SLP_GPIO_DUMP_DEBUG & slp_debug_mask)
        dump_sleep_gpio();

    if (SLP_VREG_DUMP_DEBUG & slp_debug_mask)
	dump_sleep_vreg();
...
#if CONFIG_HTC_POWER_PROFILING_6752
