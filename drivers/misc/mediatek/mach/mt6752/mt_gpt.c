...
unsigned long long mt_read_sched_clock(void)
{
	...
	return(((unsigned long long)(syscnt_cnt[1])) << 32 | (unsigned long long)(syscnt_cnt[0]));
}

static  void setup_syscnt(u32 freq)
{	
   ...
   sched_clock_register((void *)mt_read_sched_clock, 53, (unsigned long)freq);
