...
#define CPUFREQ_BOUNDARY_FOR_FHCTL   (CPU_DVFS_FREQ7)       
...
#define CPU_DVFS_FREQ0   (1994000) 
#define CPU_DVFS_FREQ1   (1794000) 
#define CPU_DVFS_FREQ2   (1690000) 
#define CPU_DVFS_FREQ3   (1612000) 
#define CPU_DVFS_FREQ4   (1560000) 
#define CPU_DVFS_FREQ4_2 (1495000) 
#define CPU_DVFS_FREQ5   (1417000) 
#define CPU_DVFS_FREQ6   (1287000) 
#define CPU_DVFS_FREQ7   (1170000) 
#define CPU_DVFS_FREQ8   (936000)  
#define CPU_DVFS_FREQ9   (702000)  
#define CPU_DVFS_FREQ10  (468000)  
...
#define PMIC_ADDR_VPROC_VOSEL_ON     0x04BA  
#define PMIC_ADDR_VPROC_VOSEL        0x04B8  
#define PMIC_ADDR_VPROC_VOSEL_CTRL   0x04B0  
#define PMIC_ADDR_VPROC_EN           0x04B4  
#define PMIC_ADDR_VSRAM_VOSEL_ON     0x0506  
#define PMIC_ADDR_VSRAM_VOSEL        0x0A58  
#define PMIC_ADDR_VSRAM_VOSEL_CTRL   0x04FC  
#define PMIC_ADDR_VSRAM_EN           0x0A34  
#define PMIC_ADDR_VSRAM_FAST_TRSN_EN 0x0A62  
#define PMIC_ADDR_VGPU_VOSEL_ON      0x0618  
#define PMIC_ADDR_VCORE_VOSEL_ON     0x0664  
#define PMIC_ADDR_VLTE_VOSEL_ON      0x063E  
...
void mt_vcore_dvfs_volt_set_by_sdio(unsigned int volt) 
{
    ...
    target_pmic_val_vcore_ao = VOLT_TO_PMIC_VAL(volt / 10); 
...
#define PLL_MIN_FREQ		(130000)	
#define PLL_DIV1_FREQ		(1001000)	
#define PLL_DIV2_FREQ		(520000)	
#define PLL_DIV4_FREQ		(260000)	
#define PLL_DIV8_FREQ		(PLL_MIN_FREQ)	

#define DDS_DIV1_FREQ		(0x0009A000)	
#define DDS_DIV2_FREQ		(0x010A0000)	
#define DDS_DIV4_FREQ		(0x020A0000)	
#define DDS_DIV8_FREQ		(0x030A0000)	
...
static unsigned int _cpu_freq_calc(unsigned int con1, unsigned int ckdiv1)
{
    ...
    freq = (vco_i + vco_f) * 1000 * vcodivsel / prediv / posdiv; 
#else   
    con1 &= _BITMASK_(26 : 0);
    ...
    if (!is_fhctl_used) {    
        ...
        switch (target_khz) {
            ...
            case CPU_DVFS_FREQ8:
                ...
                break;
            case CPU_DVFS_FREQ9:
                dds = _cpu_dds_calc(1404000);   
                sel = 10;    
                break;
            case CPU_DVFS_FREQ10:
                dds = _cpu_dds_calc(936000);   
                sel = 10;    
                break;
            default:
                BUG();
        ...
        cpufreq_write(ARMCA7PLL_CON1, dds | _BIT_(31)); 
...
static int set_cur_volt_pmic_wrap(struct mt_cpu_dvfs *p, unsigned int volt) 
{
    ...
    if (NULL != g_pCpuVoltSampler)
        g_pCpuVoltSampler(MT_CPU_DVFS_LITTLE, volt / 100);  
...
{
#define CA53_8CORE_REF_POWER	2286	
#define CA53_6CORE_REF_POWER	1736	
#define CA53_4CORE_REF_POWER	1159	
...
static int _search_available_freq_idx(struct mt_cpu_dvfs *p, unsigned int target_khz, unsigned int relation) 
{
    int new_opp_idx = -1;
    int i;
...
static unsigned int _calc_new_opp_idx(struct mt_cpu_dvfs *p, int new_opp_idx)
{
    ...
    if (p->limited_freq_by_hevc && policy && policy->governor && strcmp(policy->governor->name, "userspace") != 0) {
...
#define DORMANT_MODE_VOLT   80000
        ...
        if (!cpu_dvfs_is_extbuck_valid()) {
            ...
            pmic_config_interface(PMIC_ADDR_VPROC_VOSEL, VOLT_TO_PMIC_VAL(DORMANT_MODE_VOLT), 0x7F, 0x0); 

            
            
            pmic_config_interface(0x42E, 0x7, 0x7, 0x0);        
            
            pmic_config_interface(0x502, 0x84FF, 0xFFFF, 0x0);  
            
            pmic_config_interface(0x506, 0x40, 0x7F, 0x0);      
            pmic_config_interface(0x430, 0xF01, 0x7F7F, 0x0);   
            
            pmic_config_interface(0x432, 0x5835, 0x7F7F, 0x0);  
            
            pmic_config_interface(0x434, 0x0, 0x7F, 0x0);   
            
            pmic_config_interface(0x4B0, 0x1, 0x1, 0x1);    
            pmic_config_interface(0x4FC, 0x1, 0x1, 0x1);    
            
            pmic_config_interface(0x4BA, 0x30, 0x7F, 0x0);  
            pmic_config_interface(0x4BC, 0x10, 0x7F, 0x0);  
            
            pmic_config_interface(0x4B6, 0x84FF, 0xFFFF, 0x0);  
            pmic_config_interface(0x4C6, 0x13, 0x3F, 0x0);      
        } else {
            ...
            pmic_config_interface(PMIC_ADDR_VSRAM_VOSEL_CTRL, 0x1, 0x1, 0x1);
            pmic_config_interface(PMIC_ADDR_VSRAM_VOSEL, VOLT_TO_PMIC_VAL(MIN_VSRAM_VOLT), 0x7F, 0x9); 
        }
...
void dvfs_disable_by_ptpod(void)
{
    ...
    dvfs_set_vcore_ao_volt(VOLT_TO_PMIC_VAL(100000)); 
...
static ssize_t cpufreq_limited_by_hevc_proc_write(struct file *file, const char __user *buffer, size_t count, loff_t *pos)
{
    ...
    if (sscanf(buf, "%d", &limited_freq_by_hevc) == 1) {
        ...
        if (cpu_dvfs_is_availiable(p) && (p->limited_freq_by_hevc > cpu_dvfs_get_cur_freq(p))) {
            ...
            if (policy && policy->governor && strcmp(policy->governor->name, "userspace") != 0) {
