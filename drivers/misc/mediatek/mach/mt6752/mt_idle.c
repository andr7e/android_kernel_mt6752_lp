...
enum {
	MT_IDLE_DEBUG_DEEP_TIMER = BIT(0),
	MT_IDLE_DEBUG_SO_TIMER	= BIT(1),
};

static int mt_idle_debug_mask = 0;
module_param_named(debug_mask,mt_idle_debug_mask,int,S_IRUGO | S_IWUSR | S_IWGRP);
...
static unsigned int     dpidle_block_time_critera = 30000;
...
static unsigned int     mcidle_time_critera = 39000;
	...
	if (idle_switch[IDLE_TYPE_MC]) {
		for(;;)
		{
			if(cpu==0)
			{
				...
				if((spm_read(SPM_SLEEP_TIMER_STA)&0xfe)!=0x0)
				{
					mdelay(1);
...
#ifdef CONFIG_SMP
	if(mt_idle_debug_mask & MT_IDLE_DEBUG_SO_TIMER)
...
#ifdef CONFIG_SMP
	if(mt_idle_debug_mask & MT_IDLE_DEBUG_DEEP_TIMER)
