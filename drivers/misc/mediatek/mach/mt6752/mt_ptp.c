...
static struct ptp_det ptp_detectors[NR_PTP_DET] = {
	...
	[PTP_DET_MCUSYS] = {
		...
		.max_freq_khz	= 1700000, 
		.VBOOT		= PTP_VOLT_TO_PMIC_VAL(100000),  
	},
...
static void ptp_init_det(struct ptp_det *det, struct ptp_devinfo *devinfo)
{
	...
	case PTP_DET_MCUSYS:
		...
		det->VMIN	= PTP_VOLT_TO_PMIC_VAL(80000); 
		break;
	...
	case PTP_DET_SOC:
        ...
        det->VMIN	= PTP_VOLT_TO_PMIC_VAL(100000); 
	...
	if (det->ctrl_id == PTP_CTRL_MCUSYS) {
		f_index = (ptp_devinfo.PTP_OD1 & 0xC0) >> 6;
		f_max = mt_cpufreq_get_freq_by_idx(MT_CPU_DVFS_LITTLE, 0);

		if (f_max == 1287000) {
			switch (f_index) {
			case 1:
				det->MDES = (25928 + det->MTDES) / det->MTDES;
				det->BDES = (273 * det->MTDES - 12215) / det->MTDES;
				break;
			case 2:
				det->MDES = (11883 + det->MTDES) / det->MTDES;
				det->BDES = (273 * det->MTDES - 3824) / det->MTDES;
				break;
			case 3:
				det->MDES = (6790 + det->MTDES)/det->MTDES;
				det->BDES = (33 * det->MTDES - 1562) / det->MTDES;
				break;
			}
		}
		else if (f_max == 1495000) {
			switch (f_index) {
			case 1:
				det->MDES = (15011 + det->MTDES)/det->MTDES;
				det->BDES = (273 * det->MTDES - 7072) / det->MTDES;
				break;
			case 2:
				det->MDES = (8912 + det->MTDES)/det->MTDES;
				det->BDES = (273 * det->MTDES - 2869) / det->MTDES;
				break;
			case 3:
				det->MDES = (5704 + det->MTDES) / det->MTDES;
				det->BDES = (33 * det->MTDES - 1312) / det->MTDES;
				break;
			}
		}
	}
...
void get_devinfo(struct ptp_devinfo *p)
{
	...
	#if 0
	val[0] = ptp_read(0xF0206660); 
	val[1] = ptp_read(0xF0206664); 
	val[2] = ptp_read(0xF0206668); 
	val[3] = ptp_read(0xF020666C); 
	val[4] = ptp_read(0xF0206670); 
	val[5] = ptp_read(0xF0206674); 
	val[6] = ptp_read(0xF0206678); 
	val[7] = ptp_read(0xF020667C); 
	#endif
