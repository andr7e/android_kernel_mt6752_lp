...
#define MVG_CFG_GPIO_TO_VCC_DELAY 5000   
#define MVG_CFG_VCC_DROP_TIME     4000   
        ...
        if (mvg_addr_range_check(addr)) { \
            if ((time_max)&&(mvg_trigger())) { \
                ...
                if (mvg_get_method()==MVG_METHOD_HW_CUT) { \
                    ...
                    printk(KERN_ERR "[MVG_TEST]: HW reset in %d (GPIO+%d) us\n", time, MVG_CFG_GPIO_TO_VCC_DELAY); \
                    time |= 0x80000000; \
                    time += MVG_CFG_GPIO_TO_VCC_DELAY; \
                } else { \
                    time = time % time_max +1; \
                    printk(KERN_ERR "[MVG_TEST]: SW reset in %d us\n", time); \
                } \
                dump_stack(); \
                udelay(350); \
            } } } while(0)
...
#define PL_RESET_ON_CASE(group_name, case_name) do { \
            if (mvg_on_group_case(group_name, case_name) && mvg_get_wdt()) { \
                printk(KERN_ERR "[MVG_TEST]: reset on case %s: %s\n", group_name, case_name); \
                mvg_set_trigger(-1); \
            } } while(0)
