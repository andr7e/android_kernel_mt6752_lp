...
#ifdef LINUX
UINT_8        keyBuffer[sizeof(PARAM_KEY_T) + 16 ];
UINT_8        aucBCAddr[] = BC_MAC_ADDR;
#endif
WLAN_STATUS
wlanoidSetAddWep (
    ...
    )
{
    ...
    *pu4SetInfoLen = u4SetBufferLen;
    ...
    u4KeyId = prNewWepKey->u4KeyIndex & BITS(0,29) ;
    ...
    )
{
    ...
    *pu4QueryInfoLen = sizeof(PARAM_RSSI);
    ...
    if (kalGetMediaStateIndicated(prAdapter->prGlueInfo) == PARAM_MEDIA_STATE_DISCONNECTED) {
        ...
        rRssi = (PARAM_RSSI)prAdapter->rLinkQuality.cRssi; 
    ...
    )
{
    ...
    if(IsBufferedStatisticsUsable(prAdapter) == TRUE) {
        if(u4QueryBufferLen == sizeof(UINT_32)) {
            *pu4QueryInfoLen = sizeof(UINT_32);
            *(PUINT_32) pvQueryBuffer = (UINT_32) 0; 
        }
        else {
            *pu4QueryInfoLen = sizeof(UINT_64);
            *(PUINT_64) pvQueryBuffer = (UINT_64) 0; 
        }
...
#define MTK_CUSTOM_OID_INTERFACE_VERSION     0x00006620    
/*!
    ...
    )
{
 ...
 {
     UINT_8 ucNetTypeIndex = NETWORK_TYPE_AIS_INDEX; 
    ...
    )
{
    ...
    *pu4SetInfoLen = sizeof(UINT_32);
    ...
    rCmdBasicConfig.ucNative80211 = 0; 
    ...
    *pu4SetInfoLen = 0;
    ...
    if(u4SetBufferLen == 0) {
        if(prAdapter->fgTestMode == FALSE) {
            ...
            rCmdTestCtrl.ucAction = 0; 
            rCmdTestCtrl.u.u4OpMode = 1; 
    ...
    )
{
    ...
    *pu4SetInfoLen = sizeof(PARAM_MTK_SLT_TEST_STRUC_T);
    ...
    switch (prMtkSltInfo->rSltFuncIdx) {
    case ENUM_MTK_SLT_FUNC_INITIAL: 
        {
        ...
        break;
    case ENUM_MTK_SLT_FUNC_RATE_SET: 
        if (prSltInfo->prPseudoStaRec == NULL) {
            rWlanStatus = WLAN_STATUS_FAILURE;
        }
        else {
