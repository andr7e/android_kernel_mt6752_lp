...
#define LINKSPEED_MAX_RANGE_11BGN 3000
    ...
    if (rStatus != WLAN_STATUS_SUCCESS) {
        ...
        if(sinfo->txrate.legacy > LINKSPEED_MAX_RANGE_11BGN){
            DBGLOG(REQ, TRACE, ("Disconnected with cached Linkspeed(%d)->(%d)\n",sinfo->txrate.legacy,prGlueInfo->i4LinkSpeedCache));
            sinfo->txrate.legacy = prGlueInfo->i4LinkSpeedCache;
        }
        else{
    ...
    if(prGlueInfo->eParamMediaStateIndicated != PARAM_MEDIA_STATE_CONNECTED) {
        ...
        sinfo->filled |= STATION_INFO_TX_FAILED;
       {
            ...
            if (rStatus != WLAN_STATUS_SUCCESS) {
        		...
        		INT_32 i4RssiThreshold = -85; 
        		UINT_32 u4LinkspeedThreshold = 55; 
        		...
        		DBGLOG(REQ, TRACE,("Poorlink State TxOK(%d) TxFail(%d) Bad(%d) Retry(%d) Rate(%d) Signal(%d) Weight(%d) (%d)\n",
            		sinfo->tx_packets,
            		sinfo->tx_failed,
                    (int)u8diffTxBad,
            		(int)u8diffRetry,
    ...
    )
{
    ...
    if(prParams){
        switch(prParams->index){
            case TESTMODE_CMD_ID_SW_CMD: 
                i4Status = mtk_cfg80211_testmode_sw_cmd(wiphy, data, len);
                break;
            case TESTMODE_CMD_ID_WAPI: 
#if CFG_SUPPORT_WAPI
...
#define CHN_DIRTY_WEIGHT_UPPERBOUND 4
    ...
    if (fgIsPureAP)
    {
		...
		AcsChnRepot[NL80211_TESTMODE_AVAILABLE_CHAN_2G_BASE_1-1]	=0x20;
    }
	else
#endif
	{
