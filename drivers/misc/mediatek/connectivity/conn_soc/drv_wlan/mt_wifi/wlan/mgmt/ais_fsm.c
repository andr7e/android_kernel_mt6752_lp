	...
	LINK_FOR_EACH_ENTRY(prBssDesc, prBSSDescList, rLinkEntry, BSS_DESC_T) {
		...
		if (EQUAL_MAC_ADDR(prStaRec->aucMacAddr, prBssDesc->aucBSSID))
		{
			...
			else
			{
				...
				prBssDesc->ucBssErrTime = rCurrentTime; 
    ...
    )
{
    ...
    if (prAisBssInfo->eBand == BAND_2G4) {
        ...
        prAisBssInfo->ucPhyTypeSet =
            prAdapter->rWifiVar.ucAvailablePhyTypeSet & PHY_TYPE_SET_802_11BGN; 
        ...
        prAisBssInfo->ucPhyTypeSet =
            prAdapter->rWifiVar.ucAvailablePhyTypeSet & PHY_TYPE_SET_802_11AN; 
