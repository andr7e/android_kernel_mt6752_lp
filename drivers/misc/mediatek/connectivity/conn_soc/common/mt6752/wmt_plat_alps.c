    ...
    )
{
    ...
    if (PIN_STA_INIT == state) {
		...
		if (node) {
			bgf_irq = irq_of_parse_and_map(node, 0);
			...
			iret = request_irq(bgf_irq, wmt_plat_bgf_irq_isr, IRQF_TRIGGER_LOW,"BTIF_WAKEUP_IRQ", NULL);
			if (iret) {
				WMT_PLAT_ERR_FUNC("request_irq fail,irq_no(%d),iret(%d)\n",MT_CONN2AP_BTIF_WAKEUP_IRQ_ID,iret);
				return iret;
			} else {
				WMT_PLAT_DBG_FUNC("BGF EINT irq %d !!\n",bgf_irq);
    ...
    )
{
    if ( (PIN_ID_MAX > id)
        ...
        if (gfp_set_pin_table[id]) {
            return (*(gfp_set_pin_table[id]))(state); 
        }
        else {
...
INT32 wmt_plat_i2s_ctrl(ENUM_PIN_STATE state)
{
    ...
    #else
    WMT_PLAT_ERR_FUNC( "Error:FM digital mode set, but no I2S GPIOs defined\n");
    #endif
#else
        WMT_PLAT_INFO_FUNC( "warnning:FM digital mode is not set, no I2S GPIO settings should be modified by combo driver\n");
#endif
