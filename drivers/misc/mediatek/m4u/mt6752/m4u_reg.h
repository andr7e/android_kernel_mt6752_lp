#ifndef _MT6752_M4U_REG_H__
#define _MT6752_M4U_REG_H__
...
#define REG_MMU_INT_ID(mmu)             (0x150+((mmu)<<2))
    #define F_MMU0_INT_ID_TF_MSK        (~0x3) 
...
#define REG_MMU_PFH_VLD(mmu, set, way)     (REG_MMU_PFH_VLD_0+(((set)>>5)<<2)+((way)<<((mmu==0)?(MMU0_SET_ORDER - 3):(MMU1_SET_ORDER - 3)))) 
    #define F_MMU_PFH_VLD_BIT(set, way)      F_BIT_SET((set)&0x1f)       
