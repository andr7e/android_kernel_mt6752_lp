						...
						const CMDQ_DATA_REGISTER_ENUM destRegId)
{
	...
	cmdq_core_append_command_into_task(pTask, (CMDQ_CODE_READ << 24) | (argA & 0xffff) | ((subsysCode & 0x1f) << 16) | (2 << 21),	
					   valueRegId);
	...
	cmdq_core_append_command_into_task(pTask, (CMDQ_CODE_WRITE << 24) | (0 & 0xffff) | ((destRegId & 0x1f) << 16) | (6 << 21),	
					   valueRegId);
						       ...
						       cmdqCommandStruct *pCommandDesc)
{
	...
	if (0 == pTask->regCount && extraBufferSize == 0) {
		...
		if (pTask->regCount) {
			...
			cmdq_core_append_command_into_task(pTask, (CMDQ_CODE_WFE << 24) | regAccessToken, ((1 << 31) | (1 << 15) | 1)	
			    );
			...
			cmdq_core_append_command_into_task(pTask, (CMDQ_CODE_WFE << 24) | regAccessToken, ((1 << 31) | (1 << 16))	
			    );
		}
#ifdef CMDQ_PROFILE_COMMAND
		if (profileCommand) {
...
DEBUG_STATIC int32_t cmdq_core_exec_task_async_impl(TaskStruct *pTask, int32_t thread)
{
	...
	if (pThread->taskCount <= 0) {
		...
		if ((CMDQ_AREG_TO_PHYS(CMDQ_REG_GET32(CMDQ_THR_CURR_ADDR(thread))) == (CMDQ_AREG_TO_PHYS(CMDQ_REG_GET32(CMDQ_THR_END_ADDR(thread))) - 8)) ||	
		    (CMDQ_AREG_TO_PHYS(CMDQ_REG_GET32(CMDQ_THR_CURR_ADDR(thread))) == (CMDQ_AREG_TO_PHYS(CMDQ_REG_GET32(CMDQ_THR_END_ADDR(thread))) - 0))) {	
