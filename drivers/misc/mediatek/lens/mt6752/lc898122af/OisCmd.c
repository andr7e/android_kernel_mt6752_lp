#define		OISCMD

#include	"Ois.h"
#include	"OisDef.h"

void			MesFil( unsigned char ) ;					
#ifdef	MODULE_CALIBRATION
 #ifndef	HALLADJ_HW
 void			LopIni( unsigned char ) ;					
 #endif
 void			LopPar( unsigned char ) ;					
 #ifndef	HALLADJ_HW
  void			LopSin( unsigned char, unsigned char ) ;	
  unsigned char	LopAdj( unsigned char ) ;					
  void			LopMes( void ) ;							
 #endif
#endif
#ifndef	HALLADJ_HW
unsigned long	GinMes( unsigned char ) ;					
#endif
void			GyrCon( unsigned char ) ;					
short			GenMes( unsigned short, unsigned char ) ;	
#ifndef	HALLADJ_HW
 unsigned long	TneOff( UnDwdVal, unsigned char ) ;			
 unsigned long	TneBia( UnDwdVal, unsigned char ) ;			
#endif

void 			StbOnn( void ) ;							

void			SetSineWave(   unsigned char , unsigned char );
void			StartSineWave( void );
void			StopSineWave(  void );

void			SetMeasFil(  unsigned char );
void			ClrMeasFil( void );



#define		MES_XG1			0								
#define		MES_XG2			1								

#define		HALL_ADJ		0
#define		LOOPGAIN		1
#define		THROUGH			2
#define		NOISE			3


 #define		TNE 			80								
#ifdef	HALLADJ_HW

 #define     __MEASURE_LOOPGAIN      0x00
 #define     __MEASURE_BIASOFFSET    0x01

#else

 
 #define		MARJIN			0x0300							
 #define		BIAS_ADJ_BORDER	0x1998							

 #define		HALL_MAX_GAP	BIAS_ADJ_BORDER - MARJIN
 #define		HALL_MIN_GAP	BIAS_ADJ_BORDER + MARJIN
 
 
 #define		BIAS_LIMIT		0xFFFF							
 #define		OFFSET_DIV		2								
 #define		TIME_OUT		40								

 
 #define		MARGIN			0x0300							

 #define		BIAS_ADJ_OVER	0xD998							
 #define		BIAS_ADJ_RANGE	0xCCCC							
 #define		BIAS_ADJ_SKIP	0xBFFF							
 #define		HALL_MAX_RANGE	BIAS_ADJ_RANGE + MARGIN
 #define		HALL_MIN_RANGE	BIAS_ADJ_RANGE - MARGIN

 #define		DECRE_CAL		0x0100							
 
#endif

#ifdef H1COEF_CHANGER
 #ifdef	CORRECT_1DEG
  #define		MAXLMT		0x40400000				
  #define		MINLMT		0x3FE66666				
  #define		CHGCOEF		0xBA195555				
 #else
  #define		MAXLMT		0x40000000				
  #define		MINLMT		0x3F8CCCCD				
  #define		CHGCOEF		0xBA4C71C7				
 #endif
  #define		MINLMT_MOV	0x00000000				
  #define		CHGCOEF_MOV	0xB9700000
#endif

#ifdef	HALLADJ_HW
 unsigned char UcAdjBsy;

#else
 unsigned short	UsStpSiz	= 0 ;							
 unsigned short	UsErrBia, UsErrOfs ;
#endif



#define		ZOOMTBL	16
const unsigned long	ClGyxZom[ ZOOMTBL ]	= {
		0x3F800000,
		0x3F800000,
		0x3F800000,
		0x3F800000,
		0x3F800000,
		0x3F800000,
		0x3F800000,
		0x3F800000,
		0x3F800000,
		0x3F800000,
		0x3F800000,
		0x3F800000,
		0x3F800000,
		0x3F800000,
		0x3F800000,
		0x3F800000
	} ;

const unsigned long	ClGyyZom[ ZOOMTBL ]	= {
		0x3F800000,
		0x3F800000,
		0x3F800000,
		0x3F800000,
		0x3F800000,
		0x3F800000,
		0x3F800000,
		0x3F800000,
		0x3F800000,
		0x3F800000,
		0x3F800000,
		0x3F800000,
		0x3F800000,
		0x3F800000,
		0x3F800000,
		0x3F800000
	} ;

#define		COEFTBL	7
const unsigned long	ClDiCof[ COEFTBL ]	= {
		DIFIL_S2,		
		DIFIL_S2,		
		DIFIL_S2,		
		DIFIL_S2,		
		DIFIL_S2,		
		DIFIL_S2,		
		DIFIL_S2		
	} ;

unsigned short	TneRun( void )
{
	unsigned char	UcHlySts, UcHlxSts, UcAtxSts, UcAtySts ;
	unsigned short	UsFinSts , UsOscSts ; 								
	unsigned char	UcDrvMod ;
#ifndef	HALLADJ_HW
	UnDwdVal		StTneVal ;
#endif

#ifdef	USE_EXTCLK_ALL	
	UsOscSts	= EXE_END ;
#else
 #ifdef	MODULE_CALIBRATION
	
	UsOscSts	= OscAdj() ;
 #else
	UsOscSts	= EXE_END ;
 #endif
#endif
	
	UcDrvMod = UcPwmMod ;
	if( UcDrvMod == PWMMOD_CVL )
	{
		DrvPwmSw( Mpwm ) ;		
	}
	
#ifdef	HALLADJ_HW
	UcHlySts = BiasOffsetAdj( Y_DIR , 0 ) ;
	WitTim( TNE ) ;
	UcHlxSts = BiasOffsetAdj( X_DIR , 0 ) ;
	WitTim( TNE ) ;
	UcHlySts = BiasOffsetAdj( Y_DIR , 1 ) ;
	WitTim( TNE ) ;
	UcHlxSts = BiasOffsetAdj( X_DIR , 1 ) ;

	SrvCon( Y_DIR, OFF ) ;
	SrvCon( X_DIR, OFF ) ;
	
	if( UcDrvMod == PWMMOD_CVL )
	{
		DrvPwmSw( Mlnp ) ;		
	}
	
  #ifdef	NEUTRAL_CENTER
	TneHvc();
  #endif	
#else
	WitTim( TNE ) ;

	StTneVal.UlDwdVal	= TnePtp( Y_DIR , PTP_BEFORE ) ;
	UcHlySts	= TneCen( Y2_DIR, StTneVal ) ;
	
	StbOnnN( ON , OFF ) ;				
	WitTim( TNE ) ;

	StTneVal.UlDwdVal	= TnePtp( X_DIR , PTP_BEFORE ) ;
	UcHlxSts	= TneCen( X2_DIR, StTneVal ) ;

	StbOnnN( OFF , ON ) ;				
	WitTim( TNE ) ;

	StTneVal.UlDwdVal	= TnePtp( Y_DIR , PTP_AFTER ) ;
	UcHlySts	= TneCen( Y2_DIR, StTneVal ) ;

	StbOnnN( ON , OFF ) ;				
	WitTim( TNE ) ;

	StTneVal.UlDwdVal	= TnePtp( X_DIR , PTP_AFTER ) ;
	UcHlxSts	= TneCen( X2_DIR, StTneVal ) ;

	SrvCon( Y_DIR, OFF ) ;
	SrvCon( X_DIR, OFF ) ;
	
	if( UcDrvMod == PWMMOD_CVL )
	{
		DrvPwmSw( Mlnp ) ;		
	}
	
  #ifdef	NEUTRAL_CENTER
	TneHvc();
  #endif	
#endif
	

	WitTim( TNE ) ;

	RamAccFixMod( ON ) ;							
	
	StAdjPar.StHalAdj.UsAdxOff = (unsigned short)((unsigned long)0x00010000 - (unsigned long)StAdjPar.StHalAdj.UsHlxCna ) ;
	StAdjPar.StHalAdj.UsAdyOff = (unsigned short)((unsigned long)0x00010000 - (unsigned long)StAdjPar.StHalAdj.UsHlyCna ) ;
	
	RamWriteA( OFF0Z,  StAdjPar.StHalAdj.UsAdxOff ) ;	
	RamWriteA( OFF1Z,  StAdjPar.StHalAdj.UsAdyOff ) ;	

	RamReadA( DAXHLO, &StAdjPar.StHalAdj.UsHlxOff ) ;		
	RamReadA( DAXHLB, &StAdjPar.StHalAdj.UsHlxGan ) ;		
	RamReadA( DAYHLO, &StAdjPar.StHalAdj.UsHlyOff ) ;		
	RamReadA( DAYHLB, &StAdjPar.StHalAdj.UsHlyGan ) ;		
	RamReadA( OFF0Z, &StAdjPar.StHalAdj.UsAdxOff ) ;		
	RamReadA( OFF1Z, &StAdjPar.StHalAdj.UsAdyOff ) ;		
	
	RamAccFixMod( OFF ) ;							
	
	StbOnn() ;											

	
	WitTim( TNE ) ;

#ifdef	MODULE_CALIBRATION
	
	UcAtxSts	= LopGan( X_DIR ) ;


	
	UcAtySts	= LopGan( Y_DIR ) ;
#else		
	RamAccFixMod( ON ) ;								
	RamReadA( sxg, &StAdjPar.StLopGan.UsLxgVal ) ;		
	RamReadA( syg, &StAdjPar.StLopGan.UsLygVal ) ;		
	RamAccFixMod( OFF ) ;								
	UcAtxSts	= EXE_END ;
	UcAtySts	= EXE_END ;
#endif
	

	TneGvc() ;


	UsFinSts	= (unsigned short)( UcHlxSts - EXE_END ) + (unsigned short)( UcHlySts - EXE_END ) + (unsigned short)( UcAtxSts - EXE_END ) + (unsigned short)( UcAtySts - EXE_END ) + ( UsOscSts - (unsigned short)EXE_END ) + (unsigned short)EXE_END ;


	return( UsFinSts ) ;
}


#ifndef	HALLADJ_HW

 
unsigned long	TnePtp ( unsigned char	UcDirSel, unsigned char	UcBfrAft )
{
	UnDwdVal		StTneVal ;

	MesFil( THROUGH ) ;					


	if ( !UcDirSel ) {
		RamWrite32A( sxsin , HALL_H_VAL );		
		SetSinWavePara( 0x0A , XHALWAVE ); 
	}else{
	    RamWrite32A( sysin , HALL_H_VAL ); 		
		SetSinWavePara( 0x0A , YHALWAVE ); 
	}

	if ( !UcDirSel ) {					
		RegWriteA( WC_MES1ADD0,  ( unsigned char )AD0Z ) ;							
		RegWriteA( WC_MES1ADD1,  ( unsigned char )(( AD0Z >> 8 ) & 0x0001 ) ) ;		
	} else {							
		RegWriteA( WC_MES1ADD0,  ( unsigned char )AD1Z ) ;							
		RegWriteA( WC_MES1ADD1,  ( unsigned char )(( AD1Z >> 8 ) & 0x0001 ) ) ;		
	}

	RegWriteA( WC_MESLOOP1	, 0x00 );			
	RegWriteA( WC_MESLOOP0	, 0x01);			
	
	RamWrite32A( msmean	, 0x3F800000 );			
	
	RamWrite32A( MSMAX1, 	0x00000000 ) ;		
	RamWrite32A( MSMAX1AV, 	0x00000000 ) ;		
	RamWrite32A( MSMIN1, 	0x00000000 ) ;		
	RamWrite32A( MSMIN1AV, 	0x00000000 ) ;		
	
	RegWriteA( WC_MESABS, 0x00 ) ;				
	BsyWit( WC_MESMODE, 0x02 ) ;				

	RamAccFixMod( ON ) ;							
	
	RamReadA( MSMAX1AV, &StTneVal.StDwdVal.UsHigVal ) ;		
	RamReadA( MSMIN1AV, &StTneVal.StDwdVal.UsLowVal ) ;		

	RamAccFixMod( OFF ) ;							

	if ( !UcDirSel ) {					
		SetSinWavePara( 0x00 , XHALWAVE ); 	
	}else{
		SetSinWavePara( 0x00 , YHALWAVE ); 	
	}

	if( UcBfrAft == 0 ) {
		if( UcDirSel == X_DIR ) {
			StAdjPar.StHalAdj.UsHlxCen	= ( ( signed short )StTneVal.StDwdVal.UsHigVal + ( signed short )StTneVal.StDwdVal.UsLowVal ) / 2 ;
			StAdjPar.StHalAdj.UsHlxMax	= StTneVal.StDwdVal.UsHigVal ;
			StAdjPar.StHalAdj.UsHlxMin	= StTneVal.StDwdVal.UsLowVal ;
		} else {
			StAdjPar.StHalAdj.UsHlyCen	= ( ( signed short )StTneVal.StDwdVal.UsHigVal + ( signed short )StTneVal.StDwdVal.UsLowVal ) / 2 ;
			StAdjPar.StHalAdj.UsHlyMax	= StTneVal.StDwdVal.UsHigVal ;
			StAdjPar.StHalAdj.UsHlyMin	= StTneVal.StDwdVal.UsLowVal ;
		}
	} else {
		if( UcDirSel == X_DIR ){
			StAdjPar.StHalAdj.UsHlxCna	= ( ( signed short )StTneVal.StDwdVal.UsHigVal + ( signed short )StTneVal.StDwdVal.UsLowVal ) / 2 ;
			StAdjPar.StHalAdj.UsHlxMxa	= StTneVal.StDwdVal.UsHigVal ;
			StAdjPar.StHalAdj.UsHlxMna	= StTneVal.StDwdVal.UsLowVal ;
		} else {
			StAdjPar.StHalAdj.UsHlyCna	= ( ( signed short )StTneVal.StDwdVal.UsHigVal + ( signed short )StTneVal.StDwdVal.UsLowVal ) / 2 ;
			StAdjPar.StHalAdj.UsHlyMxa	= StTneVal.StDwdVal.UsHigVal ;
			StAdjPar.StHalAdj.UsHlyMna	= StTneVal.StDwdVal.UsLowVal ;
		}
	}

	StTneVal.StDwdVal.UsHigVal	= 0x7fff - StTneVal.StDwdVal.UsHigVal ;		
	StTneVal.StDwdVal.UsLowVal	= StTneVal.StDwdVal.UsLowVal - 0x7fff ; 	

	
	return( StTneVal.UlDwdVal ) ;
}

unsigned short	UsValBef,UsValNow ;
unsigned char	TneCen( unsigned char	UcTneAxs, UnDwdVal	StTneVal )
{
	unsigned char 	UcTneRst, UcTmeOut, UcTofRst ;
	unsigned short	UsOffDif ;
	unsigned short	UsBiasVal ;

	UsErrBia	= 0 ;
	UsErrOfs	= 0 ;
	UcTmeOut	= 1 ;
	UsStpSiz	= 1 ;
	UcTneRst	= FAILURE ;
	UcTofRst	= FAILURE ;

	while ( UcTneRst && UcTmeOut )
	{
		if( UcTofRst == FAILURE ) {
			StTneVal.UlDwdVal	= TneOff( StTneVal, UcTneAxs ) ;
		} else {
			StTneVal.UlDwdVal	= TneBia( StTneVal, UcTneAxs ) ;
			UcTofRst	= FAILURE ;
		}

		if( !( UcTneAxs & 0xF0 ) )
		{
			if ( StTneVal.StDwdVal.UsHigVal > StTneVal.StDwdVal.UsLowVal ) {									
				UsOffDif	= ( StTneVal.StDwdVal.UsHigVal - StTneVal.StDwdVal.UsLowVal ) / 2 ;
			} else {
				UsOffDif	= ( StTneVal.StDwdVal.UsLowVal - StTneVal.StDwdVal.UsHigVal ) / 2 ;
			}

			if( UsOffDif < MARJIN ) {
				UcTofRst	= SUCCESS ;
			} else {
				UcTofRst	= FAILURE ;
			}

			if ( ( StTneVal.StDwdVal.UsHigVal < HALL_MIN_GAP && StTneVal.StDwdVal.UsLowVal < HALL_MIN_GAP )		
			&& ( StTneVal.StDwdVal.UsHigVal > HALL_MAX_GAP && StTneVal.StDwdVal.UsLowVal > HALL_MAX_GAP ) ) {
				UcTneRst	= SUCCESS ;
				break ;
			} else if ( UsStpSiz == 0 ) {
				UcTneRst	= SUCCESS ;
				break ;
			} else {
				UcTneRst	= FAILURE ;
				UcTmeOut++ ;
			}
		}else{
			if( (StTneVal.StDwdVal.UsHigVal > MARGIN ) && (StTneVal.StDwdVal.UsLowVal > MARGIN ) )	
			{
				UcTofRst	= SUCCESS ;
				UsValBef = UsValNow = 0x0000 ;
			}else if( (StTneVal.StDwdVal.UsHigVal <= MARGIN ) && (StTneVal.StDwdVal.UsLowVal <= MARGIN ) ){
				UcTofRst	= SUCCESS ;
				UcTneRst	= FAILURE ;
			}else if( ((unsigned short)0xFFFF - ( StTneVal.StDwdVal.UsHigVal + StTneVal.StDwdVal.UsLowVal )) > BIAS_ADJ_OVER ) {
				UcTofRst	= SUCCESS ;
				UcTneRst	= FAILURE ;
			}else{
				UcTofRst	= FAILURE ;

				
				UsValBef = UsValNow ;

				RamAccFixMod( ON ) ;							
				
				if( !( UcTneAxs & 0x0F ) ) {
					RamReadA( DAXHLO, &UsValNow ) ;				
				}else{
					RamReadA( DAYHLO, &UsValNow ) ;				
				}
				if( ((( UsValBef & 0xFF00 ) == 0x8000 ) && ( UsValNow & 0xFF00 ) == 0x8000 )
				 || ((( UsValBef & 0xFF00 ) == 0x7F00 ) && ( UsValNow & 0xFF00 ) == 0x7F00 ) )
				{
					if( !( UcTneAxs & 0x0F ) ) {
						RamReadA( DAXHLB, &UsBiasVal ) ;		
					}else{
						RamReadA( DAYHLB, &UsBiasVal ) ;		
					}
					if( UsBiasVal > 0x8000 )
					{
						UsBiasVal -= 0x8000 ;
					}
					else
					{
						UsBiasVal += 0x8000 ;
					}
					if( UsBiasVal > DECRE_CAL )
					{
						UsBiasVal -= DECRE_CAL ;
					}
					UsBiasVal += 0x8000 ;
					
					if( !( UcTneAxs & 0x0F ) ) {
						RamWriteA( DAXHLB, UsBiasVal ) ;		
					}else{
						RamWriteA( DAYHLB, UsBiasVal ) ;		
					}
				}

				RamAccFixMod( OFF ) ;							
				
			}
			
			if((( (unsigned short)0xFFFF - ( StTneVal.StDwdVal.UsHigVal + StTneVal.StDwdVal.UsLowVal )) < HALL_MAX_RANGE )
			&& (( (unsigned short)0xFFFF - ( StTneVal.StDwdVal.UsHigVal + StTneVal.StDwdVal.UsLowVal )) > HALL_MIN_RANGE ) ) {
				if(UcTofRst	== SUCCESS)
				{
					UcTneRst	= SUCCESS ;
					break ;
				}
			}
			UcTneRst	= FAILURE ;
			UcTmeOut++ ;
		}

		if( UcTneAxs & 0xF0 )
		{
			if ( ( UcTmeOut / 2 ) == TIME_OUT ) {
				UcTmeOut	= 0 ;
			}		 																							
		}else{
			if ( UcTmeOut == TIME_OUT ) {
				UcTmeOut	= 0 ;
			}		 																							
		}
	}

	if( UcTneRst == FAILURE ) {
		if( !( UcTneAxs & 0x0F ) ) {
			UcTneRst					= EXE_HXADJ ;
			StAdjPar.StHalAdj.UsHlxGan	= 0xFFFF ;
			StAdjPar.StHalAdj.UsHlxOff	= 0xFFFF ;
		} else {
			UcTneRst					= EXE_HYADJ ;
			StAdjPar.StHalAdj.UsHlyGan	= 0xFFFF ;
			StAdjPar.StHalAdj.UsHlyOff	= 0xFFFF ;
		}
	} else {
		UcTneRst	= EXE_END ;
	}

	return( UcTneRst ) ;
}



unsigned long	TneBia( UnDwdVal	StTneVal, unsigned char	UcTneAxs )
{
	long					SlSetBia ;
	unsigned short			UsSetBia ;
	unsigned char			UcChkFst ;
	static unsigned short	UsTneVax ;							
	unsigned short			UsDecCal ;

	UcChkFst	= 1 ;

	if ( UsStpSiz == 1) {
		UsTneVax	= 2 ;

		if( UcTneAxs & 0xF0 ){
			if ( ((unsigned short)0xFFFF - ( StTneVal.StDwdVal.UsHigVal + StTneVal.StDwdVal.UsLowVal )) > BIAS_ADJ_OVER ) {
				UcChkFst	= 0 ;

				RamAccFixMod( ON ) ;							
				
				if ( !( UcTneAxs & 0x0F ) ) {							
					RamReadA( DAXHLB, &UsSetBia ) ;		
				} else {
					RamReadA( DAYHLB, &UsSetBia ) ;		
				}
				if( UsSetBia > 0x8000 )
				{
					UsSetBia -= 0x8000 ;
				}
				else
				{
					UsSetBia += 0x8000 ;
				}
				if( !UcChkFst )	{
					UsDecCal = ( DECRE_CAL << 3 ) ;
				}else{
					UsDecCal = DECRE_CAL ;
				}
				if( UsSetBia > UsDecCal )
				{
					UsSetBia -= UsDecCal ;
				}
				UsSetBia += 0x8000 ;
				if ( !( UcTneAxs & 0x0F ) ) {							
					RamWriteA( DAXHLB, UsSetBia ) ;				
					RamWriteA( DAXHLO, 0x0000 ) ;				
				} else {
					RamWriteA( DAYHLB, UsSetBia ) ;				
					RamWriteA( DAYHLO, 0x0000 ) ;				
				}
				UsStpSiz	= BIAS_LIMIT / UsTneVax ;

				RamAccFixMod( OFF ) ;							
				
			}
		}else{
			if ( ( StTneVal.StDwdVal.UsHigVal + StTneVal.StDwdVal.UsLowVal ) / 2 < BIAS_ADJ_BORDER ) {
				UcChkFst	= 0 ;
			}

			if ( !UcTneAxs ) {										
				
				RamWrite32A( DAXHLB, 0xBF800000 ) ; 	
				RamWrite32A( DAXHLO, 0x00000000 ) ;		

				UsStpSiz	= BIAS_LIMIT / UsTneVax ;
			} else {
				RamWrite32A( DAYHLB, 0xBF800000 ) ; 	
				RamWrite32A( DAYHLO, 0x00000000 ) ;		
				UsStpSiz	= BIAS_LIMIT / UsTneVax ;
			}
		}
	}

	RamAccFixMod( ON ) ;							
	
	if ( !( UcTneAxs & 0x0F ) ) {
		RamReadA( DAXHLB, &UsSetBia ) ;					
		SlSetBia	= ( long )UsSetBia ;
	} else {
		RamReadA( DAYHLB, &UsSetBia ) ;					
		SlSetBia	= ( long )UsSetBia ;
	}

	if( SlSetBia >= 0x00008000 ) {
		SlSetBia	|= 0xFFFF0000 ;
	}

	if( UcChkFst ) {
		if( UcTneAxs & 0xF0 )
		{
			if ( ((unsigned short)0xFFFF - ( StTneVal.StDwdVal.UsHigVal + StTneVal.StDwdVal.UsLowVal )) < BIAS_ADJ_RANGE ) {	
				if( ((unsigned short)0xFFFF - ( StTneVal.StDwdVal.UsHigVal + StTneVal.StDwdVal.UsLowVal )) < BIAS_ADJ_SKIP ) {
					SlSetBia	+= 0x0400 ;
				}else{
					SlSetBia	+= 0x0100 ;
				}
			} else {
				if( ((unsigned short)0xFFFF - ( StTneVal.StDwdVal.UsHigVal + StTneVal.StDwdVal.UsLowVal )) > BIAS_ADJ_OVER ) {
					SlSetBia	-= 0x0400 ;
				}else{
					SlSetBia	-= 0x0100 ;
				}
			}
			UsStpSiz	= 0x0200 ;
			
		}else{
			if ( ( StTneVal.StDwdVal.UsHigVal + StTneVal.StDwdVal.UsLowVal ) / 2 > BIAS_ADJ_BORDER ) {	
				SlSetBia	+= UsStpSiz ;
			} else {
				SlSetBia	-= UsStpSiz ;
			}

			UsTneVax	= UsTneVax * 2 ;
			UsStpSiz	= BIAS_LIMIT / UsTneVax ;
		}
	}

	if( SlSetBia > ( long )0x00007FFF ) {
		SlSetBia	= 0x00007FFF ;
	} else if( SlSetBia < ( long )0xFFFF8001 ) {
		SlSetBia	= 0xFFFF8001 ;
	}

	if ( !( UcTneAxs & 0x0F ) ) {
		RamWriteA( DAXHLB, SlSetBia ) ;		
	} else {
		RamWriteA( DAYHLB, SlSetBia ) ;		
	}
	
	RamAccFixMod( OFF ) ;							

	StTneVal.UlDwdVal	= TnePtp( UcTneAxs & 0x0F , PTP_AFTER ) ;

	return( StTneVal.UlDwdVal ) ;
}



unsigned long	TneOff( UnDwdVal	StTneVal, unsigned char	UcTneAxs )
{
	long			SlSetOff ;
	unsigned short	UsSetOff ;

	UcTneAxs &= 0x0F ;
	
	RamAccFixMod( ON ) ;							
		
	if ( !UcTneAxs ) {																			
		RamReadA( DAXHLO, &UsSetOff ) ;				
		SlSetOff	= ( long )UsSetOff ;
	} else {
		RamReadA( DAYHLO, &UsSetOff ) ;				
		SlSetOff	= ( long )UsSetOff ;
	}

	if( SlSetOff > 0x00008000 ) {
		SlSetOff	|= 0xFFFF0000 ;
	}

	if ( StTneVal.StDwdVal.UsHigVal > StTneVal.StDwdVal.UsLowVal ) {
		SlSetOff	+= ( StTneVal.StDwdVal.UsHigVal - StTneVal.StDwdVal.UsLowVal ) / OFFSET_DIV ;	
	} else {
		SlSetOff	-= ( StTneVal.StDwdVal.UsLowVal - StTneVal.StDwdVal.UsHigVal ) / OFFSET_DIV ;	
	}

	if( SlSetOff > ( long )0x00007FFF ) {
		SlSetOff	= 0x00007FFF ;
	} else if( SlSetOff < ( long )0xFFFF8001 ) {
		SlSetOff	= 0xFFFF8001 ;
	}

	if ( !UcTneAxs ) {
		RamWriteA( DAXHLO, SlSetOff ) ;		
	} else {
		RamWriteA( DAYHLO, SlSetOff ) ;		
	}

	RamAccFixMod( OFF ) ;							
	
	StTneVal.UlDwdVal	= TnePtp( UcTneAxs, PTP_AFTER ) ;

	return( StTneVal.UlDwdVal ) ;
}

#endif

void	MesFil( unsigned char	UcMesMod )
{
#ifdef	USE_EXTCLK_ALL	
	if( !UcMesMod ) {								
		
		RamWrite32A( mes1aa, 0x3D1E5A40 ) ;		
		RamWrite32A( mes1ab, 0x3D1E5A40 ) ;		
		RamWrite32A( mes1ac, 0x3F6C34C0 ) ;		
		RamWrite32A( mes1ad, 0x00000000 ) ;		
		RamWrite32A( mes1ae, 0x00000000 ) ;		
		RamWrite32A( mes1ba, 0x3F800000 ) ;		
		RamWrite32A( mes1bb, 0x00000000 ) ;		
		RamWrite32A( mes1bc, 0x00000000 ) ;		
		RamWrite32A( mes1bd, 0x00000000 ) ;		
		RamWrite32A( mes1be, 0x00000000 ) ;		
		
		
		RamWrite32A( mes2aa, 0x3D1E5A40 ) ;		
		RamWrite32A( mes2ab, 0x3D1E5A40 ) ;		
		RamWrite32A( mes2ac, 0x3F6C34C0 ) ;		
		RamWrite32A( mes2ad, 0x00000000 ) ;		
		RamWrite32A( mes2ae, 0x00000000 ) ;		
		RamWrite32A( mes2ba, 0x3F800000 ) ;		
		RamWrite32A( mes2bb, 0x00000000 ) ;		
		RamWrite32A( mes2bc, 0x00000000 ) ;		
		RamWrite32A( mes2bd, 0x00000000 ) ;		
		RamWrite32A( mes2be, 0x00000000 ) ;		
		
	} else if( UcMesMod == LOOPGAIN ) {				
		
		RamWrite32A( mes1aa, 0x3E587E00 ) ;		
		RamWrite32A( mes1ab, 0x3E587E00 ) ;		
		RamWrite32A( mes1ac, 0x3F13C100 ) ;		
		RamWrite32A( mes1ad, 0x00000000 ) ;		
		RamWrite32A( mes1ae, 0x00000000 ) ;		
		RamWrite32A( mes1ba, 0x3F7DF500 ) ;		
		RamWrite32A( mes1bb, 0xBF7DF500 ) ;		
		RamWrite32A( mes1bc, 0x3F7BEA40 ) ;		
		RamWrite32A( mes1bd, 0x00000000 ) ;		
		RamWrite32A( mes1be, 0x00000000 ) ;		
		
		
		RamWrite32A( mes2aa, 0x3E587E00 ) ;		
		RamWrite32A( mes2ab, 0x3E587E00 ) ;		
		RamWrite32A( mes2ac, 0x3F13C100 ) ;		
		RamWrite32A( mes2ad, 0x00000000 ) ;		
		RamWrite32A( mes2ae, 0x00000000 ) ;		
		RamWrite32A( mes2ba, 0x3F7DF500 ) ;		
		RamWrite32A( mes2bb, 0xBF7DF500 ) ;		
		RamWrite32A( mes2bc, 0x3F7BEA40 ) ;		
		RamWrite32A( mes2bd, 0x00000000 ) ;		
		RamWrite32A( mes2be, 0x00000000 ) ;		
		
	} else if( UcMesMod == THROUGH ) {				
		
		RamWrite32A( mes1aa, 0x3F800000 ) ;		
		RamWrite32A( mes1ab, 0x00000000 ) ;		
		RamWrite32A( mes1ac, 0x00000000 ) ;		
		RamWrite32A( mes1ad, 0x00000000 ) ;		
		RamWrite32A( mes1ae, 0x00000000 ) ;		
		RamWrite32A( mes1ba, 0x3F800000 ) ;		
		RamWrite32A( mes1bb, 0x00000000 ) ;		
		RamWrite32A( mes1bc, 0x00000000 ) ;		
		RamWrite32A( mes1bd, 0x00000000 ) ;		
		RamWrite32A( mes1be, 0x00000000 ) ;		
		
		
		RamWrite32A( mes2aa, 0x3F800000 ) ;		
		RamWrite32A( mes2ab, 0x00000000 ) ;		
		RamWrite32A( mes2ac, 0x00000000 ) ;		
		RamWrite32A( mes2ad, 0x00000000 ) ;		
		RamWrite32A( mes2ae, 0x00000000 ) ;		
		RamWrite32A( mes2ba, 0x3F800000 ) ;		
		RamWrite32A( mes2bb, 0x00000000 ) ;		
		RamWrite32A( mes2bc, 0x00000000 ) ;		
		RamWrite32A( mes2bd, 0x00000000 ) ;		
		RamWrite32A( mes2be, 0x00000000 ) ;		
		
	} else if( UcMesMod == NOISE ) {				
		
		RamWrite32A( mes1aa, 0x3D1E5A40 ) ;		
		RamWrite32A( mes1ab, 0x3D1E5A40 ) ;		
		RamWrite32A( mes1ac, 0x3F6C34C0 ) ;		
		RamWrite32A( mes1ad, 0x00000000 ) ;		
		RamWrite32A( mes1ae, 0x00000000 ) ;		
		RamWrite32A( mes1ba, 0x3D1E5A40 ) ;		
		RamWrite32A( mes1bb, 0x3D1E5A40 ) ;		
		RamWrite32A( mes1bc, 0x3F6C34C0 ) ;		
		RamWrite32A( mes1bd, 0x00000000 ) ;		
		RamWrite32A( mes1be, 0x00000000 ) ;		
		
		
		RamWrite32A( mes2aa, 0x3D1E5A40 ) ;		
		RamWrite32A( mes2ab, 0x3D1E5A40 ) ;		
		RamWrite32A( mes2ac, 0x3F6C34C0 ) ;		
		RamWrite32A( mes2ad, 0x00000000 ) ;		
		RamWrite32A( mes2ae, 0x00000000 ) ;		
		RamWrite32A( mes2ba, 0x3D1E5A40 ) ;		
		RamWrite32A( mes2bb, 0x3D1E5A40 ) ;		
		RamWrite32A( mes2bc, 0x3F6C34C0 ) ;		
		RamWrite32A( mes2bd, 0x00000000 ) ;		
		RamWrite32A( mes2be, 0x00000000 ) ;		
	}
#else
	if( !UcMesMod ) {								
		
		RamWrite32A( mes1aa, 0x3CA175C0 ) ;		
		RamWrite32A( mes1ab, 0x3CA175C0 ) ;		
		RamWrite32A( mes1ac, 0x3F75E8C0 ) ;		
		RamWrite32A( mes1ad, 0x00000000 ) ;		
		RamWrite32A( mes1ae, 0x00000000 ) ;		
		RamWrite32A( mes1ba, 0x3F800000 ) ;		
		RamWrite32A( mes1bb, 0x00000000 ) ;		
		RamWrite32A( mes1bc, 0x00000000 ) ;		
		RamWrite32A( mes1bd, 0x00000000 ) ;		
		RamWrite32A( mes1be, 0x00000000 ) ;		
		
		
		RamWrite32A( mes2aa, 0x3CA175C0 ) ;		
		RamWrite32A( mes2ab, 0x3CA175C0 ) ;		
		RamWrite32A( mes2ac, 0x3F75E8C0 ) ;		
		RamWrite32A( mes2ad, 0x00000000 ) ;		
		RamWrite32A( mes2ae, 0x00000000 ) ;		
		RamWrite32A( mes2ba, 0x3F800000 ) ;		
		RamWrite32A( mes2bb, 0x00000000 ) ;		
		RamWrite32A( mes2bc, 0x00000000 ) ;		
		RamWrite32A( mes2bd, 0x00000000 ) ;		
		RamWrite32A( mes2be, 0x00000000 ) ;		
		
	} else if( UcMesMod == LOOPGAIN ) {				
		
		RamWrite32A( mes1aa, 0x3DF21080 ) ;		
		RamWrite32A( mes1ab, 0x3DF21080 ) ;		
		RamWrite32A( mes1ac, 0x3F437BC0 ) ;		
		RamWrite32A( mes1ad, 0x00000000 ) ;		
		RamWrite32A( mes1ae, 0x00000000 ) ;		
		RamWrite32A( mes1ba, 0x3F7EF980 ) ;		
		RamWrite32A( mes1bb, 0xBF7EF980 ) ;		
		RamWrite32A( mes1bc, 0x3F7DF300 ) ;		
		RamWrite32A( mes1bd, 0x00000000 ) ;		
		RamWrite32A( mes1be, 0x00000000 ) ;		
		
		
		RamWrite32A( mes2aa, 0x3DF21080 ) ;		
		RamWrite32A( mes2ab, 0x3DF21080 ) ;		
		RamWrite32A( mes2ac, 0x3F437BC0 ) ;		
		RamWrite32A( mes2ad, 0x00000000 ) ;		
		RamWrite32A( mes2ae, 0x00000000 ) ;		
		RamWrite32A( mes2ba, 0x3F7EF980 ) ;		
		RamWrite32A( mes2bb, 0xBF7EF980 ) ;		
		RamWrite32A( mes2bc, 0x3F7DF300 ) ;		
		RamWrite32A( mes2bd, 0x00000000 ) ;		
		RamWrite32A( mes2be, 0x00000000 ) ;		
		
	} else if( UcMesMod == THROUGH ) {				
		
		RamWrite32A( mes1aa, 0x3F800000 ) ;		
		RamWrite32A( mes1ab, 0x00000000 ) ;		
		RamWrite32A( mes1ac, 0x00000000 ) ;		
		RamWrite32A( mes1ad, 0x00000000 ) ;		
		RamWrite32A( mes1ae, 0x00000000 ) ;		
		RamWrite32A( mes1ba, 0x3F800000 ) ;		
		RamWrite32A( mes1bb, 0x00000000 ) ;		
		RamWrite32A( mes1bc, 0x00000000 ) ;		
		RamWrite32A( mes1bd, 0x00000000 ) ;		
		RamWrite32A( mes1be, 0x00000000 ) ;		
		
		
		RamWrite32A( mes2aa, 0x3F800000 ) ;		
		RamWrite32A( mes2ab, 0x00000000 ) ;		
		RamWrite32A( mes2ac, 0x00000000 ) ;		
		RamWrite32A( mes2ad, 0x00000000 ) ;		
		RamWrite32A( mes2ae, 0x00000000 ) ;		
		RamWrite32A( mes2ba, 0x3F800000 ) ;		
		RamWrite32A( mes2bb, 0x00000000 ) ;		
		RamWrite32A( mes2bc, 0x00000000 ) ;		
		RamWrite32A( mes2bd, 0x00000000 ) ;		
		RamWrite32A( mes2be, 0x00000000 ) ;		
		
	} else if( UcMesMod == NOISE ) {				
		
		RamWrite32A( mes1aa, 0x3CA175C0 ) ;		
		RamWrite32A( mes1ab, 0x3CA175C0 ) ;		
		RamWrite32A( mes1ac, 0x3F75E8C0 ) ;		
		RamWrite32A( mes1ad, 0x00000000 ) ;		
		RamWrite32A( mes1ae, 0x00000000 ) ;		
		RamWrite32A( mes1ba, 0x3CA175C0 ) ;		
		RamWrite32A( mes1bb, 0x3CA175C0 ) ;		
		RamWrite32A( mes1bc, 0x3F75E8C0 ) ;		
		RamWrite32A( mes1bd, 0x00000000 ) ;		
		RamWrite32A( mes1be, 0x00000000 ) ;		
		
		
		RamWrite32A( mes2aa, 0x3CA175C0 ) ;		
		RamWrite32A( mes2ab, 0x3CA175C0 ) ;		
		RamWrite32A( mes2ac, 0x3F75E8C0 ) ;		
		RamWrite32A( mes2ad, 0x00000000 ) ;		
		RamWrite32A( mes2ae, 0x00000000 ) ;		
		RamWrite32A( mes2ba, 0x3CA175C0 ) ;		
		RamWrite32A( mes2bb, 0x3CA175C0 ) ;		
		RamWrite32A( mes2bc, 0x3F75E8C0 ) ;		
		RamWrite32A( mes2bd, 0x00000000 ) ;		
		RamWrite32A( mes2be, 0x00000000 ) ;		
	}
#endif
}



void	SrvCon( unsigned char	UcDirSel, unsigned char	UcSwcCon )
{
	if( UcSwcCon ) {
		if( !UcDirSel ) {						
			RegWriteA( WH_EQSWX , 0x03 ) ;			
			RamWrite32A( sxggf, 0x00000000 ) ;		
		} else {								
			RegWriteA( WH_EQSWY , 0x03 ) ;			
			RamWrite32A( syggf, 0x00000000 ) ;		
		}
	} else {
		if( !UcDirSel ) {						
			RegWriteA( WH_EQSWX , 0x02 ) ;			
			RamWrite32A( SXLMT, 0x00000000 ) ;		
		} else {								
			RegWriteA( WH_EQSWY , 0x02 ) ;			
			RamWrite32A( SYLMT, 0x00000000 ) ;		
		}
	}
}



#ifdef	MODULE_CALIBRATION
unsigned char	LopGan( unsigned char	UcDirSel )
{
	unsigned char	UcLpAdjSts ;
	
 #ifdef	HALLADJ_HW
	UcLpAdjSts	= LoopGainAdj( UcDirSel ) ;
 #else
	MesFil( LOOPGAIN ) ;

	
	SrvCon( X_DIR, ON ) ;
	SrvCon( Y_DIR, ON ) ;

	
	WitTim( 300 ) ;

	
	LopIni( UcDirSel ) ;

	
	UcLpAdjSts	= LopAdj( UcDirSel ) ;
 #endif
	
	SrvCon( X_DIR, OFF ) ;
	SrvCon( Y_DIR, OFF ) ;

	if( !UcLpAdjSts ) {
		return( EXE_END ) ;
	} else {
		if( !UcDirSel ) {
			return( EXE_LXADJ ) ;
		} else {
			return( EXE_LYADJ ) ;
		}
	}
}



 #ifndef	HALLADJ_HW
void	LopIni( unsigned char	UcDirSel )
{
	
	LopPar( UcDirSel ) ;

	
	LopSin( UcDirSel, ON ) ;

}
 #endif


void	LopPar( unsigned char	UcDirSel )
{
	unsigned short	UsLopGan ;

	RamAccFixMod( ON ) ;							
	
	if( !UcDirSel ) {
		UsLopGan	= SXGAIN_LOP ;
		RamWriteA( sxg, UsLopGan ) ;			
	} else {
		UsLopGan	= SYGAIN_LOP ;
		RamWriteA( syg, UsLopGan ) ;			
	}

	RamAccFixMod( OFF ) ;							
}



 #ifndef	HALLADJ_HW
void	LopSin( unsigned char	UcDirSel, unsigned char	UcSonOff )
{
	unsigned short		UsFreqVal ;
	unsigned char		UcEqSwX , UcEqSwY ;
	
	RegReadA( WH_EQSWX, &UcEqSwX ) ;				
	RegReadA( WH_EQSWY, &UcEqSwY ) ;				
		
	if( UcSonOff ) {
		
  #ifdef	USE_EXTCLK_ALL	
		
   #ifdef	ACTREG_6P5OHM
		UsFreqVal	=	0x29F1 ;				
   #endif
   #ifdef	ACTREG_10P2OHM
		UsFreqVal	=	0x29F1 ;				
   #endif
   #ifdef	ACTREG_15OHM
		UsFreqVal	=	0x3B6B ;				
   #endif
  #else
		
   #ifdef	ACTREG_6P5OHM
		UsFreqVal	=	0x14F8 ;				
   #endif
   #ifdef	ACTREG_10P2OHM
		UsFreqVal	=	0x14F8 ;				
   #endif
   #ifdef	ACTREG_15OHM
		UsFreqVal	=	0x1DB5 ;				
   #endif
  #endif
		
		RegWriteA( WC_SINFRQ0,	(unsigned char)UsFreqVal ) ;				
		RegWriteA( WC_SINFRQ1,	(unsigned char)(UsFreqVal >> 8) ) ;			
		
		if( !UcDirSel ) {

			UcEqSwX |= 0x10 ;
			UcEqSwY &= ~EQSINSW ;
			
			RamWrite32A( sxsin, 0x3CA3D70A ) ;				
		} else {

			UcEqSwX &= ~EQSINSW ;
			UcEqSwY |= 0x10 ;
			
			RamWrite32A( sysin, 0x3CA3D70A ) ;				
		}
		RegWriteA( WC_SINPHSX, 0x00 ) ;					
		RegWriteA( WC_SINPHSY, 0x00 ) ;					
		RegWriteA( WH_EQSWX, UcEqSwX ) ;				
		RegWriteA( WH_EQSWY, UcEqSwY ) ;				
		RegWriteA( WC_SINON,     0x01 ) ;				
	} else {
		UcEqSwX &= ~EQSINSW ;
		UcEqSwY &= ~EQSINSW ;
		RegWriteA( WC_SINON,     0x00 ) ;				
		if( !UcDirSel ) {
			RamWrite32A( sxsin, 0x00000000 ) ;			
		} else {
			RamWrite32A( sysin, 0x00000000 ) ;			
		}
		RegWriteA( WH_EQSWX, UcEqSwX ) ;				
		RegWriteA( WH_EQSWY, UcEqSwY ) ;				
	}
}



unsigned char	LopAdj( unsigned char	UcDirSel )
{
	unsigned char	UcAdjSts	= FAILURE ;
	unsigned short	UsRtnVal ;
	float			SfCmpVal ;
	unsigned char	UcIdxCnt ;
	unsigned char	UcIdxCn1 ;
	unsigned char	UcIdxCn2 ;
	UnFltVal		UnAdcXg1, UnAdcXg2 , UnRtnVa ;

	float			DfGanVal[ 5 ] ;
	float			DfTemVal ;

	if( !UcDirSel ) {
		RegWriteA( WC_MES1ADD0, (unsigned char)SXGZ ) ;							
		RegWriteA( WC_MES1ADD1, (unsigned char)(( SXGZ >> 8 ) & 0x0001 ) ) ;	
		RegWriteA( WC_MES2ADD0, (unsigned char)SXG3Z ) ;						
		RegWriteA( WC_MES2ADD1, (unsigned char)(( SXG3Z >> 8 ) & 0x0001 ) ) ;	
	} else {
		RegWriteA( WC_MES1ADD0, (unsigned char)SYGZ ) ;							
		RegWriteA( WC_MES1ADD1, (unsigned char)(( SYGZ >> 8 ) & 0x0001 ) ) ;	
		RegWriteA( WC_MES2ADD0, (unsigned char)SYG3Z ) ;						
		RegWriteA( WC_MES2ADD1, (unsigned char)(( SYG3Z >> 8 ) & 0x0001 ) ) ;	
	}
	
	
	for( UcIdxCnt = 0 ; UcIdxCnt < 5 ; UcIdxCnt++ )
	{
		LopMes( ) ;																

		UnAdcXg1.UlLngVal	= GinMes( MES_XG1 ) ;										
		UnAdcXg2.UlLngVal	= GinMes( MES_XG2 ) ;										

		SfCmpVal	= UnAdcXg2.SfFltVal / UnAdcXg1.SfFltVal ;					

		if( !UcDirSel ) {
			RamRead32A( sxg, &UnRtnVa.UlLngVal ) ;									
		} else {
			RamRead32A( syg, &UnRtnVa.UlLngVal ) ;									
		}
		UnRtnVa.SfFltVal	=  UnRtnVa.SfFltVal * SfCmpVal ;

		DfGanVal[ UcIdxCnt ]	= UnRtnVa.SfFltVal ;
	}

	for( UcIdxCn1 = 0 ; UcIdxCn1 < 4 ; UcIdxCn1++ )
	{
		for( UcIdxCn2 = UcIdxCn1+1 ; UcIdxCn2 < 5 ; UcIdxCn2++ )
		{
			if( DfGanVal[ UcIdxCn1 ] > DfGanVal[ UcIdxCn2 ] ) {
				DfTemVal				= DfGanVal[ UcIdxCn1 ] ;
				DfGanVal[ UcIdxCn1 ]	= DfGanVal[ UcIdxCn2 ] ;
				DfGanVal[ UcIdxCn2 ]	= DfTemVal ;
			}
		}
	}

	UnRtnVa.SfFltVal	=  ( DfGanVal[ 1 ] + DfGanVal[ 2 ] + DfGanVal[ 3 ] ) / 3  ;

	LopSin( UcDirSel, OFF ) ;

	if( UnRtnVa.UlLngVal < 0x3F800000 ) {									
		UcAdjSts	= SUCCESS ;												
	}

	if( UcAdjSts ) {
		if( !UcDirSel ) {
			RamWrite32A( sxg, 0x3F800000 ) ;						
			StAdjPar.StLopGan.UsLxgVal	= 0x7FFF ;
			StAdjPar.StLopGan.UsLxgSts	= 0x0000 ;
		} else {
			RamWrite32A( syg, 0x3F800000 ) ;						
			StAdjPar.StLopGan.UsLygVal	= 0x7FFF ;
			StAdjPar.StLopGan.UsLygSts	= 0x0000 ;
		}
	} else {
		if( !UcDirSel ) {
			RamWrite32A( sxg, UnRtnVa.UlLngVal ) ;						
			RamAccFixMod( ON ) ;								
			RamReadA( sxg, &UsRtnVal ) ;						
			StAdjPar.StLopGan.UsLxgVal	= UsRtnVal ;
			StAdjPar.StLopGan.UsLxgSts	= 0xFFFF ;
		} else {
			RamWrite32A( syg, UnRtnVa.UlLngVal ) ;						
			RamAccFixMod( ON ) ;								
			RamReadA( syg, &UsRtnVal ) ;						
			StAdjPar.StLopGan.UsLygVal	= UsRtnVal ;
			StAdjPar.StLopGan.UsLygSts	= 0xFFFF ;
		}
		RamAccFixMod( OFF ) ;							
	}
	return( UcAdjSts ) ;
}


void	LopMes( void )
{
	ClrGyr( 0x1000 , CLR_FRAM1 );					
	RamWrite32A( MSABS1AV, 0x00000000 ) ;			
	RamWrite32A( MSABS2AV, 0x00000000 ) ;			
	RegWriteA( WC_MESLOOP1, 0x04 ) ;				
	RegWriteA( WC_MESLOOP0, 0x00 ) ;				
	RamWrite32A( msmean	, 0x3A800000 );				
	RegWriteA( WC_MESABS, 0x01 ) ;					
	RegWriteA( WC_MESWAIT,     0x00 ) ;				
	BsyWit( WC_MESMODE, 0x01 ) ;					
}


unsigned long	GinMes( unsigned char	UcXg1Xg2 )
{
	unsigned long	UlMesVal ;

	if( !UcXg1Xg2 ) {
		RamRead32A( MSABS1AV, &UlMesVal ) ;			
	} else {
		RamRead32A( MSABS2AV, &UlMesVal ) ;			
	}
	
	return( UlMesVal ) ;
}

 #endif
#endif

#define	LIMITH		0x0FA0
#define	LIMITL		0xF060
#define	INITVAL		0x0000
unsigned char	TneGvc( void )
{
	unsigned char  UcRsltSts;
	
	
	
	RegWriteA( IZAH,	(unsigned char)(INITVAL >> 8) ) ;	
	RegWriteA( IZAL,	(unsigned char)INITVAL ) ;			
	RegWriteA( IZBH,	(unsigned char)(INITVAL >> 8) ) ;	
	RegWriteA( IZBL,	(unsigned char)INITVAL ) ;			
	
	MesFil( THROUGH ) ;				
	
	
	
	RegWriteA( WC_MES1ADD0, 0x00 ) ;		
	RegWriteA( WC_MES1ADD1, 0x00 ) ;		
	ClrGyr( 0x1000 , CLR_FRAM1 );					
	StAdjPar.StGvcOff.UsGxoVal = (unsigned short)GenMes( AD2Z, 0 );		
	RegWriteA( IZAH, (unsigned char)(StAdjPar.StGvcOff.UsGxoVal >> 8) ) ;	
	RegWriteA( IZAL, (unsigned char)(StAdjPar.StGvcOff.UsGxoVal) ) ;		
	
	
	
	RegWriteA( WC_MES1ADD0, 0x00 ) ;		
	RegWriteA( WC_MES1ADD1, 0x00 ) ;		
	ClrGyr( 0x1000 , CLR_FRAM1 );					
	StAdjPar.StGvcOff.UsGyoVal = (unsigned short)GenMes( AD3Z, 0 );		
	RegWriteA( IZBH, (unsigned char)(StAdjPar.StGvcOff.UsGyoVal >> 8) ) ;	
	RegWriteA( IZBL, (unsigned char)(StAdjPar.StGvcOff.UsGyoVal) ) ;		
	
	UcRsltSts = EXE_END ;						

	StAdjPar.StGvcOff.UsGxoSts	= 0xFFFF ;
	if(( (short)StAdjPar.StGvcOff.UsGxoVal < (short)LIMITL ) || ( (short)StAdjPar.StGvcOff.UsGxoVal > (short)LIMITH ))
	{
		UcRsltSts |= EXE_GXADJ ;
		StAdjPar.StGvcOff.UsGxoSts	= 0x0000 ;
	}
	
	StAdjPar.StGvcOff.UsGyoSts	= 0xFFFF ;
	if(( (short)StAdjPar.StGvcOff.UsGyoVal < (short)LIMITL ) || ( (short)StAdjPar.StGvcOff.UsGyoVal > (short)LIMITH ))
	{
		UcRsltSts |= EXE_GYADJ ;
		StAdjPar.StGvcOff.UsGyoSts	= 0x0000 ;
	}
	return( UcRsltSts );
		
}



unsigned char	RtnCen( unsigned char	UcCmdPar )
{
	unsigned char	UcCmdSts ;

	UcCmdSts	= EXE_END ;

	GyrCon( OFF ) ;											

	if( !UcCmdPar ) {										

		StbOnn() ;											
		
	} else if( UcCmdPar == 0x01 ) {							

		SrvCon( X_DIR, ON ) ;								
		SrvCon( Y_DIR, OFF ) ;
	} else if( UcCmdPar == 0x02 ) {							

		SrvCon( X_DIR, OFF ) ;								
		SrvCon( Y_DIR, ON ) ;
	}

	return( UcCmdSts ) ;
}



void	GyrCon( unsigned char	UcGyrCon )
{
	
	RegWriteA( WG_SHTON, 0x00 ) ;									
	
	if( UcGyrCon == ON ) {												

		
#ifdef	GAIN_CONT
		
#endif
		ClrGyr( 0x000E , CLR_FRAM1 );		

		RamWrite32A( sxggf, 0x3F800000 ) ;	
		RamWrite32A( syggf, 0x3F800000 ) ;	
		
	} else if( UcGyrCon == SPC ) {										

		
#ifdef	GAIN_CONT
		
#endif

		RamWrite32A( sxggf, 0x3F800000 ) ;	
		RamWrite32A( syggf, 0x3F800000 ) ;	
		

	} else {															
		
		RamWrite32A( sxggf, 0x00000000 ) ;	
		RamWrite32A( syggf, 0x00000000 ) ;	
		

#ifdef	GAIN_CONT
		
#endif
	}
}



void	OisEna( void )
{
	
	SrvCon( X_DIR, ON ) ;
	SrvCon( Y_DIR, ON ) ;

	GyrCon( ON ) ;
}

void	OisEnaLin( void )
{
	
	SrvCon( X_DIR, ON ) ;
	SrvCon( Y_DIR, ON ) ;

	GyrCon( SPC ) ;
}



void	TimPro( void )
{
#ifdef	MODULE_CALIBRATION
	if( UcOscAdjFlg )
	{
		if( UcOscAdjFlg == MEASSTR )
		{
			RegWriteA( OSCCNTEN, 0x01 ) ;		
			UcOscAdjFlg = MEASCNT ;
		}
		else if( UcOscAdjFlg == MEASCNT )
		{
			RegWriteA( OSCCNTEN, 0x00 ) ;		
			UcOscAdjFlg = MEASFIX ;
		}
	}
#endif
}



void	S2cPro( unsigned char uc_mode )
{
	if( uc_mode == 1 )
	{
#ifdef H1COEF_CHANGER
		SetH1cMod( S2MODE ) ;							
#endif
		
		RegWriteA( WG_SHTON, 0x11 ) ;							
		RamWrite32A( gxh1c, DIFIL_S2 );							
		RamWrite32A( gyh1c, DIFIL_S2 );							
	}
	else
	{
		RamWrite32A( gxh1c, UlH1Coefval );							
		RamWrite32A( gyh1c, UlH1Coefval );							
		
		RegWriteA( WG_SHTON, 0x00 ) ;							

#ifdef H1COEF_CHANGER
		SetH1cMod( UcH1LvlMod ) ;							
#endif
	}
	
}


short	GenMes( unsigned short	UsRamAdd, unsigned char	UcMesMod )
{
	short	SsMesRlt ;

	RegWriteA( WC_MES1ADD0, (unsigned char)UsRamAdd ) ;							
	RegWriteA( WC_MES1ADD1, (unsigned char)(( UsRamAdd >> 8 ) & 0x0001 ) ) ;	
	RamWrite32A( MSABS1AV, 0x00000000 ) ;				
	
	if( !UcMesMod ) {
		RegWriteA( WC_MESLOOP1, 0x04 ) ;				
		RegWriteA( WC_MESLOOP0, 0x00 ) ;				
		RamWrite32A( msmean	, 0x3A7FFFF7 );				
	} else {
		RegWriteA( WC_MESLOOP1, 0x01 ) ;				
		RegWriteA( WC_MESLOOP0, 0x00 ) ;				
		RamWrite32A( msmean	, 0x3F800000 );				
	}

	RegWriteA( WC_MESABS, 0x00 ) ;						
	BsyWit( WC_MESMODE, 0x01 ) ;						

	RamAccFixMod( ON ) ;							
	
	RamReadA( MSABS1AV, ( unsigned short * )&SsMesRlt ) ;	

	RamAccFixMod( OFF ) ;							
	
	return( SsMesRlt ) ;
}


#ifdef	USE_EXTCLK_ALL	
	
	
	
	
const unsigned short	CucFreqVal[ 17 ]	= {
		0xFFFF,				
		0x0059,				
		0x00B2,				
		0x010C,				
		0x0165,				
		0x01BF,				
		0x0218,				
		0x0272,				
		0x02CB,				
		0x0325,				
		0x037E,				
		0x03D8,				
		0x0431,				
		0x048B,				
		0x04E4,				
		0x053E,				
		0x0597				
	} ;
#else
	
	
	
	
const unsigned short	CucFreqVal[ 17 ]	= {
		0xFFFF,				
		0x002C,				
		0x0059,				
		0x0086,				
		0x00B2,				
		0x00DF,				
		0x010C,				
		0x0139,				
		0x0165,				
		0x0192,				
		0x01BF,				
		0x01EC,				
		0x0218,				
		0x0245,				
		0x0272,				
		0x029F,				
		0x02CB				
	} ;
#endif
	
#define		USE_SINLPF			
	
void	SetSinWavePara( unsigned char UcTableVal ,  unsigned char UcMethodVal )
{
	unsigned short	UsFreqDat ;
	unsigned char	UcEqSwX , UcEqSwY ;

	
	if(UcTableVal > 0x10 )
		UcTableVal = 0x10 ;			
	UsFreqDat = CucFreqVal[ UcTableVal ] ;	
	
	if( UcMethodVal == SINEWAVE) {
		RegWriteA( WC_SINPHSX, 0x00 ) ;					
		RegWriteA( WC_SINPHSY, 0x00 ) ;					
	}else if( UcMethodVal == CIRCWAVE ){
		RegWriteA( WC_SINPHSX,	0x00 ) ;				
		RegWriteA( WC_SINPHSY,	0x20 ) ;				
	}else{
		RegWriteA( WC_SINPHSX, 0x00 ) ;					
		RegWriteA( WC_SINPHSY, 0x00 ) ;					
	}

#ifdef	USE_SINLPF
	if(( UcMethodVal != XHALWAVE ) && ( UcMethodVal != YHALWAVE )) {
		MesFil( NOISE ) ;			
	}
#endif

	if( UsFreqDat == 0xFFFF )			
	{

		RegReadA( WH_EQSWX, &UcEqSwX ) ;				
		RegReadA( WH_EQSWY, &UcEqSwY ) ;				
		UcEqSwX &= ~EQSINSW ;
		UcEqSwY &= ~EQSINSW ;
		RegWriteA( WH_EQSWX, UcEqSwX ) ;				
		RegWriteA( WH_EQSWY, UcEqSwY ) ;				
		
#ifdef	USE_SINLPF
		if(( UcMethodVal != XHALWAVE ) && ( UcMethodVal != YHALWAVE )) {
			RegWriteA( WC_DPON,     0x00 ) ;			
			RegWriteA( WC_DPO1ADD0, 0x00 ) ;			
			RegWriteA( WC_DPO1ADD1, 0x00 ) ;			
			RegWriteA( WC_DPO2ADD0, 0x00 ) ;			
			RegWriteA( WC_DPO2ADD1, 0x00 ) ;			
			RegWriteA( WC_DPI1ADD0, 0x00 ) ;			
			RegWriteA( WC_DPI1ADD1, 0x00 ) ;			
			RegWriteA( WC_DPI2ADD0, 0x00 ) ;			
			RegWriteA( WC_DPI2ADD1, 0x00 ) ;			
			
			
			RamAccFixMod( ON ) ;							
			
			RamWriteA( SXOFFZ1, UsCntXof ) ;			
			RamWriteA( SYOFFZ1, UsCntYof ) ;			
			
			
			RamAccFixMod( OFF ) ;							
	
			RegWriteA( WC_MES1ADD0,  0x00 ) ;			
			RegWriteA( WC_MES1ADD1,  0x00 ) ;			
			RegWriteA( WC_MES2ADD0,  0x00 ) ;			
			RegWriteA( WC_MES2ADD1,  0x00 ) ;			
			
		}
#endif
		RegWriteA( WC_SINON,     0x00 ) ;			
		
	}
	else
	{
		
		RegReadA( WH_EQSWX, &UcEqSwX ) ;				
		RegReadA( WH_EQSWY, &UcEqSwY ) ;				
		
		if(( UcMethodVal != XHALWAVE ) && ( UcMethodVal != YHALWAVE )) {
#ifdef	USE_SINLPF
			RegWriteA( WC_DPI1ADD0,  ( unsigned char )MES1BZ2 ) ;						
			RegWriteA( WC_DPI1ADD1,  ( unsigned char )(( MES1BZ2 >> 8 ) & 0x0001 ) ) ;	
			RegWriteA( WC_DPI2ADD0,  ( unsigned char )MES2BZ2 ) ;						
			RegWriteA( WC_DPI2ADD1,  ( unsigned char )(( MES2BZ2 >> 8 ) & 0x0001 ) ) ;	
			RegWriteA( WC_DPO1ADD0, ( unsigned char )SXOFFZ1 ) ;						
			RegWriteA( WC_DPO1ADD1, ( unsigned char )(( SXOFFZ1 >> 8 ) & 0x0001 ) ) ;	
			RegWriteA( WC_DPO2ADD0, ( unsigned char )SYOFFZ1 ) ;						
			RegWriteA( WC_DPO2ADD1, ( unsigned char )(( SYOFFZ1 >> 8 ) & 0x0001 ) ) ;	
			
			RegWriteA( WC_MES1ADD0,  ( unsigned char )SINXZ ) ;							
			RegWriteA( WC_MES1ADD1,  ( unsigned char )(( SINXZ >> 8 ) & 0x0001 ) ) ;	
			RegWriteA( WC_MES2ADD0,  ( unsigned char )SINYZ ) ;							
			RegWriteA( WC_MES2ADD1,  ( unsigned char )(( SINYZ >> 8 ) & 0x0001 ) ) ;	
			
			RegWriteA( WC_DPON,     0x03 ) ;			
			
			UcEqSwX &= ~EQSINSW ;
			UcEqSwY &= ~EQSINSW ;
#else
			UcEqSwX |= 0x08 ;
			UcEqSwY |= 0x08 ;
#endif

		}else{
			if( UcMethodVal == XHALWAVE ){
		    	UcEqSwX = 0x22 ;				
			}else{
				UcEqSwY = 0x22 ;				
			}
		}
		
		RegWriteA( WC_SINFRQ0,	(unsigned char)UsFreqDat ) ;				
		RegWriteA( WC_SINFRQ1,	(unsigned char)(UsFreqDat >> 8) ) ;			
		RegWriteA( WC_MESSINMODE,     0x00 ) ;			

		RegWriteA( WH_EQSWX, UcEqSwX ) ;				
		RegWriteA( WH_EQSWY, UcEqSwY ) ;				

		RegWriteA( WC_SINON,     0x01 ) ;			
		
	}
	
	
}




#ifdef STANDBY_MODE
void	SetStandby( unsigned char UcContMode )
{
	unsigned char	UcStbb0 , UcClkon ;
	
	switch(UcContMode)
	{
	case STB1_ON:

#ifdef	AF_PWMMODE
#else
		RegWriteA( DRVFCAF	, 0x00 );				
#endif
		RegWriteA( STBB0 	, 0x00 );		
		RegWriteA( STBB1 	, 0x00 );		
		RegWriteA( PWMA 	, 0x00 );		
		RegWriteA( PWMAAF 	, 0x00 );		
		RegWriteA( CVA,  0x00 ) ;			
		DrvSw( OFF ) ;						
		AfDrvSw( OFF ) ;					
#ifdef	MONITOR_OFF
#else
		RegWriteA( PWMMONA, 0x00 ) ;		
#endif
		SelectGySleep( ON ) ;				
		break ;
	case STB1_OFF:
		SelectGySleep( OFF ) ;				
		RegWriteA( PWMMONA, 0x80 ) ;		
		DrvSw( ON ) ;						
		AfDrvSw( ON ) ;						
		RegWriteA( CVA		, 0xC0 );		
		RegWriteA( PWMAAF 	, 0x00 );		
		RegWriteA( PWMA		, 0xC0 );		
		RegWriteA( STBB1	, 0x05 ) ;		
		RegWriteA( STBB0	, 0xDF );		
		break ;
	case STB2_ON:
#ifdef	AF_PWMMODE
#else
		RegWriteA( DRVFCAF	, 0x00 );				
#endif
		RegWriteA( STBB0 	, 0x00 );		
		RegWriteA( STBB1 	, 0x00 );		
		RegWriteA( PWMA 	, 0x00 );		
		RegWriteA( PWMAAF 	, 0x00 );		
		RegWriteA( CVA,  0x00 ) ;			
		DrvSw( OFF ) ;						
		AfDrvSw( OFF ) ;					
#ifdef	MONITOR_OFF
#else
		RegWriteA( PWMMONA, 0x00 ) ;		
#endif
		SelectGySleep( ON ) ;				
		RegWriteA( CLKON, 0x00 ) ;			
		break ;
	case STB2_OFF:
		RegWriteA( CLKON,	0x1F ) ;		
		SelectGySleep( OFF ) ;				
		RegWriteA( PWMMONA, 0x80 ) ;		
		DrvSw( ON ) ;						
		AfDrvSw( ON ) ;						
		RegWriteA( CVA, 	0xC0 );			
		RegWriteA( PWMAAF 	, 0x00 );		
		RegWriteA( PWMA	, 	0xC0 );			
		RegWriteA( STBB1	, 0x05 ) ;		
		RegWriteA( STBB0	, 0xDF );		
		break ;
	case STB3_ON:
#ifdef	AF_PWMMODE
#else
		RegWriteA( DRVFCAF	, 0x00 );				
#endif
		RegWriteA( STBB0 	, 0x00 );		
		RegWriteA( STBB1 	, 0x00 );		
		RegWriteA( PWMA 	, 0x00 );			
		RegWriteA( PWMAAF 	, 0x00 );		
		RegWriteA( CVA,  0x00 ) ;			
		DrvSw( OFF ) ;						
		AfDrvSw( OFF ) ;					
#ifdef	MONITOR_OFF
#else
		RegWriteA( PWMMONA, 0x00 ) ;		
#endif
		SelectGySleep( ON ) ;				
		RegWriteA( CLKON, 0x00 ) ;			
		RegWriteA( I2CSEL, 0x01 ) ;			
		RegWriteA( OSCSTOP, 0x02 ) ;		
		break ;
	case STB3_OFF:
		RegWriteA( OSCSTOP, 0x00 ) ;		
		RegWriteA( I2CSEL, 0x00 ) ;			
		RegWriteA( CLKON,	0x1F ) ;		
		SelectGySleep( OFF ) ;				
		RegWriteA( PWMMONA, 0x80 ) ;		
		DrvSw( ON ) ;						
		AfDrvSw( ON ) ;						
		RegWriteA( CVA, 	0xC0 );			
		RegWriteA( PWMAAF,	0x00 );			
		RegWriteA( PWMA	, 	0xC0 );			
		RegWriteA( STBB1	, 0x05 ) ;		
		RegWriteA( STBB0	, 0xDF );		
		break ;
		
	case STB4_ON:
#ifdef	AF_PWMMODE
#else
		RegWriteA( DRVFCAF	, 0x00 );				
#endif
		RegWriteA( STBB0 	, 0x00 );		
		RegWriteA( STBB1 	, 0x00 );		
		RegWriteA( PWMA 	, 0x00 );		
		RegWriteA( PWMAAF 	, 0x00 );		
		RegWriteA( CVA,  	0x00 ) ;		
		DrvSw( OFF ) ;						
		AfDrvSw( OFF ) ;					
#ifdef	MONITOR_OFF
#else
		RegWriteA( PWMMONA, 0x00 ) ;		
#endif
		GyOutSignalCont( ) ;				
		RegWriteA( CLKON, 0x04 ) ;			
		break ;
	case STB4_OFF:
		RegWriteA( CLKON,	0x1F ) ;		
		SelectGySleep( OFF ) ;				
		RegWriteA( PWMMONA, 0x80 ) ;		
		DrvSw( ON ) ;						
		AfDrvSw( ON ) ;						
		RegWriteA( CVA, 	0xC0 );			
		RegWriteA( PWMAAF, 	0x00 );			
		RegWriteA( PWMA	, 	0xC0 );			
		RegWriteA( STBB1	, 0x05 ) ;		
		RegWriteA( STBB0	, 0xDF );		
		break ;
		
		
	case STB2_OISON:
		RegReadA( STBB0 	, &UcStbb0 );		
		UcStbb0 &= 0x80 ;
		RegWriteA( STBB0 	, UcStbb0 );		
		RegWriteA( PWMA 	, 0x00 );		
		RegWriteA( CVA,  0x00 ) ;			
		DrvSw( OFF ) ;						
#ifdef	MONITOR_OFF
#else
		RegWriteA( PWMMONA, 0x00 ) ;		
#endif
		SelectGySleep( ON ) ;				
		RegReadA( CLKON, &UcClkon ) ;		
		UcClkon &= 0x1A ;
		RegWriteA( CLKON, UcClkon ) ;		
		break ;
	case STB2_OISOFF:
		RegReadA( CLKON, &UcClkon ) ;		
		UcClkon |= 0x05 ;
		RegWriteA( CLKON,	UcClkon ) ;		
		SelectGySleep( OFF ) ;				
		RegWriteA( PWMMONA, 0x80 ) ;		
		DrvSw( ON ) ;						
		RegWriteA( CVA, 	0xC0 );			
		RegWriteA( PWMA	, 	0xC0 );			
		RegReadA( STBB0	, &UcStbb0 );		
		UcStbb0 |= 0x5F ;
		RegWriteA( STBB0	, UcStbb0 );	
		break ;
		
	case STB2_AFON:
#ifdef	AF_PWMMODE
#else
		RegWriteA( DRVFCAF	, 0x00 );				
#endif
		RegReadA( STBB0 	, &UcStbb0 );		
		UcStbb0 &= 0x7F ;
		RegWriteA( STBB0 	, UcStbb0 );		
		RegWriteA( STBB1 	, 0x00 );		
		RegWriteA( PWMAAF 	, 0x00 );		
		AfDrvSw( OFF ) ;					
#ifdef	MONITOR_OFF
#else
		RegWriteA( PWMMONA, 0x00 ) ;		
#endif
		RegReadA( CLKON, &UcClkon ) ;		
		UcClkon &= 0x07 ;
		RegWriteA( CLKON, UcClkon ) ;		
		break ;
	case STB2_AFOFF:
		RegReadA( CLKON, &UcClkon ) ;		
		UcClkon |= 0x18 ;
		RegWriteA( CLKON,	UcClkon ) ;		
		AfDrvSw( ON ) ;						
		RegWriteA( PWMAAF 	, 0x00 );		
		RegWriteA( STBB1	, 0x05 ) ;		
		RegReadA( STBB0	, &UcStbb0 );		
		UcStbb0 |= 0x80 ;
		RegWriteA( STBB0	, UcStbb0 );	
		break ;
		
	}
}
#endif

void	SetZsp( unsigned char	UcZoomStepDat )
{
	unsigned long	UlGyrZmx, UlGyrZmy, UlGyrZrx, UlGyrZry ;

	
	
	if(UcZoomStepDat > (ZOOMTBL - 1))
		UcZoomStepDat = (ZOOMTBL -1) ;										

	if( UcZoomStepDat == 0 )				
	{
		UlGyrZmx	= ClGyxZom[ 0 ] ;		
		UlGyrZmy	= ClGyyZom[ 0 ] ;		
		
	}
	else
	{
		UlGyrZmx	= ClGyxZom[ UcZoomStepDat ] ;
		UlGyrZmy	= ClGyyZom[ UcZoomStepDat ] ;
		
		
	}
	
	
	RamWrite32A( gxlens, UlGyrZmx ) ;		
	RamWrite32A( gylens, UlGyrZmy ) ;		

	RamRead32A( gxlens, &UlGyrZrx ) ;		
	RamRead32A( gylens, &UlGyrZry ) ;		

	
	if( UlGyrZmx != UlGyrZrx ) {
		RamWrite32A( gxlens, UlGyrZmx ) ;		
	}

	if( UlGyrZmy != UlGyrZry ) {
		RamWrite32A( gylens, UlGyrZmy ) ;		
	}

}

 
void StbOnn( void )
{
	unsigned char	UcRegValx,UcRegValy;					
	unsigned char	UcRegIni ;
	
	RegReadA( WH_EQSWX , &UcRegValx ) ;			
	RegReadA( WH_EQSWY , &UcRegValy ) ;			
	
	if( (( UcRegValx & 0x01 ) != 0x01 ) && (( UcRegValy & 0x01 ) != 0x01 ))
	{
		
		RegWriteA( WH_SMTSRVON,	0x01 ) ;				
		
		SrvCon( X_DIR, ON ) ;
		SrvCon( Y_DIR, ON ) ;
		
		UcRegIni = 0x11;
		while( (UcRegIni & 0x77) != 0x66 )
		{
			RegReadA( RH_SMTSRVSTT,	&UcRegIni ) ;		
		}
		RegWriteA( WH_SMTSRVON,	0x00 ) ;				
		
	}
	else
	{
		SrvCon( X_DIR, ON ) ;
		SrvCon( Y_DIR, ON ) ;
	}
}

 
void StbOnnN( unsigned char UcStbY , unsigned char UcStbX )
{
	unsigned char	UcRegIni ;
	unsigned char	UcSttMsk = 0 ;
	
	
	RegWriteA( WH_SMTSRVON,	0x01 ) ;				
	if( UcStbX == ON )	UcSttMsk |= 0x07 ;
	if( UcStbY == ON )	UcSttMsk |= 0x70 ;
	
	SrvCon( X_DIR, UcStbX ) ;
	SrvCon( Y_DIR, UcStbY ) ;
	
	UcRegIni = 0x11;
	while( (UcRegIni & UcSttMsk) != ( 0x66 & UcSttMsk ) )
	{
		RegReadA( RH_SMTSRVSTT,	&UcRegIni ) ;		
	}
	RegWriteA( WH_SMTSRVON,	0x00 ) ;				
		
}

void	OptCen( unsigned char UcOptmode , unsigned short UsOptXval , unsigned short UsOptYval )
{
	RamAccFixMod( ON ) ;							
	
	switch ( UcOptmode ) {
		case VAL_SET :
			RamWriteA( SXOFFZ1   , UsOptXval ) ;		
			RamWriteA( SYOFFZ1   , UsOptYval ) ;		
			break ;
		case VAL_FIX :
			UsCntXof = UsOptXval ;
			UsCntYof = UsOptYval ;
			RamWriteA( SXOFFZ1   , UsCntXof ) ;		
			RamWriteA( SYOFFZ1   , UsCntYof ) ;		

			break ;
		case VAL_SPC :
			RamReadA( SXOFFZ1   , &UsOptXval ) ;		
			RamReadA( SYOFFZ1   , &UsOptYval ) ;		
			UsCntXof = UsOptXval ;
			UsCntYof = UsOptYval ;


			break ;
	}

	RamAccFixMod( OFF ) ;							
	
}

#ifdef	MODULE_CALIBRATION
#define	RRATETABLE	8
#define	CRATETABLE	16
const signed char	ScRselRate[ RRATETABLE ]	= {
		-12,			
		 -9,			
		 -6,			
		 -3,			
		  0,			
		  3,			
		  7,			
		 11				
	} ;
const signed char	ScCselRate[ CRATETABLE ]	= {
		-14,			
		-12,			
		-10,			
		 -8,			
		 -6,			
		 -4,			
		 -2,			
		  0,			
		  0,			
		  2,			
		  4,			
		  6,			
		  8,			
		 10,			
		 12,			
		 14				
	} ;
	

#define	TARGET_FREQ		48000.0F
#define	START_RSEL		0x04	
#define	START_CSEL		0x08	
#define	MEAS_MAX		32		
#define	UNDR_MEAS		0x00
#define	FIX_MEAS		0x01
#define	JST_FIX			0x03
#define	OVR_MEAS		0x80
#define	RSELFX			0x08
#define	RSEL1ST			0x01
#define	RSEL2ND			0x02
#define	CSELFX			0x80
#define	CSELPLS			0x10
#define	CSELMNS			0x20

unsigned short	OscAdj( void )
{
	unsigned char	UcMeasFlg ;									
	UnWrdVal		StClkVal ;									
	unsigned char	UcMeasCnt ;									
	unsigned char	UcOscrsel , UcOsccsel ;						
	unsigned char	UcSrvDivBk ;								
	unsigned char	UcClkJdg ;									
	float			FcalA,FcalB ;								
	signed char		ScTblRate_Val, ScTblRate_Now, ScTblRate_Tgt ;	
	float			FlRatePbk,FlRateMbk ;							
	unsigned char	UcOsccselP , UcOsccselM ;					
	unsigned short	UsResult ;

	
	UcMeasFlg = 0 ;						
	UcMeasCnt = 0;						
	UcClkJdg = UNDR_MEAS;				
	UcOscrsel = START_RSEL ;
	UcOsccsel = START_CSEL ;
	
	
	
	RegReadA( SRVDIV, &UcSrvDivBk ) ;	
	RegWriteA( SRVDIV,	0x00 ) ;		
	RegWriteA( OSCSET, ( UcOscrsel << 5 ) | (UcOsccsel << 1 ) ) ;	
	
	while( UcClkJdg == UNDR_MEAS )
	{
		UcMeasCnt++ ;						
		UcOscAdjFlg = MEASSTR ;				
		
		while( UcOscAdjFlg != MEASFIX )
		{
			;
		}
		
		UcOscAdjFlg = 0x00 ;				
		RegReadA( OSCCK_CNTR0, &StClkVal.StWrdVal.UcLowVal ) ;		
		RegReadA( OSCCK_CNTR1, &StClkVal.StWrdVal.UcHigVal ) ;		
		
		FcalA = (float)StClkVal.UsWrdVal ;
		FcalB = TARGET_FREQ / FcalA ;
		FcalB =  FcalB - 1.0F ;
		FcalB *= 100.0F ;
		
		if( FcalB == 0.0F )
		{
			UcClkJdg = JST_FIX ;					
			UcMeasFlg |= ( CSELFX | RSELFX ) ;		
			break ;
		}

		
		if( !(UcMeasFlg & RSELFX) )
		{
			if(UcMeasFlg & RSEL1ST)
			{
				UcMeasFlg |= ( RSELFX | RSEL2ND ) ;
			}
			else
			{
				UcMeasFlg |= RSEL1ST ;
			}
			ScTblRate_Now = ScRselRate[ UcOscrsel ] ;					
			ScTblRate_Tgt = ScTblRate_Now + (short)FcalB ;
			if( ScTblRate_Now > ScTblRate_Tgt )
			{
				while(1)
				{
					if( UcOscrsel == 0 )
					{
						break;
					}
					UcOscrsel -= 1 ;
					ScTblRate_Val = ScRselRate[ UcOscrsel ] ;	
					if( ScTblRate_Tgt >= ScTblRate_Val )
					{
						break;
					}
				}
			}
			else if( ScTblRate_Now < ScTblRate_Tgt )
			{
				while(1)
				{
					if(UcOscrsel == (RRATETABLE - 1))
					{
						break;
					}
					UcOscrsel += 1 ;
					ScTblRate_Val = ScRselRate[ UcOscrsel ] ;	
					if( ScTblRate_Tgt <= ScTblRate_Val )
					{
						break;
					}
				}
			}
			else
			{
				;
			}
		}
		else
		{		
		
			if( FcalB > 0 )			
			{
				UcMeasFlg |= CSELPLS ;
				FlRatePbk = FcalB ;
				UcOsccselP = UcOsccsel ;
				if( UcMeasFlg & CSELMNS)
				{
					UcMeasFlg |= CSELFX ;
					UcClkJdg = FIX_MEAS ;			
				}
				else if(UcOsccsel == (CRATETABLE - 1))
				{
					if(UcOscrsel < ( RRATETABLE - 1 ))
					{
						UcOscrsel += 1 ;
						UcOsccsel = START_CSEL ;
						UcMeasFlg = 0 ;			
					}
					else
					{
						UcClkJdg = OVR_MEAS ;			
					}
				}
				else
				{
					UcOsccsel += 1 ;
				}
			}
			else					
			{
				UcMeasFlg |= CSELMNS ;
				FlRateMbk = (-1)*FcalB ;
				UcOsccselM = UcOsccsel ;
				if( UcMeasFlg & CSELPLS)
				{
					UcMeasFlg |= CSELFX ;
					UcClkJdg = FIX_MEAS ;			
				}
				else if(UcOsccsel == 0x00)
				{
					if(UcOscrsel > 0)
					{
						UcOscrsel -= 1 ;
						UcOsccsel = START_CSEL ;
						UcMeasFlg = 0 ;			
					}
					else
					{
					UcClkJdg = OVR_MEAS ;			
					}
				}
				else
				{
					UcOsccsel -= 1 ;
				}
			}
			if(UcMeasCnt >= MEAS_MAX)
			{
				UcClkJdg = OVR_MEAS ;			
			}
		}	
		RegWriteA( OSCSET, ( UcOscrsel << 5 ) | (UcOsccsel << 1 ) ) ;	
	}
	
	UsResult = EXE_END ;
	
	if(UcClkJdg == FIX_MEAS)
	{
		if( FlRatePbk < FlRateMbk )
		{
			UcOsccsel = UcOsccselP ; 
		}
		else
		{
			UcOsccsel = UcOsccselM ; 
		}
	
		RegWriteA( OSCSET, ( UcOscrsel << 5 ) | (UcOsccsel << 1 ) ) ;	

		
		
	}
	StAdjPar.UcOscVal = ( ( UcOscrsel << 5 ) | (UcOsccsel << 1 ) );
	
	if(UcClkJdg == OVR_MEAS)
	{
		UsResult = EXE_OCADJ ;
		StAdjPar.UcOscVal = 0x00 ;
	}
	RegWriteA( SRVDIV,	UcSrvDivBk ) ;		
	return( UsResult );
}
#endif


#ifdef HALLADJ_HW
void SetSineWave( unsigned char UcJikuSel , unsigned char UcMeasMode )
{
 #ifdef	USE_EXTCLK_ALL	
    unsigned short  UsFRQ[]   = { 0x30EE , 0x037E } ;          
 #else
    unsigned short  UsFRQ[]   = { 0x1877 , 0x01BF } ;          
 #endif
	unsigned long   UlAMP[2][2]   = {{ 0x3CA3D70A , 0x3CA3D70A } ,		
									 { 0x3F800000 , 0x3F800000 } };		
	unsigned char	UcEqSwX , UcEqSwY ;

    UcMeasMode &= 0x01;
    UcJikuSel  &= 0x01;

	
	RegWriteA( WC_SINPHSX, 0x00 ) ;					
	RegWriteA( WC_SINPHSY, 0x00 ) ;					
	
	
	RegWriteA( WC_MESSINMODE,     0x00 ) ;			
	RegWriteA( WC_MESWAIT,     0x00 ) ;				
	
    
	RamWrite32A( sxsin, UlAMP[UcMeasMode][X_DIR] ) ;				
	RamWrite32A( sysin, UlAMP[UcMeasMode][Y_DIR] ) ;				

	
	RegWriteA( WC_SINFRQ0,	(unsigned char)UsFRQ[UcMeasMode] ) ;				
	RegWriteA( WC_SINFRQ1,	(unsigned char)(UsFRQ[UcMeasMode] >> 8) ) ;			

    
	RegReadA( WH_EQSWX, &UcEqSwX ) ;				
	RegReadA( WH_EQSWY, &UcEqSwY ) ;				
    if( !UcMeasMode && !UcJikuSel )       
    {
		UcEqSwX |= 0x10 ;				
		UcEqSwY &= ~EQSINSW ;
    }
    else if( !UcMeasMode && UcJikuSel )   
    {
		UcEqSwX &= ~EQSINSW ;
		UcEqSwY |= 0x10 ;				
    }
    else if( UcMeasMode && !UcJikuSel )   
    {
    	UcEqSwX = 0x22 ;				
    	UcEqSwY = 0x03 ;
    }
    else                    
    {
		UcEqSwX = 0x03 ;
		UcEqSwY = 0x22 ;				

    }
	RegWriteA( WH_EQSWX, UcEqSwX ) ;				
	RegWriteA( WH_EQSWY, UcEqSwY ) ;				
}

void StartSineWave( void )
{
    
	RegWriteA( WC_SINON,     0x01 ) ;				

}

void StopSineWave( void )
{
	unsigned char		UcEqSwX , UcEqSwY ;
	
	RegWriteA( WC_SINON,     0x00 ) ;				
	RegReadA( WH_EQSWX, &UcEqSwX ) ;				
	RegReadA( WH_EQSWY, &UcEqSwY ) ;				
	UcEqSwX &= ~EQSINSW ;
	UcEqSwY &= ~EQSINSW ;
	RegWriteA( WH_EQSWX, UcEqSwX ) ;				
	RegWriteA( WH_EQSWY, UcEqSwY ) ;				

}

void SetMeasFil( unsigned char UcFilSel )
{
	
	MesFil( UcFilSel ) ;					

}

void ClrMeasFil( void )
{
    
	ClrGyr( 0x1000 , CLR_FRAM1 );		
 
}

 #ifdef	MODULE_CALIBRATION
unsigned char	 LoopGainAdj( unsigned char UcJikuSel)
{

	unsigned short	UsRltVal ;
	unsigned char	UcAdjSts	= FAILURE ;
	
    UcJikuSel &= 0x01;

	StbOnn() ;											
	
	
	WitTim( 200 ) ;
	
	
	LopPar( UcJikuSel ) ;
	
	
    SetSineWave( UcJikuSel , __MEASURE_LOOPGAIN );

	
	RegWriteA( WC_MESLOOP1, 0x00 ) ;				
	RegWriteA( WC_MESLOOP0, 0x40 ) ;				
	RamWrite32A( msmean	, 0x3C800000 );				
	RegWriteA( WC_MESABS, 0x01 ) ;					
	
    
    RamWrite32A( LGMax  , 0x3F800000 );		
    RamWrite32A( LGMin  , 0x3E000100 );		
    RegWriteA( WC_AMJLOOP1, 0x00 );			
    RegWriteA( WC_AMJLOOP0, 0x41 );			
    RegWriteA( WC_AMJIDL1, 0x00 );			
    RegWriteA( WC_AMJIDL0, 0x00 );			

    
    SetMeasFil( LOOPGAIN );

    
    ClrMeasFil();

    
    StartSineWave();

    
    
	BsyWit( WC_AMJMODE, (0x0E | ( UcJikuSel << 4 )) ) ;				

	RegReadA( RC_AMJERROR, &UcAdjBsy ) ;							

	
	RamAccFixMod( ON ) ;							
	
	if( UcAdjBsy )
	{
		if( UcJikuSel == X_DIR )
		{
			RamReadA( sxg, &UsRltVal ) ;						
			StAdjPar.StLopGan.UsLxgVal	= UsRltVal ;
			StAdjPar.StLopGan.UsLxgSts	= 0x0000 ;
		} else {
			RamReadA( syg, &UsRltVal ) ;						
			StAdjPar.StLopGan.UsLygVal	= UsRltVal ;
			StAdjPar.StLopGan.UsLygSts	= 0x0000 ;
		}

	}
	else
	{
		if( UcJikuSel == X_DIR )
		{
			RamReadA( sxg, &UsRltVal ) ;						
			StAdjPar.StLopGan.UsLxgVal	= UsRltVal ;
			StAdjPar.StLopGan.UsLxgSts	= 0xFFFF ;
		} else {
			RamReadA( syg, &UsRltVal ) ;						
			StAdjPar.StLopGan.UsLygVal	= UsRltVal ;
			StAdjPar.StLopGan.UsLygSts	= 0xFFFF ;
		}
		UcAdjSts	= SUCCESS ;												
	}

	
	RamAccFixMod( OFF ) ;							
	
    
    StopSineWave();

	return( UcAdjSts ) ;
}
 #endif

unsigned char  BiasOffsetAdj( unsigned char UcJikuSel , unsigned char UcMeasCnt )
{
	unsigned char 	UcHadjRst ;
									
	unsigned long  UlTgtVal[2][5]    =  {{ 0x3F800000 , 0x3D200140 , 0xBD200140 , 0x3F547AE1 , 0x3F451EB8 },	
										 { 0x3F000000 , 0x3D200140 , 0xBD200140 , 0x3F50A3D7 , 0x3F48F5C3 }} ;	

	if(UcMeasCnt > 1)		UcMeasCnt = 1 ;
	
    UcJikuSel &= 0x01;

    
    SetSineWave( UcJikuSel , __MEASURE_BIASOFFSET );

	
	RegWriteA( WC_MESLOOP1, 0x00 ) ;				
	RegWriteA( WC_MESLOOP0, 0x04 ) ;				
	RamWrite32A( msmean	, 0x3E000000 );				
	RegWriteA( WC_MESABS, 0x00 ) ;					

    
    RamWrite32A( HOStp  , UlTgtVal[UcMeasCnt][0] );		
    RamWrite32A( HOMax  , UlTgtVal[UcMeasCnt][1] );		
    RamWrite32A( HOMin  , UlTgtVal[UcMeasCnt][2] );		
    RamWrite32A( HBStp  , UlTgtVal[UcMeasCnt][0] );		
    RamWrite32A( HBMax  , UlTgtVal[UcMeasCnt][3] );		
    RamWrite32A( HBMin  , UlTgtVal[UcMeasCnt][4] );		

	RegWriteA( WC_AMJLOOP1, 0x00 );			
    RegWriteA( WC_AMJLOOP0, 0x40 );			
    RegWriteA( WC_AMJIDL1, 0x00 );			
    RegWriteA( WC_AMJIDL0, 0x00 );			
	
    
    SetMeasFil( HALL_ADJ );

    
    ClrMeasFil();

    
    StartSineWave();

    
	BsyWit( WC_AMJMODE, (0x0C | ( UcJikuSel << 4 )) ) ;				
	
	RegReadA( RC_AMJERROR, &UcAdjBsy ) ;							
	
	if( UcAdjBsy )
	{
		if( UcJikuSel == X_DIR )
		{
			UcHadjRst = EXE_HXADJ ;
		}
		else
		{
			UcHadjRst = EXE_HYADJ ;
		}

	}
	else
	{
		UcHadjRst = EXE_END ;
	}

    
    StopSineWave();

    
	
	
	RamAccFixMod( ON ) ;							
	
	if( UcJikuSel == X_DIR )
	{
		RamReadA( MSPP1AV, &StAdjPar.StHalAdj.UsHlxMxa  ) ;	 	
		RamReadA( MSCT1AV, &StAdjPar.StHalAdj.UsHlxCna  ) ;	 	
	}
	else
	{
		RamReadA( MSPP1AV, &StAdjPar.StHalAdj.UsHlyMxa  ) ;	 	
		RamReadA( MSCT1AV, &StAdjPar.StHalAdj.UsHlyCna  ) ;	 	
	}

	StAdjPar.StHalAdj.UsHlxCna = (unsigned short)((signed short)StAdjPar.StHalAdj.UsHlxCna << 1 ) ;
	StAdjPar.StHalAdj.UsHlyCna = (unsigned short)((signed short)StAdjPar.StHalAdj.UsHlyCna << 1 ) ;
	
	RamAccFixMod( OFF ) ;							
	
	return( UcHadjRst ) ;
}

#endif


void	GyrGan( unsigned char UcGygmode , unsigned long UlGygXval , unsigned long UlGygYval )
{
	switch ( UcGygmode ) {
		case VAL_SET :
			RamWrite32A( gxzoom, UlGygXval ) ;		
			RamWrite32A( gyzoom, UlGygYval ) ;		
			break ;
		case VAL_FIX :
			RamWrite32A( gxzoom, UlGygXval ) ;		
			RamWrite32A( gyzoom, UlGygYval ) ;		

			break ;
		case VAL_SPC :
			RamRead32A( gxzoom, &UlGygXval ) ;		
			RamRead32A( gyzoom, &UlGygYval ) ;		
		
			break ;
	}

}

void	SetPanTiltMode( unsigned char UcPnTmod )
{
	switch ( UcPnTmod ) {
		case OFF :
			RegWriteA( WG_PANON, 0x00 ) ;			
			break ;
		case ON :
			RegWriteA( WG_PANON, 0x01 ) ;			
			break ;
	}

}


#ifdef GAIN_CONT
unsigned char	TriSts( void )
{
	unsigned char UcRsltSts = 0;
	unsigned char UcVal ;

	RegReadA( WG_ADJGANGXATO, &UcVal ) ;	
	if( UcVal & 0x03 ){						
		RegReadA( RG_LEVJUGE, &UcVal ) ;	
		UcRsltSts = UcVal & 0x11 ;		
		UcRsltSts |= 0x80 ;				
	}
	return( UcRsltSts ) ;
}
#endif

unsigned char	DrvPwmSw( unsigned char UcSelPwmMod )
{

	switch ( UcSelPwmMod ) {
		case Mlnp :
			RegWriteA( DRVFC	, 0xF0 );			
			UcPwmMod = PWMMOD_CVL ;
			break ;
		
		case Mpwm :
#ifdef	PWM_BREAK
			RegWriteA( DRVFC	, 0x00 );			
#else
			RegWriteA( DRVFC	, 0xC0 );			
#endif
			UcPwmMod = PWMMOD_PWM ;
 			break ;
	}
	
	return( UcSelPwmMod << 4 ) ;
}

 #ifdef	NEUTRAL_CENTER
unsigned char	TneHvc( void )
{
	unsigned char	UcRsltSts;
	unsigned short	UsMesRlt1 ;
	unsigned short	UsMesRlt2 ;
	
	SrvCon( X_DIR, OFF ) ;				
	SrvCon( Y_DIR, OFF ) ;				
	
	WitTim( 500 ) ;
	
	
	
	MesFil( THROUGH ) ;					
	
	RegWriteA( WC_MESLOOP1	, 0x00 );			
	RegWriteA( WC_MESLOOP0	, 0x40);			

	RamWrite32A( msmean	, 0x3C800000 );			
	
	RegWriteA( WC_MES1ADD0,  ( unsigned char )AD0Z ) ;							
	RegWriteA( WC_MES1ADD1,  ( unsigned char )(( AD0Z >> 8 ) & 0x0001 ) ) ;		
	RegWriteA( WC_MES2ADD0,  ( unsigned char )AD1Z ) ;							
	RegWriteA( WC_MES2ADD1,  ( unsigned char )(( AD1Z >> 8 ) & 0x0001 ) ) ;		
	
	RamWrite32A( MSABS1AV, 	0x00000000 ) ;		
	RamWrite32A( MSABS2AV, 	0x00000000 ) ;		
	
	RegWriteA( WC_MESABS, 0x00 ) ;				
	
	BsyWit( WC_MESMODE, 0x01 ) ;				
	
	RamAccFixMod( ON ) ;							
	
	RamReadA( MSABS1AV, &UsMesRlt1 ) ;			
	RamReadA( MSABS2AV, &UsMesRlt2 ) ;			

	RamAccFixMod( OFF ) ;							
	
	StAdjPar.StHalAdj.UsHlxCna = UsMesRlt1;			
	StAdjPar.StHalAdj.UsHlxCen = UsMesRlt1;			
	
	StAdjPar.StHalAdj.UsHlyCna = UsMesRlt2;			
	StAdjPar.StHalAdj.UsHlyCen = UsMesRlt2;			
	
	UcRsltSts = EXE_END ;				
	
	return( UcRsltSts );
}
 #endif	

void	SetGcf( unsigned char	UcSetNum )
{
	
	
	if(UcSetNum > (COEFTBL - 1))
		UcSetNum = (COEFTBL -1) ;			

	UlH1Coefval	= ClDiCof[ UcSetNum ] ;
		
	
	RamWrite32A( gxh1c, UlH1Coefval ) ;		
	RamWrite32A( gyh1c, UlH1Coefval ) ;		

#ifdef H1COEF_CHANGER
		SetH1cMod( UcSetNum ) ;							
#endif

}

#ifdef H1COEF_CHANGER
void	SetH1cMod( unsigned char	UcSetNum )
{
	
	switch( UcSetNum ){
	case ( ACTMODE ):				
		IniPtMovMod( OFF ) ;							
		
		
		
		UlH1Coefval	= ClDiCof[ 0 ] ;
			
		UcH1LvlMod = 0 ;
		
		
		RamWrite32A( gxlmt6L, MINLMT ) ;		
		RamWrite32A( gxlmt6H, MAXLMT ) ;		

		RamWrite32A( gylmt6L, MINLMT ) ;		
		RamWrite32A( gylmt6H, MAXLMT ) ;		

		RamWrite32A( gxhc_tmp, 	UlH1Coefval ) ;	
		RamWrite32A( gxmg, 		CHGCOEF ) ;		

		RamWrite32A( gyhc_tmp, 	UlH1Coefval ) ;	
		RamWrite32A( gymg, 		CHGCOEF ) ;		
		
		RegWriteA( WG_HCHR, 0x12 ) ;			
		break ;
		
	case( S2MODE ):				
		RegWriteA( WG_HCHR, 0x10 ) ;			
		break ;
		
	case( MOVMODE ):			
		IniPtMovMod( ON ) ;							
		
		RamWrite32A( gxlmt6L, MINLMT_MOV ) ;	
		RamWrite32A( gylmt6L, MINLMT_MOV ) ;	

		RamWrite32A( gxmg, CHGCOEF_MOV ) ;		
		RamWrite32A( gymg, CHGCOEF_MOV ) ;		
			
		RamWrite32A( gxhc_tmp, UlH1Coefval ) ;		
		RamWrite32A( gyhc_tmp, UlH1Coefval ) ;		
		
		RegWriteA( WG_HCHR, 0x12 ) ;			
		break ;
		
	default :
		IniPtMovMod( OFF ) ;							
		
		UcH1LvlMod = UcSetNum ;
			
		RamWrite32A( gxlmt6L, MINLMT ) ;		
		RamWrite32A( gylmt6L, MINLMT ) ;		
		
		RamWrite32A( gxmg, 	CHGCOEF ) ;			
		RamWrite32A( gymg, 	CHGCOEF ) ;			
			
		RamWrite32A( gxhc_tmp, UlH1Coefval ) ;		
		RamWrite32A( gyhc_tmp, UlH1Coefval ) ;		
		
		RegWriteA( WG_HCHR, 0x12 ) ;			
		break ;
	}
}
#endif

unsigned short	RdFwVr( void )
{
	unsigned short	UsVerVal ;
	
	UsVerVal = (unsigned short)((MDL_VER << 8) | FW_VER ) ;
	return( UsVerVal ) ;
}
