...
#define AF_DRVNAME "DRV201AF"
#define I2C_SLAVE_ADDRESS        0x1C
#define I2C_REGISTER_ID            0x03
#define PLATFORM_DRIVER_NAME "lens_actuator_DRV201af"
#define AF_DRIVER_CLASS_NAME "actuatordrv_DRV201af"
...
static int s4AF_WriteInitReg(){
    int  i4RetValue = 0;
    char puSendCmd[2] = {0x02,0x2};
    char puSendCmd1[2] = {0x06,0x2};
    char puSendCmd2[2] = {0x07,0x82};
    UINT16 data = 0;
...
static int s4AF_WriteReg(u16 a_u2Data)
{
    ...
    char puSendCmd[2] = {0x03 , 0x00};
    char puSendCmd1[2] = {0x04 , 0xFF};
    UINT16 data = 0;
    LOG_INF("a_u2Data = %d", a_u2Data);
    i4RetValue = i2c_master_send(g_pstAF_I2Cclient, puSendCmd, 2);
...
inline static int getAFInfo(__user stDRV201AF_MotorInfo * pstMotorInfo)
{
...
#ifdef LensdrvCM3
inline static int getAFMETA(__user stDRV201AF_MotorMETAInfo * pstMotorMETAInfo)
{
    stDRV201AF_MotorMETAInfo stMotorMETAInfo;
    stMotorMETAInfo.Aperture=2.8;       
    stMotorMETAInfo.Facing=1;
    stMotorMETAInfo.FilterDensity=1;   
    stMotorMETAInfo.FocalDistance=1.0;    
    stMotorMETAInfo.FocalLength=34.0;  
    stMotorMETAInfo.FocusRange=1.0;    
    ...
    if(copy_to_user(pstMotorMETAInfo , &stMotorMETAInfo , sizeof(stDRV201AF_MotorMETAInfo)))
    {
        LOG_INF("copy to user failed when getting motor information \n");
    }
...
inline static int moveAF(unsigned long a_u4Position)
{
    ...
    {
        puSendCmd[0] = 0x3;
        puSendCmd[1]= a_u4Position >> 8;
        ret = i2c_master_send(g_pstAF_I2Cclient, puSendCmd, 2);
        if(ret < 0) printk("%s %d: write I2C error \n", __func__, __LINE__);
        puSendCmd[0] = 0x4;
        puSendCmd[1]= a_u4Position & 0xff;
        ret = i2c_master_send(g_pstAF_I2Cclient, puSendCmd, 2);
        if(ret < 0) printk("%s %d: write I2C error \n", __func__, __LINE__);
...
unsigned long a_u4Param)
{
    ...
    switch(a_u4Command)
    {
        case DRV201AFIOC_G_MOTORINFO :
            i4RetValue = getAFInfo((__user stDRV201AF_MotorInfo *)(a_u4Param));
        break;
        #ifdef LensdrvCM3
        case DRV201AFIOC_G_MOTORMETAINFO :
            i4RetValue = getAFMETA((__user stDRV201AF_MotorMETAInfo *)(a_u4Param));
        break;
        #endif
...
{
    if(i2c_register_board_info(LENS_I2C_BUSNUM, &kd_lens_dev, 1) < 0)
        printk("\n%s %d: I2C register board error \n", __func__, __LINE__);
