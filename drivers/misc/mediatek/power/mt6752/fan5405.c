...
int fan5405_dump_register_htc(char *buf, int size)
{
    int i=0;
	int len = 0;
...
static ssize_t store_fan5405_access(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    ...
    if(buf != NULL && size != 0)
    {
        battery_xlog_printk(BAT_LOG_FULL,"[store_fan5405_access] buf is %s and size is %lu \n",buf,size);
...
static DEVICE_ATTR(fan5405_access, 0664, show_fan5405_access, store_fan5405_access); 
