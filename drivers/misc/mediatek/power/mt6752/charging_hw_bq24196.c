...
static kal_uint32 charging_hw_init(void *data)
{
	...
	bq24196_set_vindpm(0xA); 
	bq24196_set_reg_rst(0x0);
	bq24196_set_wdt_rst(0x1); 
	bq24196_set_sys_min(0x5); 
	bq24196_set_iprechg(0x3); 
	bq24196_set_iterm(0x0); 

	#if defined(HIGH_BATTERY_VOLTAGE_SUPPORT)
	bq24196_set_vreg(0x32); 
	#else
	bq24196_set_vreg(0x2C); 
	#endif    

	bq24196_set_batlowv(0x1); 
	bq24196_set_vrechg(0x0); 
	bq24196_set_en_term(0x1); 
	bq24196_set_term_stat(0x0); 
	bq24196_set_watchdog(0x1); 
	bq24196_set_en_timer(0x0); 
	bq24196_set_int_mask(0x0); 
...
static kal_uint32 charging_set_cv_voltage(void *data)
{
	...
	if(mt6325_upmu_get_swcid() == PMIC6325_E1_CID_CODE)
	{
		...
		bq24196_set_vreg(0x1F);
		#else
		bq24196_set_vreg(0x14);
		#endif
...
static kal_uint32 charging_set_ta_current_pattern(void *data)
{
    ...
    if(KAL_FALSE == charging_status)
    {
        ...
        bq24196_set_ichg(0x0);  
        bq24196_set_chg_config(0x1);  
    }

    if(increase == KAL_TRUE)
    {
        bq24196_set_iinlim(0x0); 
        msleep(85);
