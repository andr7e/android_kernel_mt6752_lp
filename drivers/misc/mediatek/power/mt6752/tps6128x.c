...
{       
    ...
    tps6128x_config_interface(0x3, 0xB, 0x1F, 0); 
    tps6128x_config_interface(0x4, 0xF, 0xF,  0); 
...
static ssize_t store_tps6128x_access(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    ...
    if(buf != NULL && size != 0)
    {
        battery_xlog_printk(BAT_LOG_CRTI,"[store_tps6128x_access] buf is %s and size is %zu \n",buf,size);
...
static DEVICE_ATTR(tps6128x_access, 0664, show_tps6128x_access, store_tps6128x_access); 
