 ...
 static kal_uint32 charging_hw_init(void *data)
{
   ...
   ncp1854_set_chgto_dis(0x1); 
    if((ncp1854_status == 0x8) || (ncp1854_status == 0x9) || (ncp1854_status == 0xA)) 
        ncp1854_set_ctrl_vbat(0x1C); 

   	
	
         ncp1854_set_ieoc(0x4); 
         ncp1854_set_iweak(0x3); 
      
	

	ncp1854_set_aicl_en(0x1); 

	ncp1854_set_iinset_pin_en(0x0); 

    ncp1854_set_ctrl_vfet(0x3); 
...
void kick_charger_wdt(void)
{
	...
	#if 0
    upmu_set_rg_chrwdt_td(0x0);           
    upmu_set_rg_chrwdt_int_en(1);         
    upmu_set_rg_chrwdt_en(1);             
    upmu_set_rg_chrwdt_wr(1);             
	#endif
...
static kal_uint32 charging_set_power_off(void *data)
{
 ...
 static kal_uint32 charging_set_ta_current_pattern(void *data)
 {
	 ...
	 {
		 ncp1854_set_iinlim(0x0); 
		 msleep(85);
		 
		 ncp1854_set_iinlim(0x1); 
...
static void diso_polling_handler(struct work_struct *work)
{
	...
	switch(trigger_channel)
	{
		...
		default:
			set_vdc_auxadc_irq(DISO_IRQ_DISABLE, 0);
			set_vusb_auxadc_irq(DISO_IRQ_DISABLE, 0);
			battery_xlog_printk(BAT_LOG_CRTI, "[DISO]VUSB auxadc IRQ triggered ERROR OR TEST, channel=%d\n", trigger_channel);
			return; 
	}
