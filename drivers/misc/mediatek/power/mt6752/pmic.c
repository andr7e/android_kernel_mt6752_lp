...
#define BAT_HV_THD   (POWER_INT0_VOLT*4096/7200) 
#define BAT_LV_1_THD (POWER_INT1_VOLT*4096/7200) 
#define BAT_LV_2_THD (POWER_INT2_VOLT*4096/7200) 
...
{
    mt6325_upmu_set_rg_lbat_en_min(en_val);  
...
void low_battery_protect_init(void)
{
    ...
    mt6325_upmu_set_rg_lbat_debt_max(20);
...
extern void tc_bif_1008_step_1(void);
#endif
...
irqreturn_t mt6325_eint_handler(int irq, void *desc)
{
    mt_pmic_eint_irq_mt6325();
...
#if 1
	g_mt6325_irq = mt_gpio_to_irq(g_eint_pmit_mt6325_num);

	mt_gpio_set_debounce(g_eint_pmit_mt6325_num, g_cust_eint_mt_pmic_debounce_cn);

	request_irq(g_mt6325_irq, mt6325_eint_handler, g_cust_eint_mt_pmic_type, "mt6325-eint", NULL);

	xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[CUST_EINT] mt6325_irq=%d\n", g_mt6325_irq);
...
void dct_pmic_VDVFS11_enable(kal_bool dctEnable)
{
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[dct_pmic_VRTC_enable] %d\n", dctEnable);
...
void dct_pmic_VDVFS12_enable(kal_bool dctEnable)
{
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[dct_pmic_VRTC_enable] %d\n", dctEnable);
...
void dct_pmic_VCORE1_enable(kal_bool dctEnable)
{
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[dct_pmic_VRTC_enable] %d\n", dctEnable);
...
void dct_pmic_VCORE2_enable(kal_bool dctEnable)
{
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[dct_pmic_VRTC_enable] %d\n", dctEnable);
...
void dct_pmic_VGPU_enable(kal_bool dctEnable)
{
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[dct_pmic_VRTC_enable] %d\n", dctEnable);
...
void dct_pmic_VDRAM_enable(kal_bool dctEnable)
{
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[dct_pmic_VRTC_enable] %d\n", dctEnable);
...
void dct_pmic_VRF18_0_enable(kal_bool dctEnable)
{
    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[dct_pmic_VRTC_enable] %d\n", dctEnable);
...
void pmic_ldo_enable(MT_POWER powerId, kal_bool powerEnable)
{
    ...
    else if(powerId == MT6325_POWER_BUCK_VDVFS11	)  { dct_pmic_VDVFS11_enable(powerEnable);}
    else if(powerId == MT6325_POWER_BUCK_VDVFS12	)  { dct_pmic_VDVFS12_enable(powerEnable);}
    else if(powerId == MT6325_POWER_BUCK_VCORE1		)  { dct_pmic_VCORE1_enable(powerEnable);}
    else if(powerId == MT6325_POWER_BUCK_VCORE2		)  { dct_pmic_VCORE2_enable(powerEnable);}
    else if(powerId == MT6325_POWER_BUCK_VGPU		)  { dct_pmic_VGPU_enable(powerEnable);}
    else if(powerId == MT6325_POWER_BUCK_VIO18		)  { dct_pmic_VIO18_enable(powerEnable);}
    else if(powerId == MT6325_POWER_BUCK_VDRAM		)  { dct_pmic_VDRAM_enable(powerEnable);}
    else if(powerId == MT6325_POWER_BUCK_VPA		)  { dct_pmic_VPA_enable(powerEnable);}
    else if(powerId == MT6325_POWER_BUCK_VRF18_0	)  { dct_pmic_VRF18_0_enable(powerEnable);}
...
int pmic_ldo_get_status(int powerId)
{
	char vreg_en_stat[128];

	if(     powerId == MT6325_POWER_LDO_VTCXO0)        { show_LDO_VTCXO0_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_LDO_VTCXO1)        { show_LDO_VTCXO1_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_LDO_VAUD28)        { show_LDO_VAUD28_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_LDO_VAUXA28)       { show_LDO_VAUXA28_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_LDO_VBIF28)        { show_LDO_VBIF28_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_LDO_VCAMA)         { show_LDO_VCAMA_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_LDO_VCN28)         { show_LDO_VCN28_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_LDO_VCN33)         { show_LDO_VCN33_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_LDO_VRF18_1)       { show_LDO_VRF18_1_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_LDO_VUSB33)        { show_LDO_VUSB33_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_LDO_VMCH)          { show_LDO_VMCH_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_LDO_VMC)           { show_LDO_VMC_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_LDO_VEMC33)        { show_LDO_VEMC33_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_LDO_VIO28)         { show_LDO_VIO28_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_LDO_VCAM_AF)       { show_LDO_VCAM_AF_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_LDO_VGP1)          { show_LDO_VGP1_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_LDO_VEFUSE)        { show_LDO_VEFUSE_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_LDO_VSIM1)         { show_LDO_VSIM1_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_LDO_VSIM2)         { show_LDO_VSIM2_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_LDO_VMIPI)         { show_LDO_VMIPI_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_LDO_VCN18)         { show_LDO_VCN18_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_LDO_VGP2)          { show_LDO_VGP2_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_LDO_VCAMD)         { show_LDO_VCAMD_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_LDO_VCAM_IO)       { show_LDO_VCAM_IO_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_LDO_VSRAM_DVFS1)   { show_LDO_VSRAM_DVFS1_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_LDO_VGP3)          { show_LDO_VGP3_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_LDO_VBIASN)        { show_LDO_VBIASN_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_LDO_VRTC)          { show_LDO_VRTC_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_BUCK_VDVFS11)      { show_BUCK_VDVFS11_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_BUCK_VDVFS12)      { show_BUCK_VDVFS12_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_BUCK_VCORE1)       { show_BUCK_VCORE1_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_BUCK_VCORE2)       { show_BUCK_VCORE2_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_BUCK_VGPU)         { show_BUCK_VGPU_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_BUCK_VIO18)        { show_BUCK_VIO18_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_BUCK_VDRAM)        { show_BUCK_VDRAM_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_BUCK_VPA)          { show_BUCK_VPA_STATUS(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_BUCK_VRF18_0)      { show_BUCK_VRF18_0_STATUS(NULL, NULL, vreg_en_stat); }
	else {
		xlog_printk(ANDROID_LOG_WARN, "Power/PMIC", "[pmic_ldo_enable] UnKnown powerId (%d)\n", powerId);
		return -1;
	}

	return simple_strtol(vreg_en_stat, NULL, 0);
}

int pmic_ldo_get_voltage(int powerId)
{
	char vreg_en_stat[128];

	if(     powerId == MT6325_POWER_LDO_VTCXO0)        { show_LDO_VTCXO0_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_LDO_VTCXO1)        { show_LDO_VTCXO1_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_LDO_VAUD28)        { show_LDO_VAUD28_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_LDO_VAUXA28)       { show_LDO_VAUXA28_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_LDO_VBIF28)        { show_LDO_VBIF28_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_LDO_VCAMA)         { show_LDO_VCAMA_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_LDO_VCN28)         { show_LDO_VCN28_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_LDO_VCN33)         { show_LDO_VCN33_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_LDO_VRF18_1)       { show_LDO_VRF18_1_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_LDO_VUSB33)        { show_LDO_VUSB33_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_LDO_VMCH)          { show_LDO_VMCH_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_LDO_VMC)           { show_LDO_VMC_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_LDO_VEMC33)        { show_LDO_VEMC33_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_LDO_VIO28)         { show_LDO_VIO28_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_LDO_VCAM_AF)       { show_LDO_VCAM_AF_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_LDO_VGP1)          { show_LDO_VGP1_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_LDO_VEFUSE)        { show_LDO_VEFUSE_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_LDO_VSIM1)         { show_LDO_VSIM1_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_LDO_VSIM2)         { show_LDO_VSIM2_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_LDO_VMIPI)         { show_LDO_VMIPI_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_LDO_VCN18)         { show_LDO_VCN18_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_LDO_VGP2)          { show_LDO_VGP2_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_LDO_VCAMD)         { show_LDO_VCAMD_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_LDO_VCAM_IO)       { show_LDO_VCAM_IO_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_LDO_VSRAM_DVFS1)   { show_LDO_VSRAM_DVFS1_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_LDO_VGP3)          { show_LDO_VGP3_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_LDO_VBIASN)        { show_LDO_VBIASN_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_LDO_VRTC)          { show_LDO_VRTC_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_BUCK_VDVFS11)      { show_BUCK_VDVFS11_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_BUCK_VDVFS12)      { show_BUCK_VDVFS12_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_BUCK_VCORE1)       { show_BUCK_VCORE1_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_BUCK_VCORE2)       { show_BUCK_VCORE2_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_BUCK_VGPU)         { show_BUCK_VGPU_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_BUCK_VIO18)        { show_BUCK_VIO18_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_BUCK_VDRAM)        { show_BUCK_VDRAM_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_BUCK_VPA)          { show_BUCK_VPA_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else if(powerId == MT6325_POWER_BUCK_VRF18_0)      { show_BUCK_VRF18_0_VOLTAGE(NULL, NULL, vreg_en_stat); }
	else {
		xlog_printk(ANDROID_LOG_WARN, "Power/PMIC", "[pmic_ldo_enable] UnKnown powerId (%d)\n", powerId);
		return -1;
	}
...
static DEVICE_ATTR(low_battery_protect_ut, 0664, show_low_battery_protect_ut, store_low_battery_protect_ut); 
...
static DEVICE_ATTR(battery_percent_protect_stop, 0664, show_battery_percent_protect_stop, store_battery_percent_protect_stop); 
...
static DEVICE_ATTR(battery_percent_protect_level, 0664, show_battery_percent_protect_level, store_battery_percent_protect_level); 
...
static ssize_t store_pmic_dvt(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    ...
    if(buf != NULL && size != 0)
    {
        printk("[store_pmic_dvt] buf is %s and size is %zu\n",buf,size);
...
void pmic_dig_reset(void)
{
    ...
    ret_val=pmic_config_interface(MT6325_TOP_RST_MISC_CLR, 0x0002, 0xFFFF, 0); 
    ret_val=pmic_config_interface(MT6325_TOP_RST_MISC_SET, 0x0001, 0xFFFF, 0); 
...
void pmic_full_reset(void)
{
    ...
    ret_val=pmic_config_interface(MT6325_TOP_RST_MISC_SET, 0x0002, 0xFFFF, 0); 
    ret_val=pmic_config_interface(MT6325_TOP_RST_MISC_SET, 0x0001, 0xFFFF, 0); 
...
void PMIC_INIT_SETTING_V1(void)
{
    ...
    if(mt6325_chip_version >= PMIC6325_E5_CID_CODE)
    {
        ...
        ret = pmic_config_interface(0x2E,0x1,0x1,12); 
        ...
        ret = pmic_config_interface(0x224,0x1,0x1,0); 
        ret = pmic_config_interface(0x224,0x1,0x1,1); 
        ret = pmic_config_interface(0x224,0x1,0x1,2); 
        ret = pmic_config_interface(0x224,0x1,0x1,3); 
        ret = pmic_config_interface(0x23E,0x1,0x1,2); 
        ret = pmic_config_interface(0x23E,0x1,0x1,3); 
        ret = pmic_config_interface(0x244,0x1,0x1,13); 
        ret = pmic_config_interface(0x244,0x1,0x1,14); 
        ret = pmic_config_interface(0x250,0x1,0x1,11); 
        ret = pmic_config_interface(0x250,0x2,0x3,14); 
        ret = pmic_config_interface(0x268,0x1,0x1,10); 
        ret = pmic_config_interface(0x2BC,0x1,0x1,0); 
        ret = pmic_config_interface(0x42C,0x1,0x1,3); 
        ret = pmic_config_interface(0x43A,0x2,0x7,5); 
        ret = pmic_config_interface(0x43A,0x2,0x7,10); 
        ret = pmic_config_interface(0x446,0x3,0x7,4); 
        ret = pmic_config_interface(0x464,0x7,0x7,0); 
        ret = pmic_config_interface(0x464,0x7,0x7,3); 
        ret = pmic_config_interface(0x466,0x4,0x7,4); 
        ret = pmic_config_interface(0x466,0x4,0x7,7); 
        ret = pmic_config_interface(0x46A,0x0,0x1,8); 
        ret = pmic_config_interface(0x46A,0x0,0x1,9); 
        ret = pmic_config_interface(0x470,0x7,0xF,10); 
        ret = pmic_config_interface(0x472,0x2,0x7,5); 
        ret = pmic_config_interface(0x474,0x5,0x7,4); 
        ret = pmic_config_interface(0x484,0x1,0x7,5); 
        ret = pmic_config_interface(0x486,0x3,0x7,4); 
        ret = pmic_config_interface(0x490,0x2,0x7,4); 
        ret = pmic_config_interface(0x4B0,0x1,0x1,1); 
        ret = pmic_config_interface(0x4B6,0x7F,0x7F,0); 
        ret = pmic_config_interface(0x4B6,0x4,0x7F,8); 
        ret = pmic_config_interface(0x4BC,0x10,0x7F,0); 
        ret = pmic_config_interface(0x4C6,0x3,0x3,0); 
        ret = pmic_config_interface(0x4C6,0x1,0x3,4); 
        ret = pmic_config_interface(0x502,0x11,0x7F,0); 
        ret = pmic_config_interface(0x502,0x4,0x7F,8); 
        ret = pmic_config_interface(0x538,0x1,0x1,8); 
        ret = pmic_config_interface(0x54A,0x1,0x3,0); 
        ret = pmic_config_interface(0x548,0x1,0x1,0); 
        ret = pmic_config_interface(0x614,0x11,0x7F,0); 
        ret = pmic_config_interface(0x614,0x4,0x7F,8); 
        ret = pmic_config_interface(0x63A,0x11,0x7F,0); 
        ret = pmic_config_interface(0x63A,0x4,0x7F,8); 
        ret = pmic_config_interface(0x640,0x10,0x7F,0); 
        ret = pmic_config_interface(0x634,0x1,0x1,1); 
        ret = pmic_config_interface(0x64A,0x1,0x1,8); 
        ret = pmic_config_interface(0x660,0x11,0x7F,0); 
        ret = pmic_config_interface(0x660,0x4,0x7F,8); 
        ret = pmic_config_interface(0x666,0x10,0x7F,0); 
        ret = pmic_config_interface(0x65A,0x1,0x1,1); 
        ret = pmic_config_interface(0x670,0x1,0x1,8); 
        ret = pmic_config_interface(0x6B0,0x4,0x7F,0); 
        ret = pmic_config_interface(0x6B0,0x0,0x1,7); 
        ret = pmic_config_interface(0x6B0,0x4,0x7F,8); 
        ret = pmic_config_interface(0x6B0,0x0,0x1,15); 
        ret = pmic_config_interface(0x6CA,0x3,0x3,4); 
        ret = pmic_config_interface(0xA00,0x1,0x1,3); 
        ret = pmic_config_interface(0xA00,0x0,0x3,12); 
        ret = pmic_config_interface(0xA04,0x1,0x1,2); 
        ret = pmic_config_interface(0xA06,0x1,0x1,2); 
        ret = pmic_config_interface(0xA06,0x0,0x3,4); 
        ret = pmic_config_interface(0xA08,0x1,0x1,3); 
        ret = pmic_config_interface(0xA08,0x0,0x3,12); 
        ret = pmic_config_interface(0xA12,0x1,0x1,2); 
        ret = pmic_config_interface(0xA12,0x0,0x3,4); 
        ret = pmic_config_interface(0xA18,0x1,0x1,2); 
        ret = pmic_config_interface(0xA18,0x0,0x3,4); 
        ret = pmic_config_interface(0xA1A,0x1,0x1,2); 
        ret = pmic_config_interface(0xA1A,0x0,0x3,4); 
        ret = pmic_config_interface(0xA20,0x0,0x1,1); 
        ret = pmic_config_interface(0xA38,0x0,0x1,1); 
        ret = pmic_config_interface(0xA48,0x0,0x1,9); 
        ret = pmic_config_interface(0xA4A,0x0,0x1,9); 
        ret = pmic_config_interface(0xA60,0x0,0x1F,0); 
        ret = pmic_config_interface(0xA64,0xF,0x1F,11); 
        ret = pmic_config_interface(0xCBC,0x1,0x1,8); 
        ret = pmic_config_interface(0xCBC,0x1,0x1,9); 
        ret = pmic_config_interface(0xCBC,0x1,0x1,10); 
        ...
        ret = pmic_config_interface(0xCEE,0x1,0x1,1); 
        ret = pmic_config_interface(0xE12,0x0,0x1,15); 
        ret = pmic_config_interface(0xE9C,0x2,0x3,0); 
        ret = pmic_config_interface(0xE9C,0x1,0x3,2); 
        ret = pmic_config_interface(0xE9C,0x1,0x3,4); 
        ret = pmic_config_interface(0xE9C,0x1,0x3,6); 
        ret = pmic_config_interface(0xE9C,0x3,0x3,8); 
        ret = pmic_config_interface(0xE9C,0x1,0x3,10); 
        ret = pmic_config_interface(0xE9C,0x0,0x3,14); 
        ret = pmic_config_interface(0xEBE,0x1,0x1,15); 
        
        #if defined(CONFIG_MTK_PUMP_EXPRESS_PLUS_SUPPORT)
        ret = pmic_config_interface(0xEF4,0xF,0xF,4); 
        #else
        ret = pmic_config_interface(0xEF4,0xB,0xF,4); 
        #endif         

        ret = pmic_config_interface(0xEFE,0x4,0xF,1); 
        ret = pmic_config_interface(0xF0C,0x3,0xF,0); 
        ret = pmic_config_interface(0xF1A,0x1,0x1,1); 
        ret = pmic_config_interface(0xF1E,0x0,0x7,4); 
        ret = pmic_config_interface(0xF24,0x1,0x1,2); 
        ret = pmic_config_interface(0xF24,0x1,0x1,6); 
        ret = pmic_config_interface(0xF24,0x1,0x1,7); 
        ret = pmic_config_interface(0x00E,0x3,0x3,0); 
    }
    else if(mt6325_chip_version >= PMIC6325_E3_CID_CODE)
    {
        ...
        ret = pmic_config_interface(0x46A,0x0,0x1,9); 
        ret = pmic_config_interface(0x470,0x7,0xF,10); 
        ret = pmic_config_interface(0x472,0x4,0x7,5); 
        ret = pmic_config_interface(0x474,0x5,0x7,4); 
        ret = pmic_config_interface(0x484,0x4,0x7,5); 
        ret = pmic_config_interface(0x486,0x3,0x7,4); 
        ret = pmic_config_interface(0x48E,0x3,0x7,5); 
        ret = pmic_config_interface(0x490,0x2,0x7,4); 
        ret = pmic_config_interface(0x4B0,0x1,0x1,1); 
        ret = pmic_config_interface(0x4B6,0x7F,0x7F,0); 
        ret = pmic_config_interface(0x4B6,0x4,0x7F,8); 
        ...
        ret = pmic_config_interface(0xE9C,0x0,0x3,14); 
        ret = pmic_config_interface(0xEBE,0x1,0x1,15); 
        ret = pmic_config_interface(0x43C,0x1,0x1,15); 
    ...
    else if(mt6325_chip_version >= PMIC6325_E1_CID_CODE)
    {
        ...
        ret = pmic_config_interface(0x1E,0x0,0x1,11); 
        ret = pmic_config_interface(0x462,0x1,0x1,3); 
        ret = pmic_config_interface(0x466,0x1,0x1,14); 
        ret = pmic_config_interface(0x466,0x1,0x1,15); 
        ret = pmic_config_interface(0x43C,0x1,0x1,15); 
        ret = pmic_config_interface(0x446,0x1,0x1,15); 
        ret = pmic_config_interface(0x474,0x1,0x1,15); 
        ret = pmic_config_interface(0x486,0x1,0x1,15); 
        ret = pmic_config_interface(0x490,0x1,0x1,9); 
        ret = pmic_config_interface(0x4,0x1,0x1,4); 
        ret = pmic_config_interface(0xA,0x1,0x1,0); 
        ret = pmic_config_interface(0xC,0x1,0x1,0); 
        ret = pmic_config_interface(0xC,0x1,0x1,1); 
        ...
        ret = pmic_config_interface(0x268,0x1,0x1,10); 
        ret = pmic_config_interface(0x446,0x3,0x7,4); 
        ret = pmic_config_interface(0x462,0xF,0xF,5); 
        ret = pmic_config_interface(0x464,0x7,0x7,0); 
        ret = pmic_config_interface(0x464,0x7,0x7,3); 
        ret = pmic_config_interface(0x466,0x4,0x7,4); 
        ret = pmic_config_interface(0x466,0x4,0x7,7); 
        ret = pmic_config_interface(0x468,0xF7,0xFF,2); 
        ret = pmic_config_interface(0x472,0x2,0x7,5); 
        ret = pmic_config_interface(0x474,0x5,0x7,4); 
        ret = pmic_config_interface(0x484,0x1,0x7,5); 
        ret = pmic_config_interface(0x486,0x3,0x7,4); 
        ret = pmic_config_interface(0x4B0,0x1,0x1,1); 
        ret = pmic_config_interface(0x4B6,0x11,0x7F,0); 
        ret = pmic_config_interface(0x4B6,0x4,0x7F,8); 
        ret = pmic_config_interface(0x4C6,0x3,0x3,0); 
        ret = pmic_config_interface(0x4C6,0x1,0x3,4); 
        ret = pmic_config_interface(0x502,0x11,0x7F,0); 
        ...
        ret = pmic_config_interface(0xEFE,0x4,0xF,1); 
        ret = pmic_config_interface(0xF0C,0x3,0xF,0); 
        ret = pmic_config_interface(0xF16,0x2,0x1F,0); 
        ret = pmic_config_interface(0xF1A,0x1,0x1,1); 
        ret = pmic_config_interface(0xF1E,0x0,0x7,4); 
        ret = pmic_config_interface(0xF24,0x1,0x1,2); 
        ret = pmic_config_interface(0xF24,0x1,0x1,6); 
...
static int pmic_mt_probe(struct platform_device *dev)
{
    ...
    if(get_mt6325_pmic_chip_version() < PMIC6325_E5_CID_CODE)
    {
        ...
        pmic_config_interface(0x204,0x0,0x1,6); 
        pmic_config_interface(0x538,0x0,0x1,8); 
        pmic_config_interface(0x64A,0x0,0x1,8); 
        pmic_config_interface(0x670,0x0,0x1,8); 
