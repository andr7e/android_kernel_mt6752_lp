...
static void *INFRA_MISC_ADDRESS = NULL;

static const kal_uint32 btsco_PacketValidMask[6][6] = {{0x1   , 0x1 << 1, 0x1 << 2, 0x1 << 3, 0x1 << 4 , 0x1 << 5 }, 
...
static const kal_uint8 btsco_PacketInfo[6][6] = {{ 30, 6, BT_SCO_PACKET_180 / SCO_TX_ENCODE_SIZE, BT_SCO_PACKET_180 / SCO_RX_PLC_SIZE}, 
    { 60, 3, BT_SCO_PACKET_180 / SCO_TX_ENCODE_SIZE, BT_SCO_PACKET_180 / SCO_RX_PLC_SIZE}, 
    { 90, 2, BT_SCO_PACKET_180 / SCO_TX_ENCODE_SIZE, BT_SCO_PACKET_180 / SCO_RX_PLC_SIZE}, 
    {120, 1, BT_SCO_PACKET_120 / SCO_TX_ENCODE_SIZE, BT_SCO_PACKET_120 / SCO_RX_PLC_SIZE}, 
...
static long AudDrv_btcvsd_ioctl(struct file *fp, unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd)
    {
        ...
        case SET_BTCVSD_STATE:
        {
            ...
            {
                btsco.uTXState = arg;
                printk("SET_BTCVSD_STATE set btsco.uTXState to 0x%lu \n", arg);
            }
            else 
            {
                btsco.uRXState = arg;
...
{
    printk("AudDrv_btcvsd_compat_ioctl cmd = 0x%x arg = %lu\n", cmd, arg);
    long ret;
	int err;
    ...
    if (ret < 0)
    {
        printk("AudDrv_btcvsd_compat_ioctl Fail \n");
    }
    else
    {
        printk("-AudDrv_btcvsd_compat_ioctl\n");
    }
    return ret;
}
#endif
...
static int AudDrv_btcvsd_probe(struct platform_device *dev)
{
    ...
    ret = request_irq(AP_BT_CVSD_IRQ_LINE, (irq_handler_t)AudDrv_BTCVSD_IRQ_handler, IRQF_TRIGGER_LOW, "BTCVSD_ISR_Handle", dev);
    if (ret < 0)
    {
        printk("AudDrv_btcvsd_probe request_irq AP_BT_CVSD_IRQ_LINE Fail \n");
    }

    
    void *AUDIO_INFRA_BASE_VIRTUAL = ioremap_nocache(AUDIO_INFRA_BASE_PHYSICAL, 0x1000);
    INFRA_MISC_ADDRESS = (volatile kal_uint32 *)(AUDIO_INFRA_BASE_VIRTUAL + 0x0F00);
    printk("AUDIO_INFRA_BASE_PHYSICAL = 0x%x, AUDIO_INFRA_BASE_VIRTUAL = %p, INFRA_MISC_ADDRESS = %p\n", AUDIO_INFRA_BASE_PHYSICAL, AUDIO_INFRA_BASE_VIRTUAL, INFRA_MISC_ADDRESS);
...
static ssize_t AudDrv_btcvsd_write(struct file *fp, const char __user *data, size_t count, loff_t *offset)
{
    ...
    while (count)
    {
        ...
        spin_lock_irqsave(&auddrv_BTCVSDTX_lock, flags);
        copy_size = btsco.pTX->u4BufferSize - (btsco.pTX->iPacket_w - btsco.pTX->iPacket_r) * SCO_TX_ENCODE_SIZE; 
        spin_unlock_irqrestore(&auddrv_BTCVSDTX_lock, flags);
        ...
        if (copy_size != 0)
        {
            ...
            if (BTSCOTX_WriteIdx + copy_size < btsco.pTX->u4BufferSize) 
            {
                if (!access_ok(VERIFY_READ, data_w_ptr, copy_size))
                {
                    printk("AudDrv_btcvsd_write 0ptr invalid data_w_ptr=%lx, size=%d \n", (unsigned long)data_w_ptr, copy_size);
            ...
            else  
            {
                ...
                {
                    printk("AudDrv_btcvsd_write 1ptr invalid data_w_ptr=%lx, size_1=%d \n", (unsigned long)data_w_ptr, size_1);
                    printk("AudDrv_btcvsd_write u4BufferSize=%d, BTSCOTX_WriteIdx=%d \n", btsco.pTX->u4BufferSize, BTSCOTX_WriteIdx);
                }
                else
                {
                ...
                {
                    printk("AudDrv_btcvsd_write 2ptr invalid data_w_ptr=%lx, size_1=%d, size_2=%d \n", (unsigned long)data_w_ptr, size_1, size_2);
...
static ssize_t AudDrv_btcvsd_read(struct file *fp,  char __user *data, size_t count, loff_t *offset)
{
    ...
    while (count)
    {
        PRINTK_AUDDRV("AudDrv_btcvsd_read btsco.pRX->iPacket_w=%d, btsco.pRX->iPacket_r=%d,count=%d \n",btsco.pRX->iPacket_w, btsco.pRX->iPacket_r,count);

        spin_lock_irqsave(&auddrv_BTCVSDRX_lock, flags);
        u4DataRemained = (btsco.pRX->iPacket_w - btsco.pRX->iPacket_r) * (SCO_RX_PLC_SIZE + BTSCO_CVSD_PACKET_VALID_SIZE); 
        ...
        PRINTK_AUDDRV("AudDrv_btcvsd_read read_size=%d, BTSCORX_ReadIdx_tmp=%d \n",read_size, BTSCORX_ReadIdx_tmp);
        ...
        {
            PRINTK_AUDDRV("AudDrv_btcvsd_read 1 copy_to_user target=0x%x, source=0x%x, read_size=%d \n",Read_Data_Ptr,((kal_uint8 *)btsco.pRX->PacketBuf+BTSCORX_ReadIdx_tmp),read_size);
            if (copy_to_user((void __user *)Read_Data_Ptr, (void *)((kal_uint8 *)btsco.pRX->PacketBuf + BTSCORX_ReadIdx_tmp), read_size))
            {
                printk("AudDrv_btcvsd_read Fail 1 copy to user Read_Data_Ptr:%p, pRX->PacketBuf:%p, BTSCORX_ReadIdx_tmp:%zu, read_size:%zu",
                       Read_Data_Ptr, (kal_uint8 *)btsco.pRX->PacketBuf, BTSCORX_ReadIdx_tmp, read_size);
                if (read_count == 0)
                {
                    return -1;
        ...
        else 
        {
            ...
            PRINTK_AUDDRV("AudDrv_btcvsd_read 2-2 copy_to_user target=0x%x, source=0x%x, size_1=%d\n",Read_Data_Ptr,((kal_uint8 *)btsco.pRX->PacketBuf+BTSCORX_ReadIdx_tmp),size_1);
        ...
        if (count != 0)
        {
            ...
            PRINTK_AUDDRV("AudDrv_btcvsd_read WAITING... pRX->iPacket_r=%d, count=%d \n",btsco.pRX->iPacket_r,count);
            t1 = sched_clock();
            ...
            t2 = sched_clock();
            PRINTK_AUDDRV("AudDrv_btcvsd_read WAKEUP...count=%d \n",count);
            t2 = t2 - t1; 
...
static int AudDrv_btcvsd_mod_init(void)
{
    ...
    if (!mtk_btcvsd_dev)
    {
        printk("-AudDrv_btcvsd_mod_init, platform_device_alloc() fail, return\n");
        return -ENOMEM;
    }
    ...
    if (ret != 0)
    {
        printk("-AudDrv_btcvsd_mod_init, platform_device_add() fail, return\n");
