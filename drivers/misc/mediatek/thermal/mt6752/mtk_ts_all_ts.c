...
#define TSALLTS_TEMP_CRIT 120000 
...
static int tsallts_get_temp5(struct thermal_zone_device *thermal,
			       unsigned long *t)
{
#if MTK_ALLTS_SW_FILTER == 1
	int curr_temp;
...
static ssize_t tsallts_write1(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
		...
		&time_msec) == 32)
	{
		...
		if (NULL == thz_dev1) {
			thz_dev1 = mtk_thermal_zone_device_register("mtkts1", num_trip, NULL, &tsallts_dev_ops1, 0, 0, 0, interval);
		}
		return count;
	}
	else
...
static ssize_t tsallts_write5(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
	int len=0,time_msec=0;
	int trip[10]={0};
	int t_type[10]={0};
		...
		&time_msec) == 32)
	{
		...
		tsallts_dprintk("[tsallts_write5] g_THERMAL_TRIP_0=%d,g_THERMAL_TRIP_1=%d,g_THERMAL_TRIP_2=%d,g_THERMAL_TRIP_3=%d,g_THERMAL_TRIP_4=%d,\
		g_THERMAL_TRIP_5=%d,g_THERMAL_TRIP_6=%d,g_THERMAL_TRIP_7=%d,g_THERMAL_TRIP_8=%d,g_THERMAL_TRIP_9=%d,\n",
			g_THERMAL_TRIP[0],g_THERMAL_TRIP[1],g_THERMAL_TRIP[2],g_THERMAL_TRIP[3],g_THERMAL_TRIP[4],
			g_THERMAL_TRIP[5],g_THERMAL_TRIP[6],g_THERMAL_TRIP[7],g_THERMAL_TRIP[8],g_THERMAL_TRIP[9]);
		...
		tsallts_dprintk("[tsallts_write5] trip_0_temp=%d,trip_1_temp=%d,trip_2_temp=%d,trip_3_temp=%d,trip_4_temp=%d,\
		trip_5_temp=%d,trip_6_temp=%d,trip_7_temp=%d,trip_8_temp=%d,trip_9_temp=%d,time_ms=%d\n",
		trip_temp[0],trip_temp[1],trip_temp[2],trip_temp[3],trip_temp[4],
		trip_temp[5],trip_temp[6],trip_temp[7],trip_temp[8],trip_temp[9],interval);
		...
		if (NULL == thz_dev5) {
			thz_dev5 = mtk_thermal_zone_device_register("mtkts5", num_trip, NULL, &tsallts_dev_ops5, 0, 0, 0, interval);
		}
...
static ssize_t tsallts_write3(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
		...
		&time_msec) == 32)
	{
		...
		if (NULL == thz_dev3) {
			thz_dev4 = mtk_thermal_zone_device_register("mtkts3", num_trip, NULL, &tsallts_dev_ops3, 0, 0, 0, interval);
		}
...
static ssize_t tsallts_write4(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
		...
		&time_msec) == 32)
	{
		...
		tsallts_dprintk("[tsallts_write4] tsallts_register_thermal\n");
                if (NULL == thz_dev4) {
                        thz_dev4 = mtk_thermal_zone_device_register("mtkts4", num_trip, NULL, &tsallts_dev_ops4, 0, 0, 0, interval);
                }
