...
#define mtktspmic_TEMP_CRIT 150000 
...
void mtktspmic_read_6325_efuse(void);
extern kal_uint32 upmu_get_reg_value(kal_uint32 reg);

static u16 ts_pmic_read(u16 addr)
{
	u32 rdata=0;
...
void mtktspmic_read_6325_efuse(void)
{
    ...
    mtktspmic_dprintk("[mtktspmic_read_6325_efuse] start\n");
    ...
    for(i=0xC;i<=0xE;i++)
    {
		...
		udelay(1000);
        printk("5. 6325 delay 1 ms\n");


        
        efusevalue[j] = upmu_get_reg_value(0x0C18);
        printk("6325_efuse : efusevalue[%d]=0x%x\n",j, efusevalue[j]);
	...
	g_adc_cali_en = (efusevalue[0]>>14)&0x1;
    g_degc_cali   = ((efusevalue[0]>>15)&0x1) + ((efusevalue[1]& 0x1F)<<1);
    g_o_vts       = ((efusevalue[1]>>5)&0x7FF) + (((efusevalue[2])&0x3)<<11);
	g_o_slope_sign= (efusevalue[2]>>2)&0x1;
    g_o_slope     = (efusevalue[2]>>3)&0x3F;
	g_id          = (efusevalue[2]>>9)&0x1;
...
static int mtktspmic_get_hw_temp(void)
{
	...
	temp = PMIC_IMM_GetOneChannelValue(AUX_TSENSE_AP , y_pmic_repeat_times , 2);
...
mtktspmic_read_6325_efuse();
    ...
    seq_printf(m,"6325/PMIC_Thermal: ts_pmic_read(0x0E9E) = 0x%x\n", pmic_data);




	 seq_printf(m,"6325/PMIC_Thermal: g_o_vts        = 0x%x\n", g_o_vts);
     seq_printf(m,"6325/PMIC_Thermal: g_degc_cali    = 0x%x\n", g_degc_cali);
     seq_printf(m,"6325/PMIC_Thermal: g_adc_cali_en  = 0x%x\n", g_adc_cali_en);
     seq_printf(m,"6325/PMIC_Thermal: g_o_slope      = 0x%x\n", g_o_slope);
     seq_printf(m,"6325/PMIC_Thermal: g_o_slope_sign = 0x%x\n", g_o_slope_sign);
     seq_printf(m,"6325/PMIC_Thermal: g_id           = 0x%x\n", g_id);
