...
#define K2_VCODEC_DEV_MAJOR_NUMBER 160   

static dev_t vcodec_devno = MKDEV(K2_VCODEC_DEV_MAJOR_NUMBER,0);
...
static VAL_EVENT_T DecHWLockEvent;    
static VAL_EVENT_T EncHWLockEvent;    
static VAL_EVENT_T DecIsrEvent;    
static VAL_EVENT_T EncIsrEvent;    
static VAL_INT32_T K2Driver_Open_Count;         
static VAL_UINT32_T gu4PWRCounter = 0;      
static VAL_UINT32_T gu4EncEMICounter = 0;   
static VAL_UINT32_T gu4DecEMICounter = 0;   
static VAL_UINT32_T gu4L2CCounter = 0;      
static VAL_BOOL_T   bIsOpened = VAL_FALSE;    
static VAL_UINT32_T gu4HwVencIrqStatus = 0; 

static VAL_UINT32_T gu4VdecPWRCounter = 0;  
static VAL_UINT32_T gu4VencPWRCounter = 0;  

static VAL_UINT32_T gLockTimeOutCount = 0;

static VAL_UINT32_T gu4VdecLockThreadId = 0;

#ifdef K2_VCODEC_DEBUG
#undef VCODEC_DEBUG
...
void enc_isr(void)
{
    ...
    else if (grVcodecEncHWLock.eDriverType == VAL_DRIVER_TYPE_HEVC_ENC) 
    {
        MFV_LOGE("[enc_isr] VAL_DRIVER_TYPE_HEVC_ENC!!\n");
    }
    else
    {
...
static long vcodec_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    switch(cmd)
    {
        case VCODEC_SET_THREAD_ID:
            {
                MFV_LOGE("[K2] VCODEC_SET_THREAD_ID [EMPTY] + tid = %d\n", current->pid);
                MFV_LOGE("[K2] VCODEC_SET_THREAD_ID [EMPTY] - tid = %d\n", current->pid);
            }
            break;
        ...
        case VCODEC_ALLOC_NON_CACHE_BUFFER:
            {
                MFV_LOGE("[K2][M4U]! VCODEC_ALLOC_NON_CACHE_BUFFER + tid = %d\n", current->pid);
                ...
                rTempMem.u4ReservedSize  = (VAL_ULONG_T)dma_alloc_coherent(0, rTempMem.u4MemSize, (dma_addr_t *)&rTempMem.pvMemPa, GFP_KERNEL);
        ...
        case VCODEC_FREE_NON_CACHE_BUFFER:
            {
                MFV_LOGE("[K2][M4U]! VCODEC_FREE_NON_CACHE_BUFFER + tid = %d\n", current->pid);
        ...
        case VCODEC_INC_DEC_EMI_USER:
            {
                MFV_LOGD("[K2] VCODEC_INC_DEC_EMI_USER + tid = %d\n", current->pid);
        ...
        case VCODEC_DEC_DEC_EMI_USER:
            {
                MFV_LOGD("[K2] VCODEC_DEC_DEC_EMI_USER + tid = %d\n", current->pid);
        ...
        case VCODEC_INC_ENC_EMI_USER:
            {
                MFV_LOGD("[K2] VCODEC_INC_ENC_EMI_USER + tid = %d\n", current->pid);
        ...
        case VCODEC_DEC_ENC_EMI_USER:
            {
                MFV_LOGD("[K2] VCODEC_DEC_ENC_EMI_USER + tid = %d\n", current->pid);
        ...
        case VCODEC_LOCKHW:
            {
                MFV_LOGD("[K2] VCODEC_LOCKHW + tid = %d\n", current->pid);
                user_data_addr = (VAL_UINT8_T *)arg;
        ...
        case VCODEC_UNLOCKHW:
            {
                MFV_LOGD("[K2] VCODEC_UNLOCKHW + tid = %d\n", current->pid);
                user_data_addr = (VAL_UINT8_T *)arg;
                ret = copy_from_user(&rHWLock, user_data_addr, sizeof(VAL_HW_LOCK_T));
                if (ret)
                {
        ...
        case VCODEC_INC_PWR_USER:
            {
                MFV_LOGD("[K2] VCODEC_INC_PWR_USER + tid = %d\n", current->pid);
                user_data_addr = (VAL_UINT8_T *)arg;
        ...
        case VCODEC_DEC_PWR_USER:
            {
                MFV_LOGD("[K2] VCODEC_DEC_PWR_USER + tid = %d\n", current->pid);
                user_data_addr = (VAL_UINT8_T *)arg;
                ret = copy_from_user(&rPowerParam, user_data_addr, sizeof(VAL_POWER_T));
                if (ret) {
        ...
        case VCODEC_WAITISR:
            ...
            {
                MFV_LOGE("[K2] VCODEC_INITHWLOCK [EMPTY] + - tid = %d\n", current->pid);
                MFV_LOGE("[K2] VCODEC_INITHWLOCK [EMPTY] - - tid = %d\n", current->pid);
            }
            break;
            ...
            {
                MFV_LOGE("[K2] VCODEC_DEINITHWLOCK [EMPTY] + - tid = %d\n", current->pid);
                MFV_LOGE("[K2] VCODEC_DEINITHWLOCK [EMPTY] - - tid = %d\n", current->pid);
            }
            break;
        ...
        case VCODEC_GET_CPU_LOADING_INFO:
            {
                ...
                MFV_LOGD("[K2] VCODEC_GET_CPU_LOADING_INFO +\n");
                user_data_addr = (VAL_UINT8_T *)arg;
        ...
        case VCODEC_GET_CORE_LOADING:
            {
                MFV_LOGD("[K2] VCODEC_GET_CORE_LOADING + - tid = %d\n", current->pid);
        ...
        case VCODEC_GET_CORE_NUMBER:
            {
                MFV_LOGD("[K2] VCODEC_GET_CORE_NUMBER + - tid = %d\n", current->pid);
        ...
        case VCODEC_SET_CPU_OPP_LIMIT:
            {
                MFV_LOGE("[K2] VCODEC_SET_CPU_OPP_LIMIT [EMPTY] + - tid = %d\n", current->pid);
                user_data_addr = (VAL_UINT8_T *)arg;
...
static int vcodec_open(struct inode *inode, struct file *file)
{
    ...
    MFV_LOGE("vcodec_open pid = %d, K2Driver_Open_Count %d\n", current->pid, K2Driver_Open_Count);
...
static int vcodec_flush(struct file *file, fl_owner_t id)
{
    ...
    MFV_LOGE("vcodec_flush pid = %d, K2Driver_Open_Count %d\n", current->pid, K2Driver_Open_Count);
...
static int vcodec_release(struct inode *inode, struct file *file)
{
    ...
    mutex_lock(&DriverOpenCountLock);
    MFV_LOGE("vcodec_release pid = %d, K2Driver_Open_Count %d\n", current->pid, K2Driver_Open_Count);
    K2Driver_Open_Count--;

    if (K2Driver_Open_Count == 0)
    {
        mutex_lock(&VencHWLock);
        if (grVcodecEncHWLock.eDriverType == VAL_DRIVER_TYPE_H264_ENC)
        {
...
MODULE_DESCRIPTION("K2 Vcodec Driver");
MODULE_LICENSE("GPL");
