...
static unsigned int             sg_wp_size = 0;  
...
#if 0
#define MTK_RESERVE_REGION 0xa800                                           
#define OTP_RESERVE_REGION 0x15800                                          
...
unsigned int  emmc_otp_start(void)
{
    ...
    if (mmc_card_blockaddr(host_ctl->mmc->card)){
        ...
        l_size = host_ctl->mmc->card->csd.capacity * (1 << host_ctl->mmc->card->csd.read_blkbits); 
    ...
    l_addr = lp_hd_struct->start_sect;      
