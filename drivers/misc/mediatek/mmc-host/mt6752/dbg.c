...
u32 gpt_enable = 0;
u32 sdio_pro_enable = 0;   
static unsigned long long sdio_pro_time = 30;     
...
static int sd_multi_rw_compare(int host_num, uint address, int count)
{
    ...
    for(i=0; i<count; i++) 
    {
        ...
        mutex_lock(&sd_lock);
        error = sd_multi_rw_compare_slave(host_num, 0, address); 
        if(error)
        {
        ...
        for(j=0; j<1; j++){
            error = sd_multi_rw_compare_slave(host_num, 1, address); 
            if(error)
            {
...
int emmc_multi_rw_compare(int host_num, uint address, int count)
{
    ...
    for(i=0; i<count; i++) 
    {
        ...
        error = emmc_multi_rw_compare_slave(host_num, 0, address); 
        if(error)
        {
            printk("[%s]: failed to write data, error=%d\n", __func__, error);
        ...
        for(j=0; j<1; j++){
            error = emmc_multi_rw_compare_slave(host_num, 1, address); 
            if(error)
            {
                printk("[%s]: failed to read data, error=%d\n", __func__, error);
                break;
        ...
        if(id >= HOST_MAX_NUM || id < 0)
            ...
            if((p2 < 0) || (p2 > 2))
                ...
                if (p2 == 0){ 
                    ...
                    msdc_get_rdtdsel_dbg(host, 1, &p3); 
                    msdc_get_rdtdsel_dbg(host, 0, &p4); 
...
static ssize_t msdc_voltage_proc_write(struct file *file, const char __user* buf, size_t count, loff_t *data)
{
    ...
    if( sdio_vio18_flag > 1600 && sdio_vio18_flag <2000)
    {
        ...
        pmic_config_interface(0x68c, vio18_reg, 0x7F, 0); 
    ...
    if( sdio_vcore1_flag > 900 && sdio_vcore1_flag <1200)
    {
        ...
        pmic_config_interface(0x63c, vcore1_reg, 0x7F, 0); 
    ...
    if( sdio_vcore2_flag > 900 && sdio_vcore2_flag <1200)
    {
        ...
        pmic_config_interface(0x662, vcore2_reg, 0x7F, 0); 
