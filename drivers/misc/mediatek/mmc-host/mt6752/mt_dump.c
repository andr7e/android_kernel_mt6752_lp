...
static void msdc_mdelay(u32 time)
{
    ...
    t_start = localtimer_get_phy_count(); 
...
static void simp_msdc_dump_register(struct simp_msdc_host *host)
{
    ...
    if(host->id == 0) {
      ...
      #if 0
        pr_err( "R[%x]=0x%x  R[%x]=0x%x  R[%x]=0x%x  R[%x]=0x%x  R[%x]=0x%x\n",
        ...
        pr_err( "R[%x]=0x%x  R[%x]=0x%x  R[%x]=0x%x  R[%x]=0x%x\n",
...
static unsigned int simp_mmc_enable_clk(struct simp_mmc_host *host)
{
    ...
    clk_setl(msdc_apmixed_base+0x25C, 0x3);
    msdc_mdelay(1);
    clk_clrl(msdc_apmixed_base+0x25C, 0x2);

    clk_setl(msdc_apmixed_base+0x254, 0x80000000);

    clk_setl(msdc_apmixed_base+0x250, 0x1);
    msdc_mdelay(1);

    
    mt_reg_sync_writel(0x02060200, (msdc_topckgen_base+0x070));

    
    clk_setl(msdc_infracfg_base+0x8C, 0x14);

#if MTK_MMC_DUMP_DBG
    pr_err( "[pll]0xF000C25C =0x%x, 0xF000C250 =0x%x, [mux]0xF0000070 =0x%x, [cg]0xF0001094 =0x%x\n",
            sdr_read32(msdc_apmixed_base+0x25C), sdr_read32(msdc_apmixed_base+0x250), sdr_read32(msdc_topckgen_base+0x070), sdr_read32(msdc_infracfg_base+0x094));
#endif
    ...
    struct simp_mmc_card *oldcard)
{
    ...
    sdr_set_field(SDC_CFG, SDC_CFG_BUSWIDTH, 1);  
...
static int simp_mmc_sd_init(struct simp_mmc_host *host)
{
    ...
    phost->card->rca = (resp & 0xffff0000) >> 16;     
...
static int emmc_dump_read(unsigned char* buf, unsigned int len, unsigned int offset,unsigned int slot)
{
    ...
    for (i = 0; i < (len/512); i++) {
        ...
        msdc_ctl.address = (l_start_offset >> 9) + i; 
