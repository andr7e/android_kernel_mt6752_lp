...
#if 0 
                    msdc_ldo_power(on, MT65XX_POWER_LDO_VGP6, VOL_3000, &g_msdc2_io); 
#endif
                }
        ...
        unsigned long       timeout)
{
    ...
    if (intsts & cmdsts) {
        ...
        if ((intsts & MSDC_INT_CMDRDY) || (intsts & MSDC_INT_ACMDRDY) ||
            ...
            IRQ_MSG("[%s]: msdc%d XXX CMD<%d> MSDC_INT_RSPCRCERR Arg<0x%.8x>\n", __func__, host->id, cmd->opcode, cmd->arg);
            
            msdc_reset_hw(host->id);
        } else if (intsts & MSDC_INT_CMDTMO) {
            cmd->error = (unsigned int)-ETIMEDOUT;
            IRQ_MSG("[%s]: msdc%d XXX CMD<%d> MSDC_INT_CMDTMO Arg<0x%.8x>\n", __func__, host->id, cmd->opcode, cmd->arg);
...
static void msdc_tune_async_request(struct mmc_host *mmc, struct mmc_request *mrq)
{
    ...
    do{
        ...
        if (cmd->error == (unsigned int)-ETIMEDOUT) {
                ...
                else if (cmd->opcode == MMC_READ_SINGLE_BLOCK || cmd->opcode == MMC_READ_MULTIPLE_BLOCK || cmd->opcode == MMC_WRITE_BLOCK || cmd->opcode == MMC_WRITE_MULTIPLE_BLOCK ){
...
out:
    if (host->hw->host_function == MSDC_SD) {
        if (err_count >= HTC_MAX_RETRY_COUNT) {
            ...
            ERR_MSG("%s() Tuning end (insert:%d, count:%d)", __func__, is_card_present(host), count);
        }
    }
...
static int msdc_drv_probe(struct platform_device *pdev)
{
    ...
    sdr_write16(PWR_GPIO_EO, (l_val | PWR_MASK_EN | PWR_MASK_VOL_33 | PWR_MASK_VOL_18));
