...
{
       return single_open(file, ccci_modem_proc_show, NULL);
}
...
void ccci_modem_ex_proc_init(struct ccci_port *port)
{
       ...
       ccci_modem_ex_proc = proc_create("ccci_ex_info", 0664, NULL, &ccci_modem_ex_fops);
       if(ccci_modem_ex_proc == NULL) {
               CCCI_DBG_MSG(port->modem->index, KERN, "fail to create proc entry\n");
       }
}
#endif
...
#ifdef CONFIG_HTC_MODEM_EXCEPTION_INFO_EXPORT
    if ((port != NULL) && (!strcmp(port->name, "ccci_ctrl"))) {
...
#ifdef CONFIG_HTC_MODEM_EXCEPTION_INFO_EXPORT
            memset(ccci_modem_ex_info_msg, 0, sizeof(ccci_modem_ex_info_msg));
            snprintf(ccci_modem_ex_info_msg, CCCI_MODEM_EX_INFO_MAX_SIZE, "%d%02d%02d %02d:%02d:%02d.%u, exception type(%d):%s, %s, %d, %d, %d, %d\n",
                tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday,
                tm.tm_hour, tm.tm_min, tm.tm_sec,
                (unsigned int) tv.tv_usec,
                debug_info->type,debug_info->name?:"Unknown",
