...
static long dev_char_ioctl( struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	unsigned int tmp_md_img_list[MAX_IMG_NUM]; 
	int scaned_num;
...
static int port_char_recv_req(struct ccci_port *port, struct ccci_request *req)
{
    ...
    if(!atomic_read(&port->usage_cnt) && (port->rx_ch!=CCCI_UART2_RX && port->rx_ch!=CCCI_FS_RX && port->rx_ch!=CCCI_RPC_RX)) 
        goto drop;
...
{
    register_chrdev_region(MKDEV(md->major, md->minor_base), 120, CCCI_DEV_NAME); 
