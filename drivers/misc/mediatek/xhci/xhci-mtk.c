...
#ifdef CONFIG_HTC_MHL_DETECTION
void mhl_status_notifier_func(bool isMHL, bool isPOW)
{
	if (isMHL) {
		if ((mtk_idpin_cur_stat == IDPIN_IN_HOST) && isPOW) {
			mtk_disable_pmic_otg_mode();
		...
		mtk_xhci_mtk_log("[Cable][%s] MHL disconnect, enable OUT detect\n", __func__);
	}
}
...
static int htc_second_detect()
{
	...
	if ((adc_value >= 645 && adc_value <= 990))
#ifdef CONFIG_HTC_MHL_DETECTION
...
{
	int id_status = 0, adc_value, acce_type;
	...
	if (id_status == 0) {
		...
		if (adc_value > -100 && adc_value < 90)
			...
			else if (adc_value > 200 && adc_value < 280)
	...
	prev_type = acce_type;
	return (stable_count >= ADC_RETRY) ? acce_type : -2;
