...
static int __init mt_spi_probe(struct platform_device *pdev)
{
	...
	if(pdev->dev.of_node)
	{
		...
		{
			if_config=0;
			dev_err(&pdev->dev, "SPI get DTS spi pin failed, using DCT tool setting as default config\n");
		}
		...
		{
			dev_err(&pdev->dev, "SPI get DTS spi pin mode failed, using DCT tool setting as default config\n");
		}
