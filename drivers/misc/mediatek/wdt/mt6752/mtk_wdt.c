...
void wdt_arch_reset(char mode)
{
	...
	printk("wdt_arch_reset called@Kernel mode =%d\n",(int)mode);
	...
	else if(mode == NO_AUTOREBOOT_RESET) 
	{
	...
	if(mode == SW_RESET)
		...
		printk("wdt_arch_reset: %s_reset happen\n", (mode == NO_AUTOREBOOT_RESET)?"NO_reboot":"HW");
...
static void wdt_fiq(void *arg, void *regs, void *svc_sp)
{
	...
	mrdump_set_ramdump_reason(AEE_REBOOT_MODE_WDT, "wdt_fiq");
