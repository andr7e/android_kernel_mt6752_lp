...
#include <mach/i2c.h>
#if defined(CONFIG_MD32_SUPPORT) && defined(CONFIG_MTK_SENSOR_HUB_SUPPORT)
...
static DEFINE_MUTEX(mutex_mt_i2c_standard_i2c);
#define TAG     "MT_I2C"
...
static S32 i2c_set_speed(mt_i2c *i2c)
{
  ...
  if((mode == FS_MODE && khz > MAX_HS_MODE_SPEED) || (mode == HS_MODE && khz > MAX_HS_MODE_SPEED)){
    I2CERR(" the speed is too fast for this mode.\n");
    I2C_BUG_ON((mode == FS_MODE && khz > MAX_HS_MODE_SPEED) || (mode == HS_MODE && khz > MAX_HS_MODE_SPEED));
    ret = -EINVAL_I2C;
    goto end;
  }
...
static S32 standard_i2c_transfer(struct i2c_adapter *adap, struct i2c_msg msgs[], S32 num)
{
	...
	mutex_lock(&mutex_mt_i2c_standard_i2c);
	...
	mutex_unlock(&mutex_mt_i2c_standard_i2c);
...
static S32 mt_i2c_probe(struct platform_device *pdev)
{
  ...
  mt_i2c *i2c = NULL;
  struct resource *res;
#ifdef MTK_I2C_DTS_GPIO
#define I2C_GPIOS_DATA_NUM 2
  int sda_gpios[I2C_GPIOS_DATA_NUM];
  int scl_gpios[I2C_GPIOS_DATA_NUM];
#endif
  ...
  if (of_property_read_u32(pdev->dev.of_node, "def_speed", &pdev->id)) {
    ...
    i2c->def_speed = pdev->id;
  }

#ifdef MTK_I2C_DTS_GPIO
  if (i2c->id != I2C2) {
    if (of_property_read_u32_array(pdev->dev.of_node, "mtk,scl-gpio", scl_gpios, I2C_GPIOS_DATA_NUM)) {
      I2CERR("I2C get scl-gpio failed\n");
      return -ENODEV;
    }

    if (of_property_read_u32_array(pdev->dev.of_node, "mtk,sda-gpio", sda_gpios, I2C_GPIOS_DATA_NUM)) {
      I2CERR("I2C get sda-gpio failed\n");
      return -ENODEV;
    }

    pr_info("scl_gpio: %d %d, sda_gpio: %d %d\n", scl_gpios[0], scl_gpios[1], sda_gpios[0], sda_gpios[1]);

    mt_set_gpio_mode(scl_gpios[0] | 0x80000000, scl_gpios[1]);
    mt_set_gpio_mode(sda_gpios[0] | 0x80000000, sda_gpios[1]);
  }
#endif
