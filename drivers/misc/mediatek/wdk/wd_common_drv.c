...
void mtk_watchdog_bark(void)
{
	pr_notice("%s has been called! dumping stack...\n", __func__);
	dump_stack();

	if (!g_enable) {
		pr_notice("%s: MTK Apps Watchdog is not enabled.\n", __func__);
		return;
	}

	pr_notice("%s: triggering MTK Apps Watchdog bark...\n", __func__);	
...
#if defined(CONFIG_HTC_DEBUG_WATCHDOG)
					htc_debug_watchdog_update_last_pet(sched_clock());
#endif
					local_bit = 0;
				}
				kick_bit = local_bit;
