...
static int md32_vow_notify(struct notifier_block *self, unsigned long action, void *dev)
{
    ...
    switch (action)
    {
        ...
        case MD32_SELF_TRIGGER_TCM_SWAP:
        {
            ...
            if (ret < 0)
            {
                xlog_printk(ANDROID_LOG_INFO, VOW_TAG, "md32_vow_notify swap tcm failed\n");
            }
            else
            {
                vowserv.swapid = -1;
...
#endif
        default:
            xlog_printk(ANDROID_LOG_INFO, VOW_TAG, "[VOW_Kernel] md32 vow get unkown action %d\n", action);
            break;
    }
    return NOTIFY_OK;
}
...
bool vow_ipi_sendmsg(vow_ipi_msgid_t id, void *buf, unsigned int size, unsigned int type, uint32 wait)
{
    ...
    xlog_printk(ANDROID_LOG_INFO, VOW_TAG, "[VOW_Kernel] vow_ipi_sendmsg:%x\n", id);
    if (type == 0)
    {
        if(size!=0)
    ...
    if (status != DONE)
    {
        xlog_printk(ANDROID_LOG_INFO, VOW_TAG, "[VOW_Kernel] vow_ipi_sendmsgFail:%x %x\n", id, status);
        return false;
    }
#endif
    return true;
...
void vow_ipi_handler(int id, void *data, unsigned int len)
{
    ...
    switch (*vowmsg)
    {
        ...
        {
            xlog_printk(ANDROID_LOG_INFO, VOW_TAG, "MD32_IPIMSG_VOW_Enable_ACK:result_%x\n",result);
        ...
        {
            xlog_printk(ANDROID_LOG_INFO, VOW_TAG, "MD32_IPIMSG_VOW_DISABLE_ACK:result_%x\n",result);
            if(result != VOW_IPI_SUCCESS)
            {
                vowserv.ipi_fail_result = true;
            }
        ...
        {
            xlog_printk(ANDROID_LOG_INFO, VOW_TAG, "MD32_IPIMSG_VOW_SETMODE_ACK:result_%x\n",result);
            if(result != VOW_IPI_SUCCESS)
            {
                vowserv.ipi_fail_result = true;
            }
        ...
        case MD32_IPIMSG_VOW_APREGDATA_ADDR_ACK:
        {
            ...
            xlog_printk(ANDROID_LOG_INFO, VOW_TAG, "MD32_IPIMSG_VOW_APREGDATA_ADDR_ACK:result_%x %x\n",result, *ptr32);
            if(result != VOW_IPI_SUCCESS)
            {
                vowserv.ipi_fail_result = true;
            }
        ...
        {
            xlog_printk(ANDROID_LOG_INFO, VOW_TAG, "MD32_IPIMSG_SET_VOW_MODEL_ACK:result_%x\n",result);
            if(result != VOW_IPI_SUCCESS)
            {
                vowserv.ipi_fail_result = true;
            }
        ...
        {
            xlog_printk(ANDROID_LOG_INFO, VOW_TAG, "MD32_IPIMSG_VOW_SETGAIN_ACK:result_%x\n",result);
            if(result != VOW_IPI_SUCCESS)
            {
                vowserv.ipi_fail_result = true;
            }
        ...
        {
            xlog_printk(ANDROID_LOG_INFO, VOW_TAG, "MD32_IPIMSG_SET_FLAG_ACK:result_%x\n",result);
            if(result != VOW_IPI_SUCCESS)
            {
                vowserv.ipi_fail_result = true;
            }
        ...
        {
            xlog_printk(ANDROID_LOG_INFO, VOW_TAG, "MD32_IPIMSG_VOW_RECOGNIZE_OK\n");
            vow_ipi_reg_ok(vowmsg[2]);
            break;
        }
        default:
            xlog_printk(ANDROID_LOG_INFO, VOW_TAG, "UNKNOWN MSG NOTIFY FROM MD32:%x\n", *vowmsg);
            break;
    }
}
...
static bool vow_ipimsg_wait(vow_ipi_msgid_t id)
{
    ...
    while (vowserv.ipimsgwait)
    {
        ...
        {
            xlog_printk(ANDROID_LOG_INFO, VOW_TAG, "Error: IPI MSG timeout:id_%x\n", id);
            vowserv.ipimsgwait = false;
            return false;
        }
    }
    xlog_printk(ANDROID_LOG_INFO, VOW_TAG, "IPI MSG -: time:%x, id:%x\n", timeout, id);
#endif
...
static void vow_service_Init(void)
{
   ...
   xlog_printk(ANDROID_LOG_INFO, VOW_TAG, "vow_service_Init:%x\n", init_flag);
   if(init_flag == 0)
   {
#if DYNAMIC_VOW_SWAP
      vowserv.swapid          = -1;
      ...
      xlog_printk(ANDROID_LOG_INFO, VOW_TAG, "Set Debug1 Buffer Address:%x\n", vowserv.voicedata_scp_addr);
...
int vow_service_GetParameter(unsigned long arg)
{
    ...
    {
        xlog_printk(ANDROID_LOG_INFO, VOW_TAG, "Get parameter fail\n");
        return -EFAULT;
    }    
    xlog_printk(ANDROID_LOG_INFO, VOW_TAG, "Get parameter: %lu %lu %lu %lu\n", vowserv.vow_info_apuser[0], vowserv.vow_info_apuser[1], vowserv.vow_info_apuser[2], vowserv.vow_info_apuser[3]);
...
static int vow_service_CopyModel(int model, int slot)
{
    switch(model)
    {
        case VOW_MODEL_INIT:
            ...
            {
                xlog_printk(ANDROID_LOG_INFO, VOW_TAG, "Copy Init Model Fail\n");
                return -EFAULT;
            }
            break;
        case VOW_MODEL_SPEAKER:
            ...
            {
                xlog_printk(ANDROID_LOG_INFO, VOW_TAG, "Copy Speaker Model Fail\n");
                return -EFAULT;
            }
        ...
        case VOW_MODEL_NOISE:
            ...
            {
                xlog_printk(ANDROID_LOG_INFO, VOW_TAG, "Copy Noise Model Fail\n");
                return -EFAULT;
            }
            break;
        case VOW_MODEL_FIR:
            ...
            {
                xlog_printk(ANDROID_LOG_INFO, VOW_TAG, "Copy FIR Model Fail\n");
                return -EFAULT;
            }
            break;
        default:
...
static int32 vow_service_FindFreeSpeakerModel(void)
{
    ...
    {
        xlog_printk(ANDROID_LOG_INFO, VOW_TAG, "Find Free Speaker Model Fail\n");
        return -1;
    }
    return I;
}
...
static int32 vow_service_SearchSpeakerModel(int id)
{
    ...
    if( I == MAX_VOW_SPEAKER_MODEL)
    {
        xlog_printk(ANDROID_LOG_INFO, VOW_TAG, "Search Speaker Model By ID Fail:%x\n", id);
        return -1;
    }
    return I;
}
...
static bool vow_service_ReleaseSpeakerModel(int id)
{
    ...
    if(I == -1)
    {
        xlog_printk(ANDROID_LOG_INFO, VOW_TAG, "Speaker Model Fail:%x\n", id);
        return false;
    }
...
static bool vow_service_SetSpeakerModel(unsigned long arg)
{
    ...
    if( I == -1)
    {
        xlog_printk(ANDROID_LOG_INFO, VOW_TAG, "SetSpeakerModel Fail\n");
        return false;
    }
    vow_service_GetParameter(arg);
...
int VowDrv_EnableHW(int status)
{
    ...
    if (status < 0){
  	    xlog_printk(ANDROID_LOG_INFO, VOW_TAG, "VowDrv_EnableHW error input:%x \n", status);
        ret = -1;
    }
    else
    {
...
{
    xlog_printk(ANDROID_LOG_INFO, VOW_TAG, "VowDrv_SetFlag:type%x, set:%x\n", type, set);
...
static int VowDrv_SetVowEINTStatus(int status)
{
    ...
    {
        xlog_printk(ANDROID_LOG_INFO, VOW_TAG, "VowDrv_SetVowEINTStatus error input:%x \n", status);
        ret = -1;
    }
    return ret;
}
...
static long VowDrv_ioctl(struct file *fp, unsigned int cmd, unsigned long arg)
{
    ...
    switch ((unsigned int)cmd)
    {
        ...
        case VOW_SET_CONTROL:
        {
            ...
            { 
                case VOWControlCmd_Init:
                    xlog_printk(ANDROID_LOG_INFO,VOW_TAG,"VOW_SET_CONTROL VOWControlCmd_Init");
                    vow_service_Init();
                    break;
                ...
                case VOWControlCmd_EnableDebug:
                    xlog_printk(ANDROID_LOG_INFO,VOW_TAG,"VOW_SET_CONTROL VOWControlCmd_EnableDebug");
                    vowserv.voicedata_idx = 0;
                    vowserv.recording_flag = true;
                    VowDrv_SetFlag(VOW_FLAG_DEBUG, true);
                    break;
                case VOWControlCmd_DisableDebug:
                    xlog_printk(ANDROID_LOG_INFO,VOW_TAG,"VOW_SET_CONTROL VOWControlCmd_DisableDebug");
                    VowDrv_SetFlag(VOW_FLAG_DEBUG, false);
                    vowserv.recording_flag = false;
                    vow_service_getVoiceData();
                    break;
                default:
                    xlog_printk(ANDROID_LOG_INFO, VOW_TAG,"VOW_SET_CONTROL no such command = %x", arg);
                    break;
            }
            break;
        }
        ...
        {
            xlog_printk(ANDROID_LOG_INFO, VOW_TAG,"VOW_SET_SPEAKER_MODEL(%x)", arg);
        ...
        {
            xlog_printk(ANDROID_LOG_INFO, VOW_TAG,"VOW_CLR_SPEAKER_MODEL(%x)", arg);
        ...
        {
            xlog_printk(ANDROID_LOG_INFO, VOW_TAG,"VOW_SET_INIT_MODEL(%x)", arg);
        ...
        {
            xlog_printk(ANDROID_LOG_INFO, VOW_TAG,"VOW_SET_FIR_MODEL(%x)", arg);
        ...
        {
            xlog_printk(ANDROID_LOG_INFO, VOW_TAG,"VOW_SET_NOISE_MODEL(%x)", arg);
        ...
        {
            xlog_printk(ANDROID_LOG_INFO, VOW_TAG,"VOW_SET_APREG_INFO(%x)", arg);
        ...
        {
            xlog_printk(ANDROID_LOG_INFO, VOW_TAG,"VOW_SET_MODE(%x)", arg);
        ...
        {
            vow_ipi_reg_ok(0);
            xlog_printk(ANDROID_LOG_INFO, VOW_TAG,"VOW_FAKE_WAKEUP(%x)", arg);
        }
        default:
        {
            xlog_printk(ANDROID_LOG_INFO, VOW_TAG,"WrongParameter(%x)", arg);
            break;
        }
    }
    ...
    if(VowDrv_Wait_Queue_flag ==1)
    {
        ...
        else
        {
            ...
            if(vowserv.md32_command_flag)
            {
                ...
                xlog_printk(ANDROID_LOG_INFO, VOW_TAG, "Wakeup by MD32\n");
