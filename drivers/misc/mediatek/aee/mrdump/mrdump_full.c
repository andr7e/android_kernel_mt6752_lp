...
AEE_REBOOT_MODE mrdump_get_ramdump_reason(void) {
	if(mrdump_cb == NULL)
		return -1;
	return mrdump_cb->crash_record.reboot_mode;
}
EXPORT_SYMBOL(mrdump_get_ramdump_reason);

int mrdump_set_ramdump_reason(AEE_REBOOT_MODE reset_reason, char *msg)
{
	if(mrdump_cb == NULL)
		return -1;

        mrdump_cb->crash_record.reboot_mode = reset_reason;


	printk(KERN_INFO "[%s] reboot_mode:%d\n", __func__, reset_reason);

        switch(mrdump_get_ramdump_reason()) {
        case AEE_REBOOT_MODE_KERNEL_PANIC:
		snprintf(mrdump_cb->crash_record.msg, sizeof(mrdump_cb->crash_record.msg),
				"KP: %s", msg);
#ifdef CONFIG_HTC_REBOOT_REASON_INFORMATION
                htc_set_reboot_reason_ramdump(mrdump_cb->crash_record.msg);
#endif
                break;
...
EXPORT_SYMBOL(mrdump_set_ramdump_reason);
...
#error "Unsupport configuration"
	...
	if((crash_record->reboot_mode == AEE_REBOOT_MODE_RAMDUMP) || (crash_record->reboot_mode == AEE_REBOOT_MODE_NORMAL))
        {
...
static int __init mrdump_init_log(void)
{
	...
	printk(KERN_INFO "%s: mrdump_cb->sig:", __func__);
	for(i=0; i<8; i++)
		printk(KERN_INFO "%c", mrdump_cb->sig[i]);
	printk(KERN_INFO ".\n");
	...
	if (memcmp(mrdump_cb->sig, "XRDUMP01", 8) == 0) {
		...
		printk(KERN_INFO "%s: No LK signature found\n", __func__);
	}
...
static int __init mrdump_init(void)
{
	...
	mrdump_set_ramdump_reason(AEE_REBOOT_MODE_RAMDUMP, "UNKNOWN!");
