...
#ifndef CONFIG_NAND_OTP_SUPPORT
#define CONFIG_NAND_OTP_SUPPORT 0
#endif

#if CONFIG_NAND_OTP_SUPPORT
...
static u8 *local_buffer_16_align;   
...
static int mtk_nand_write_oob(struct mtd_info *mtd, struct nand_chip *chip, int page)
{
    ...
    if (mtk_nand_write_oob_hw(mtd, chip, page_in_block + mapped_block * page_per_block  ))
    {
...
static int mtk_nand_probe(struct platform_device *pdev)
{
    ...
    nand_chip->ecc.mode = hw->nand_ecc_mode;    
...
#if (CONFIG_NAND_OTP_SUPPORT && SAMSUNG_OTP_SUPPORT)
