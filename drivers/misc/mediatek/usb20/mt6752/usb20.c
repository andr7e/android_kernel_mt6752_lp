...
static ssize_t mt_usb_show_phy_setting(struct device* dev, struct device_attribute *attr, char *buf)
{
	int n = 0;
	if (!dev) {
		DBG(0,"dev is null!!\n");
	...
	n += scnprintf(buf + n, PAGE_SIZE - n, "0x5 = %x\n",USBPHY_READ8(0x5));
	n += scnprintf(buf + n, PAGE_SIZE - n, "0x6 = %x\n",USBPHY_READ8(0x6));
	return n;
}
static ssize_t mt_usb_store_phy_setting(struct device* dev, struct device_attribute *attr,
	const char *buf, size_t count)
{
	...
	if (!dev) {
		DBG(0,"dev is null!!\n");
		return count;
	} else if (2 == sscanf(buf, "%x %x", &offset, &value )) {
			USBPHY_WRITE8(offset, value);
	}
	return count;
}
DEVICE_ATTR(phy_setting,  0664, mt_usb_show_phy_setting, mt_usb_store_phy_setting);
...
static int mt_usb_probe(struct platform_device *pdev)
{
	...
	ret = device_create_file(&pdev->dev, &dev_attr_phy_setting);
#ifdef CONFIG_MTK_UART_USB_SWITCH
