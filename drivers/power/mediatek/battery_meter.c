...
kal_int32 gFG_columb_save = 0;
kal_bool  gFG_ever_full_charged = false;
kal_bool  gFG_rtc_used = false;
...
void fgauge_get_profile_id(void)
{
	...
	if(ret != 0)
		bm_print(BM_LOG_CRTI, "[fgauge_get_profile_id_klcib]id_volt read fail\n");
	else
		bm_print(BM_LOG_CRTI, "[fgauge_get_profile_id_klcib]id_volt = %d\n", id_volt);
	...
	bm_print(BM_LOG_CRTI, "[fgauge_get_profile_id_klcib]g_fg_battery_id = %d\n",g_fg_battery_id);
}
...
void fgauge_algo_run(void)
{
	...
	bm_print(BM_LOG_CRTI,
		 "[FGADC] Curr: %c%d,CC: %d,TCC: %c%d,Volt: %d,SOC_by_v: %d,%c(rtc)%d,(hw)%d,%d,(sw)%d,%d,SOC: %d,%d,%d,Caging: %d,%d,%d,%d,%d,%d,%d,%d,D0: %d,D1: %d,%d,%d\r\n",
		 gFG_Is_Charging>0? '+':'-' , gFG_current, gFG_columb,gFG_ever_full_charged==true?'f':' ',(gFG_columb+gFG_columb_save), gFG_voltage,
		 gFG_capacity_by_v,gFG_rtc_used==true?'*':' ', g_rtc_fg_soc, gFG_capacity_by_hwocv_init, gFG_hwocv_init, gFG_capacity_by_v_init, gFG_swocv_init,
