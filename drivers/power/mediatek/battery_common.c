...
static int g_ovp_det_gpio = ARCH_NR_GPIOS ;
static int g_ovp_irq = NR_IRQS;
...
{
    static kal_uint32 uiCount = 0;
    kal_uint32 uiEarly_complete_vol = fgauge_read_v_by_d(0) - 50;
...
{
    kal_int32 iGap_temp_map_tbl[] = {0, 10};
    kal_uint32 ui0percent_volt_map_tbl[] = {2950, 3400, 3400};
    int i;
...
void mt_battery_GetBatteryData(void)
{
	...
	battery_xlog_printk(BAT_LOG_CRTI,
			    ...
			    "bat_charging_state=(0x%x),ftm_chg_ctrl=(%d),chg_ctrl=(%d),overload=(%d)\n",
			    ...
			    g_ftm_charger_ctrl_stat, charger_ctrl_stat, BMT_status.is_overload);
...
static PMU_STATUS htc_battery_FullLevelCheck(void)
{
	...
	if ((0 < BMT_status.full_level) && (BMT_status.full_level < 100)) {
		...
		if (BMT_status.UI_SOC >= BMT_status.full_level) {
			...
			if (BMT_status.UI_SOC <= (BMT_status.full_level - 30)) { 
				BMT_status.bat_charging_state = CHR_PRE;
				is_input_chg_off_by_bounding = KAL_FALSE;
			}
		}
    ...
    switch( BMT_status.aicl_state )
    {
        ...
        case AICL_LOOP:
            if( battery_charging_control(CHARGING_CMD_HTC_AICL_CHECK_STATUS, &vchrin) == 0 ){
                ...
                ichr = CHR_CUR[index];
                if( ichr > CHARGE_CURRENT_1400_00_MA)
                    index = 3;
                else
                    index = 0;
                ichr = CHR_CUR[index];
...
#define LIMITED_POWER_STAGE_TWO_POWER  572
...
static irqreturn_t ovp_det_handler(unsigned int irq,struct irq_desc *desc)
{
	int value = gpio_get_value(g_ovp_det_gpio);

	if(!value){
		BMT_status.charger_protect_status = charger_OVER_VOL;
		BMT_status.bat_charging_state = CHR_ERROR;
		irq_set_irq_type(g_ovp_irq, IRQF_TRIGGER_RISING);
	}else {
		BMT_status.charger_protect_status = 0;
		BMT_status.bat_charging_state = CHR_PRE;
		irq_set_irq_type(g_ovp_irq, IRQF_TRIGGER_FALLING);
	}
	wake_up_bat();
	battery_xlog_printk(BAT_LOG_CRTI, "%s, gpio(%d) is %s\n",
		__func__, g_ovp_det_gpio, value?"high":"low");
	return IRQ_HANDLED;
}
static int htc_ovp_det_init(struct platform_device *dev)
{
	...
	if(of_machine_hwid() < 1){
		battery_xlog_printk(BAT_LOG_CRTI, "XA not support OVP.\n");
		return 0;
	}

	eint_node = of_find_compatible_node(NULL, NULL, eint_str);
	if(eint_node){
		rc = of_property_read_u32(eint_node,"interrupts",&g_ovp_det_gpio);
		if(rc){
			battery_xlog_printk(BAT_LOG_CRTI,"No GPIO info in eint_node.\n");
			return rc;
		}
		if (rc=!gpio_is_valid(g_ovp_det_gpio))
		{
			battery_xlog_printk(BAT_LOG_CRTI,"OVP GPIO is not valid.\n");
			return rc;
		}
		g_ovp_irq = irq_of_parse_and_map(eint_node, 0);
	} else {
		battery_xlog_printk(BAT_LOG_CRTI, "eint_node is NULL.\n");
		return rc;
	}
	battery_xlog_printk(BAT_LOG_CRTI, "%s, gpio(%d), irq(%d)\n", __func__, g_ovp_det_gpio, g_ovp_irq);

	irq_set_irq_type(g_ovp_irq, IRQF_TRIGGER_FALLING);
	irq_set_chained_handler(g_ovp_irq, ovp_det_handler);
