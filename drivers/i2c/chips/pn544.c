		...
		const struct i2c_device_id *id)
{
	...
	mt_set_gpio_mode(pni->ven_gpio, GPIO_IRQ_NFC_PIN_M_GPIO);
	mt_set_gpio_dir(pni->ven_gpio, GPIO_DIR_OUT);
	mt_set_gpio_out(pni->ven_gpio, GPIO_OUT_ZERO);

	I("%s: config NFC_IRQ pin\n", __func__); 
	mt_set_gpio_mode(pni->irq_gpio, GPIO_IRQ_NFC_PIN_M_EINT);
	...
	mt_set_gpio_mode(pni->firm_gpio, GPIO_IRQ_NFC_PIN_M_GPIO);
