	...
	struct i2c_client *client)
{
	...
	I("%s akm->sense_info[0]:%d, akm->sense_info[1]:%d\n", __func__, akm->sense_info[0], akm->sense_info[1]);

	return err;
}

#ifdef IS_MTK_PLATFORM
static int akm09911_irq_registration(unsigned int irq, irq_handler_t handler, unsigned long flags, const char *name, void *dev)
{
  struct device_node *node = NULL;
  int ret = 0; 
  u32 ints[2] = {0,0};

  node = of_find_compatible_node(NULL, NULL, "mediatek, IRQ_AKM09911-eint");
  if(node){
    ...
    ret = request_irq(irq, handler, flags, "IRQ_AKM09911-eint", dev);
    if(ret > 0){
      ret = -1;
...
#else
		prop = of_find_property(dt, "compass_akm09911,gpio_RSTN", NULL);
        if (prop) {
                of_property_read_u32(dt, "compass_akm09911,gpio_RSTN", &irq_gpio_flags);
                pdata->gpio_RSTN = irq_gpio_flags;
                I("%s: gpio_RSTN = %d\n", __func__, pdata->gpio_RSTN);
        } else {
                I("%s: compass_akm09911,gpio_RSTN not found", __func__);
        }
		if (pdata->gpio_RSTN < 0) {
				E("%s: get compass_akm09911,gpio_RSTN fails: pdata->gpio_RSTN\n", __func__);
                pdata->gpio_RSTN = 0;
		}
#endif
...
#endif
	I2CDMABuf_va = (u8 *)dma_alloc_coherent(&client->adapter->dev, DMA_SIZE, &I2CDMABuf_pa, GFP_KERNEL);
	if (I2CDMABuf_va == NULL) {
		I2CDMABuf_pa = NULL;
		E("%s: Allocate DMA I2C Buffer failed\n", __func__);
		return -ENOMEM;
	}
#endif
...
#else
	err = akm09911_irq_registration(client->irq, akm_compass_irq,
			IRQF_TRIGGER_HIGH, client->name, s_akm);
	if (err < 0) {
		dev_err(&client->dev,
			"%s: request irq failed.", __func__);
		goto exit5;
...
#ifdef IS_MTK_PLATFORM
	if (I2CDMABuf_va != NULL) {
		dma_free_coherent(&client->adapter->dev, DMA_SIZE, I2CDMABuf_va, I2CDMABuf_pa);
		I2CDMABuf_va = NULL;
		I2CDMABuf_pa = NULL;
	}
#endif
