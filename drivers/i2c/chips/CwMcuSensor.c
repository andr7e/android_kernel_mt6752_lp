...
struct cwmcu_data {
    ...
    struct delayed_work  mcu_irq_work;
			 ...
			 s64 timestamp)
{
	...
	if (mcu_data->indio_dev->active_scan_mask &&
		...
		if (mcu_data->indio_dev && (!mcu_data->w_clear_fifo_running))
...
static int cwmcu_set_touch_solution(struct cwmcu_data *mcu_data)
{
	...
	if (g_touch_status == TOUCH_STATUS_TURN_HUB_OFF ||
	  g_touch_status == TOUCH_STATUS_TURN_HUB_ON) {
		CWMCU_i2c_write_power(s_mcu_data, TOUCH_STATUS_REGISTER, &g_touch_status, 1);
	} else {
		W("[TP] %s: unexpected status = %d\n", __func__, g_touch_status);
	}
    ...
    uint8_t *data, int length)
{
	...
	for (retry = 0; retry < RETRY_TIMES; ++retry){
		...
		if (ret < 0){
			I("i2c_read_dma fail, addr is %x retry times = %d", client->addr, retry);
			continue;
		}
	...
	I("Dma I2C Read Error: 0x%04X, %d byte(s), err-code: %d", addr, length, ret);
	mutex_unlock(&dma_i2c_lock);
	return ret;
}
static int i2c_write_dma(struct i2c_client *client, uint8_t addr,
    uint8_t *data, int length)
{
	...
	for (retry = 0; retry < RETRY_TIMES; ++retry){
		...
		if (ret < 0){
			I("i2c_write_dma fail addr is %x retry times = %d", client->addr, retry);
			continue;
		}
		mutex_unlock(&dma_i2c_lock);
		return 0;
	}
	I("Dma I2C Write Error: 0x%04X, %d byte(s), err-code: %d", addr, length, ret);
	mutex_unlock(&dma_i2c_lock);
	return ret;
}
#endif
...
int touch_status(u8 status)
{
	...
	if (status == TOUCH_STATUS_TURN_HUB_OFF ||
	  status == TOUCH_STATUS_TURN_HUB_ON) {
		I("[TP] %s: status = %d\n", __func__, status);
...
static irqreturn_t cwmcu_irq_handler(int irq, void *handle)
{
    ...
    D("[CWMCU] %s\n", __func__);
    queue_delayed_work(mcu_data->mcu_wq, &mcu_data->mcu_irq_work, mcu_data->irq_delay_ms);
	mcu_data->irq_delay_ms = 0;
    return IRQ_HANDLED;
}
...
static void cwmcu_irq_work(struct work_struct *work)
{
	...
	if (INT_st4 & CW_MCU_INT_BIT_ANY_MOTION) {
		...
		ret = CWMCU_i2c_write_power(mcu_data, CWSTM32_INT_ST4, &clear_intr,
				      1);
	}
				  ...
				  const char *buf, size_t count)
{
	...
	for (i = 0; i < GESTURE_MOTION_UPDATE_ATTRIBUTE_LEN; i++) {
		CWMCU_i2c_write_power(mcu_data, GESTURE_MOTION_UPDATE_ATTRIBUTE, &data[i], 1);
	}
	cwmcu_powermode_switch(mcu_data, 0);
...
static void cwmcu_one_shot(struct work_struct *work)
{
	...
	if (mcu_data->w_batch_read == true) {
		...
		CWMCU_i2c_write(mcu_data, CWSTM32_BATCH_MODE_COMMAND, &clear_intr, 1);
		cwmcu_powermode_switch(mcu_data, 0);
				       ...
				       const struct i2c_device_id *id)
{
    ...
    INIT_DELAYED_WORK(&mcu_data->mcu_irq_work, cwmcu_irq_work);
