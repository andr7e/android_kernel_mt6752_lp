...
static int bma253_get_bandwidth(struct i2c_client *client, unsigned char *BW)
{
	...
	comres = bma253_smbus_read_byte(client, BMA253_BANDWIDTH__REG,
			&data);
...
static ssize_t bma253_chip_layout_show(struct device *dev,	
		struct device_attribute *attr, char *buf)
{
	if (gdata == NULL) {
		E("%s: gdata == NULL\n", __func__);
...
static ssize_t bma253_get_raw_data_show(struct device *dev,	
		struct device_attribute *attr, char *buf)
{
	struct bma253_data *bma253 = gdata;
	static struct bma253acc acc;
		...
		const char *buf, size_t count)
{
	...
	bma253_set_selftest_st(bma253->bma253_client, 1); 
	bma253_set_selftest_stn(bma253->bma253_client, 0); 
	mdelay(10);
	bma253_read_accel_x(bma253->bma253_client, &value1);
	bma253_set_selftest_stn(bma253->bma253_client, 1); 
	mdelay(10);
	...
	bma253_set_selftest_st(bma253->bma253_client, 2); 
	bma253_set_selftest_stn(bma253->bma253_client, 0); 
	mdelay(10);
	...
	bma253_set_selftest_st(bma253->bma253_client, 3); 
	bma253_set_selftest_stn(bma253->bma253_client, 0); 
	mdelay(10);
		...
		const char *buf, size_t count)
{
	...
	if(enable == 1 && !power_key_pressed){ 
	    ...
	    error = bma253_set_Int_Mode(bma253->bma253_client, 1);

	    error += bma253_set_slope_duration(bma253->bma253_client, 0x01);
	    error += bma253_set_slope_threshold(bma253->bma253_client, 0x07);

	    
	    error += bma253_set_Int_Enable(bma253->bma253_client, 5, 1);
	    error += bma253_set_Int_Enable(bma253->bma253_client, 6, 1);
	    error += bma253_set_Int_Enable(bma253->bma253_client, 7, 0);
	    ...
	    error += bma253_set_Int_Enable(bma253->bma253_client, 5, 0);
	    error += bma253_set_Int_Enable(bma253->bma253_client, 6, 0);
	    error += bma253_set_Int_Enable(bma253->bma253_client, 7, 0);
		...
		const char *buf, size_t count)
{
	...
	bma253->offset_buf[0] = ((bma253->pdata->negate_x) ? (user_cali_offset_buf[bma253->pdata->axis_map_x])
		   : (-user_cali_offset_buf[bma253->pdata->axis_map_x]));
	bma253->offset_buf[1] = ((bma253->pdata->negate_y) ? (user_cali_offset_buf[bma253->pdata->axis_map_y])
		   : (-user_cali_offset_buf[bma253->pdata->axis_map_y]));
	bma253->offset_buf[2] = ((bma253->pdata->negate_z) ? (user_cali_offset_buf[bma253->pdata->axis_map_z])
