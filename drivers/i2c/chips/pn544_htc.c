...
void pn547_work (struct work_struct *work) {
	...
	if (ret) {
		E("%s : request CPU2HUB_I2C_SCL NFC_I2C_SCL %d fail\n",
			__func__, NFC_I2C_SCL);
		return;
	}
	ret = gpio_request(NFC_I2C_SDA, "nfc_i2c_sda");
	if (ret) {
		E("%s : request CPU2HUB_I2C_SDA NFC_I2C_SDA %d fail\n",
			__func__, NFC_I2C_SDA);
		return;
	}
	mt_set_gpio_mode(NFC_I2C_SCL, GPIO_MODE_00);
	mt_set_gpio_mode(NFC_I2C_SDA, GPIO_MODE_00);
	mt_set_gpio_dir(NFC_I2C_SCL, GPIO_DIR_OUT);
	mt_set_gpio_dir(NFC_I2C_SDA, GPIO_DIR_OUT);
	mt_set_gpio_out(NFC_I2C_SCL, GPIO_OUT_ONE);
	mt_set_gpio_out(NFC_I2C_SDA, GPIO_OUT_ONE);
	I("%s : SCL_SDA_HIGH",__func__);
	mdelay(2);
	mt_set_gpio_out(NFC_I2C_SCL, GPIO_OUT_ZERO);
	I("%s : SCL_LOW",__func__);
	mdelay(1);
	mt_set_gpio_out(NFC_I2C_SDA, GPIO_OUT_ZERO);
