...
reswitch:
	...
	if ((status[16] & 0xF) != card->sd_bus_speed){
		...
		err = mmc_set_signal_voltage(card->host, MMC_SIGNAL_VOLTAGE_180);
		if (err)
			pr_err("[msdc1] sd_set_bus_speed_mode() Fail to reset new driving!\n");
	}
	...
	struct mmc_card *oldcard)
{
	...
	pr_notice("%s: %s status : %#x, err = %#x, speed : %d\n",
