...
static void gpio_keys_gpio_work_func(struct work_struct *work)
{
	...
	if ((mt_get_gpio_in(bdata->button->gpio) ? 1 : 0) ^ bdata->button->active_low)
				...
				const struct gpio_keys_button *button)
{
	...
	const char * eint_str[] = {
		"mediatek, KEY_POWER-eint",
		"mediatek, KEY_VOLUP-eint",
		"mediatek, KEY_VOLDOWN-eint"};
	int i = 0;
#endif
	...
	if (gpio_is_valid(button->gpio) && (button->irq <= 0)) {
		...
		default:
			i = -EINVAL;
			dev_err(dev,
				"Un-recognized keycode: %d\n",
				button->code);
			break;
		}
		if (i >= 0)
			eint_node = of_find_compatible_node(NULL, NULL, eint_str[i]);
		else
			eint_node = NULL;
		...
		if ((mt_get_gpio_in(button->gpio) ? 1 : 0) ^ button->active_low)
...
#if defined(MTK_PLATFORM)
	error = request_irq(bdata->irq, isr, irqflags, desc, bdata);
#else
...
#if defined(MTK_PLATFORM)
		if (of_property_read_u32(pp, "gpios", &gpio)) {
			error = -EINVAL;
			dev_err(dev,
				"Failed to get property: gpios, error: %d\n",
				error);
			goto err_free_pdata;
		}
