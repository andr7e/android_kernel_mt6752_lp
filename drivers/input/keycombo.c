...
void clear_hw_reset(void)
{
	...
	mt6325_upmu_set_rg_pwrkey_rst_en(0x00);
	mt6325_upmu_set_rg_homekey_rst_en(0x00);
	mt6325_upmu_set_rg_pwrkey_rst_td(1);
	msleep(300);
	mt6325_upmu_set_rg_pwrkey_rst_en(0x01);
	mt6325_upmu_set_rg_homekey_rst_en(0x00);
	mt6325_upmu_set_rg_pwrkey_rst_td(1);
	KEY_LOGI("clear_hw_reset----------\n");
}
...
static void keep_hw_reset_clear(struct work_struct *dummy)
{
	clear_hw_reset();
#if defined(MTK_PLATFORM)
	schedule_delayed_work(&clear_restart_work, msecs_to_jiffies(3000));
#else
...
{
	return scnprintf(buf, PAGE_SIZE, "no clr gpio\n");
}
